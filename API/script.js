// ===== Extraído de <script id='script-1' type='classic'> — linha 603 =====
...
function pushWordToken(word,ci){
  ...
  return`<span class="wunit ${lvcls}${hidden}${target}${uncls}" data-tid="${tid}" data-lv="${level>=1&&level<=6?level:'x'}" data-ci="${ci}" data-cilen="${[...word].length}" onclick="onTap(this)"><span class="hzrow">${buildHanziCells(word,pyInfo.parts)}</span></span>`;
}
...
// ===== Extraído de <script id='script-2' type='module'> — linha 2999 =====
(async()=>{
try{
...
// ===== Extraído de <script id='v29-script' type='classic'> — linha 3017 =====
...
// ===== Extraído de <script id='script-4' type='classic'> — linha 3175 =====
...
// ===== Extraído de <script id='v32-script' type='classic'> — linha 3358 =====
...
// ===== Extraído de <script id='v34-script' type='classic'> — linha 3836 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='v35-ttsfree-natural-script' type='classic'> — linha 3969 =====
...
// ===== Extraído de <script id='v36-audio-books-flashcards-fix' type='classic'> — linha 4035 =====
...
// ===== Extraído de <script id='v37-baidu-books-sources-script' type='classic'> — linha 4328 =====
...
// ===== Extraído de <script id='script-10' type='classic'> — linha 4468 =====
(()=>{
...
// ===== Extraído de <script id='v39-dict-sents-term-audio-patch' type='classic'> — linha 4639 =====
...
// ===== Extraído de <script id='v41-edge-sogou-voice-fix' type='classic'> — linha 4688 =====
(()=>{
...
// ===== Extraído de <script id='v42-edge-sogou-finalfix' type='classic'> — linha 4755 =====
(()=>{
'use strict';
...
// ===== Extraído de <script id='v44-sogou-debug-bridge-fix' type='classic'> — linha 4987 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='script-15' type='classic'> — linha 5088 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='h46-edge-tts-fetch-proxy-fix' type='classic'> — linha 5194 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='h48-extractor-sources-generate-selection-tts' type='classic'> — linha 5452 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='v35-script' type='classic'> — linha 5845 =====
(function(){
'use strict';
...
// ===== Extraído de <script id='v36-script' type='classic'> — linha 6926 =====
(function(){
'use strict';
      const VOICES = [
        { name:'zh-CN-XiaoxiaoNeural', label:'晓晓 · Xiaoxiao — feminina expressiva', styles:['general','affectionate','angry','assistant','calm','chat','chat-casual','cheerful','customerservice','disgruntled','excited','fearful','friendly','gentle','lyrical','newscast','poetry-reading','sad','serious','sorry','whispering'] },
...
// ===== Extraído de <script id='hz-practice-module' type='classic'> — linha 8145 =====
...
// ===== Extraído de <script id='hz-settings-i18n-v49' type='classic'> — linha 8329 =====
...
// ===== Extraído de <script id='hz-stats-v50' type='classic'> — linha 8514 =====
...
// ===== Extraído de <script id='hz-v50-profile-sources' type='classic'> — linha 8716 =====
...
// ===== Extraído de <script id='hz-voice-api-fallback' type='classic'> — linha 8946 =====
/* Voz: motor direto SEMPRE primeiro (comportamento idêntico ao HTML único);
   /api/edge-tts entra apenas como contingência quando hospedado (Vercel etc.).
   Em file:// ou sem a rota, o fallback falha em silêncio e nada muda. */
(function(){
'use strict';
function hzApiTtsBlob(ssml,settings){
  settings=settings||{};
  const payload={
    ssml:String(ssml||''),
    format:settings.quality||settings.format||'audio-24khz-48kbitrate-mono-mp3',
    voice:settings.voice||settings.classicVoice||'zh-CN-XiaoxiaoNeural',
    style:settings.style||settings.classicStyle||'general',
    degree:settings.degree||settings.styledegree||1.35,
    speed:settings.speed||1,pitch:settings.pitch||0,volume:settings.volume||0
  };
  return fetch('/api/edge-tts',{method:'POST',headers:{'Content-Type':'application/json','X-HZ-App':'1'},body:JSON.stringify(payload)})
    .then(r=>{if(!r.ok)return r.text().then(t=>{throw new Error('api tts '+r.status+': '+String(t).slice(0,220));});return r.blob();})
    .then(b=>{if(!b||b.size<200)throw new Error('api tts blob vazio');return b;});
}
function hzWrap(name){
  const orig=window[name];
  if(typeof orig!=='function'||orig.__hzApiWrapped)return false;
  const wrapped=async function(ssml,settings){
    try{return await orig.apply(this,arguments);}          // 1) motor direto (HTML único)
    catch(e){
      if(location.protocol==='http:'||location.protocol==='https:'){
        try{return await hzApiTtsBlob(ssml,settings);}     // 2) contingência: rota da Vercel
        catch(e2){e.message=(e.message||'')+' | api: '+(e2.message||e2);}
      }
      throw e;
    }
  };
  wrapped.__hzApiWrapped=true;
  window[name]=wrapped;
  return true;
}
let tries=0;
(function boot(){
  const a=hzWrap('H46_audioFromSsml');
  const b=hzWrap('h42AudioFromSsml');
  const c=hzWrap('H46_edgeBlob');
  if((a||b||c)||++tries>10)return;
  setTimeout(boot,700);
})();
})();
