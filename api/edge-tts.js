...
async function endpoint(){
  ...
  if(!r.ok) throw new Error(`endpoint ${r.status}: ${raw.slice(0,500)}`);
...
async function ttsBuffer(payload){
  ...
  const output = payload.format || 'audio-24khz-48kbitrate-mono-mp3';
...
module.exports = async function handler(req,res){
  try{
    if(req.method === 'OPTIONS'){ res.setHeader('access-control-allow-methods','GET,POST,OPTIONS'); res.setHeader('access-control-allow-headers','content-type,x-hz-app'); return res.status(204).end(); }
    ...
    const {buf, contentType} = await ttsBuffer(payload);
