...
// hsrc-simple-import-listener: abrir Sources filtradas para leitura simples.
{const osrc=document.getElementById('osrc');if(osrc){osrc.addEventListener('click',()=>{try{closeModals();}catch{}try{if(window.hsrcSetType)window.hsrcSetType('simple_reading');}catch{}try{showScreen('sd');}catch{}setTimeout(()=>{try{if(window.hsrcSetType)window.hsrcSetType('simple_reading');}catch{}},80);});}}
...
'use strict';
      ...
      function v36DefaultSettings() {
        return {
          ...
          emotionV3On: false,          // Voz Emocional v3 Beta — usa dicionários expandidos
...
/* hsrc-book-import-render-wrapper: restaura botão Importar em Livros e integra com Sources locais. */
(function(){
  function goLocalSources(type){
    try{closeModals();}catch{}
    try{if(window.hsrcSetType)window.hsrcSetType(type||null);}catch{}
    try{showScreen('sd');}catch{}
    setTimeout(()=>{try{if(window.hsrcSetType)window.hsrcSetType(type||null);}catch{}},80);
  }
  function injectBookImport(){
    try{
      if(typeof v29LibMode==='undefined'||v29LibMode!=='books')return;
      const bc=document.getElementById('bc');
      const tools=bc&&bc.querySelector('.lib-tools');
      if(!tools)return;
      let btn=tools.querySelector('#book-import-chip');
      if(!btn){
        btn=document.createElement('button');
        btn.className='lib-chip';
        btn.id='book-import-chip';
        btn.textContent='Importar';
        const ref=tools.querySelector('#book-new-chip')||tools.children[1]||null;
        tools.insertBefore(btn,ref);
      }
      btn.onclick=()=>goLocalSources('book');
      const wrap=document.getElementById('book-wrap');
      const empty=wrap&&wrap.querySelector('.emptyx');
      if(empty&&/Nenhum livro/.test(empty.textContent||''))empty.innerHTML='<b>Nenhum livro.</b><br>Toque em Importar ou Novo livro para adicionar.';
    }catch(e){}
  }
  try{
    const oldRender=renderLib;
    if(typeof oldRender==='function'&&!oldRender.__hsrcBookImport){
      renderLib=function(){const r=oldRender.apply(this,arguments);setTimeout(injectBookImport,0);return r;};
      renderLib.__hsrcBookImport=true;
    }
  }catch(e){}
  document.addEventListener('click',e=>{if(e.target&&e.target.closest&&e.target.closest('#book-import-chip')){e.preventDefault();e.stopPropagation();goLocalSources('book');}},true);
  setTimeout(injectBookImport,400);setTimeout(injectBookImport,1200);setTimeout(injectBookImport,2500);
})();
