...
export function evaluateDrawing(strokes, { refSeqs = null, refCount = null, canvasW = 300, canvasH = 300 } = {}) {
  const clean = (strokes || []).filter(s => s && s.length >= 2 && lengthOf(s) >= 4);
  const n = clean.length;
  const empty = { percent: 0, parts: { order: 0, direction: 0, shape: 0, position: 0, proportion: 0 }, codes: '', refSeq: null, hasRefSeq: false, hasRefCount: Boolean(refCount), recognized: false };
  if (!n) return empty;
  ...
  const centerScore = clamp01(1 - offset / (0.34 * size));
  ...
  const position = clamp01(centerScore * 0.62 + fitScore * 0.38);
  ...
  const fillScore = clamp01(1 - Math.abs(fill - 0.46) / 0.46);
  const aspect = Math.abs(Math.log2(Math.max(4, raw.w) / Math.max(4, raw.h)));
  const aspectScore = clamp01(1 - aspect / 1.35);
  const proportion = clamp01((countRatio != null ? countRatio : 0.7) * 0.5 + fillScore * 0.3 + aspectScore * 0.2);

  const W = { order: 0.25, direction: 0.20, shape: 0.20, position: 0.20, proportion: 0.15 };
  const total = order * W.order + direction * W.direction + shape * W.shape + position * W.position + proportion * W.proportion;
  const percent = Math.round(clamp01(total) * 100);
  const recognized = percent >= 60 && (hasRefSeq ? order >= 0.5 : (countRatio == null || countRatio >= 0.75));

  const pct = v => Math.round(clamp01(v) * 100);
  return {
    percent,
    parts: { order: pct(order), direction: pct(direction), shape: pct(shape), position: pct(position), proportion: pct(proportion) },
...
function initHanziWritingUI() {
  ...
  const I18N = {
    pt: {
      ...
      showRef: 'Mostrar referência', hideRef: 'Ocultar referência', loadingRef: 'Buscando ordem dos traços…',
      ...
      pOrder: 'Ordem', pDirection: 'Direção', pShape: 'Forma', pPosition: 'Posição', pProportion: 'Proporção',
    ...
    en: {
      ...
      showRef: 'Show reference', hideRef: 'Hide reference', loadingRef: 'Fetching stroke order…',
      ...
      pOrder: 'Order', pDirection: 'Direction', pShape: 'Shape', pPosition: 'Position', pProportion: 'Proportion',
    ...
    es: {
      ...
      showRef: 'Mostrar referencia', hideRef: 'Ocultar referencia', loadingRef: 'Buscando el orden de los trazos…',
      ...
      pOrder: 'Orden', pDirection: 'Dirección', pShape: 'Forma', pPosition: 'Posición', pProportion: 'Proporción',
  ...
  function buildGame(panel) {
    const state = {
      ...
      strokes: [], active: null, finished: false, ctx: null, w: 0, h: 0, dpr: 1
    };
    panel.innerHTML = `
      <div class="hzw2-shell">
        ...
        <div class="hzw2-body">
          <aside class="hzw2-ref" data-ref>
            <button type="button" class="hzw2-ref-toggle" data-ref-toggle aria-expanded="true">${esc(t('hideRef'))}<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    function resize() {
      ...
      state.w = Math.max(240, Math.round(r.width));
      state.h = Math.max(240, Math.round(r.height));
      refs.canvas.width = Math.round(state.w * state.dpr);
      refs.canvas.height = Math.round(state.h * state.dpr);
    ...
    function showResult(res) {
      const bars = [['pOrder', res.parts.order], ['pDirection', res.parts.direction], ['pShape', res.parts.shape], ['pPosition', res.parts.position], ['pProportion', res.parts.proportion]]
    ...
    function registerCurrent() {
      ...
      state.items.push({ char: state.char || '·', percent: res.percent, parts: res.parts, recognized: res.recognized, strokes: state.strokes.length, hasRefSeq: res.hasRefSeq, at: Date.now() });
      showResult(res);
      state.strokes = [];
      syncCounts(); draw();
    ...
    function finish(reason = 'ui') {
      ...
      const score = items.reduce((s, i) => s + i.percent, 0) + recognized * 25;
      const stats = [
        { label: t('mCharsDone'), value: n },
        { label: t('mRecognized'), value: `${recognized}/${n}` },
        { label: t('mAvgSim'), value: avg + '%' },
        { label: t('mOrder'), value: avgPart('order') + '%' },
        { label: t('mDirection'), value: avgPart('direction') + '%' },
        { label: t('mPosition'), value: avgPart('position') + '%' },
      ...
      try {
        window.hzStore?.saveSession?.('hanzi-writing', {
          ...
          durationMs: duration, score, items: items.map(i => ({ c: i.char, p: i.percent, r: i.recognized ? 1 : 0 }))
        });
      } catch {}
      const restart = () => {
        state.items = []; state.strokes = []; state.startedAt = Date.now(); state.finished = false;
        window.__hzPracticeFinish = r => finish(r);
        hideResult(); syncCounts(); draw();
      };
      const show = () => {
        if (typeof window.hzShowPracticeCelebration === 'function') {
          window.hzShowPracticeCelebration({
            kicker: t('celKicker'), title: t('celTitle'),
            score: recognized, total: n, percent: avg, scoreLabel: String(score),
            subtitle: t('celSubtitle')(n), stats,
            againLabel: t('again'), onAgain: restart, onClose: () => window.hzBackToHub?.()
          });
    ...
    function openIntro(firstRun) {
      document.getElementById('hzw2-intro')?.remove();
      const steps = t('introSteps').map((s, i) => `<li><b>${i + 1}</b><span>${esc(s)}</span></li>`).join('');
      ...
      wrap.innerHTML = `<div class="hz51-overlay-back"></div>
        <section class="hz51-intro-card" tabindex="-1">
          <p class="hz51-kicker">${esc(t('helpBtn'))}</p>
          <h2>${esc(t('introTitle'))}</h2>
          <p class="hz51-lead">${esc(t('introLead'))}</p>
          <ol class="hz51-steps">${steps}</ol>
          <div class="hz51-intro-actions"><button type="button" class="pri" data-intro-ok>${esc(firstRun ? t('start') : t('gotIt'))}</button></div>
        </section>`;
      document.body.appendChild(wrap);
      const closeIt = () => { wrap.classList.remove('open'); setTimeout(() => wrap.remove(), 160); };
      wrap.querySelector('[data-intro-ok]').onclick = () => { prefSet(INTRO_KEY, '1'); closeIt(); };
    ...
    let ro = null;
    if ('ResizeObserver' in window) { ro = new ResizeObserver(() => requestAnimationFrame(resize)); ro.observe(refs.wrap); }
    window.addEventListener('resize', resize, { passive: true });
    requestAnimationFrame(() => requestAnimationFrame(() => { resize(); syncCounts(); }));
