...
const text={
  pt:{
    practice:'Prática',toneListen:'Tons e escuta',production:'Escrita',leisure:'Lazer',toneTab:'Tons',productionTab:'Escrita',leisureTab:'Lazer',
  ...
  en:{
    practice:'Practice',toneListen:'Tones and listening',production:'Writing',leisure:'Leisure',toneTab:'Tones',productionTab:'Writing',leisureTab:'Leisure',
  ...
  es:{
    practice:'Práctica',toneListen:'Tonos y escucha',production:'Escritura',leisure:'Ocio',toneTab:'Tonos',productionTab:'Escritura',leisureTab:'Ocio',
...
const showPracticeCelebration=showPracticeSummary;
...
const audioService=practiceAudioService;
...
function installPracticeHub(){
  ...
  if(hub.dataset.hzLearningOrganized){
    ...
    if(shell){refreshPracticeCopy(shell);setPracticeCategory(shell,'listening',{persist:false});}return;
  }
  ...
  tabs.innerHTML=[['production',tr().productionTab],['listening',tr().toneTab],['leisure',tr().leisureTab]].map(([key,label])=>`<button type="button" role="tab" data-practice-tab="${key}" aria-selected="false">${esc(label)}</button>`).join('');
  ...
  panels.append(production,listening,leisure);shell.append(tabs,panels);hub.replaceChildren(shell);
  ...
  setPracticeCategory(shell,'listening',{persist:false});refreshPracticeCopy(shell);
}
...
function sequenceHelpTopics(){
  const c=tr(),language=uiLang();
  const labels=language==='en'
    ?['Goal','How to answer','How to correct','Scoring','Finish or leave']
    :language==='es'
      ?['Objetivo','Cómo responder','Cómo corregir','Puntuación','Concluir o salir']
      :['Objetivo','Como responder','Como corrigir','Pontuação','Concluir ou sair'];
  return[
    {title:labels[0],detail:c.howIntro},
    {title:labels[1],detail:`${c.how1} ${c.how2} ${c.how3}`},
    {title:labels[2],detail:language==='en'?'Tap a filled position to remove it, or use Undo before checking.':language==='es'?'Toca una posición llena para quitarla o usa Deshacer antes de comprobar.':'Toque numa posição preenchida para removê-la ou use Desfazer antes de conferir.'},
    {title:labels[3],detail:c.how4},
    {title:labels[4],detail:c.howNote}
  ];
}
class ToneSequenceGame{
  constructor(panel){
    ...
    this.stats={done:0,hits:0,streak:0,bestStreak:0,replays:0,sessionStart:0,combos:{},perTone:{1:{seen:0,err:0},2:{seen:0,err:0},3:{seen:0,err:0},4:{seen:0,err:0},5:{seen:0,err:0}}};
  }
  ...
  async next(){
    if(this.destroyed||this.finished)return;
    audioService.cancelScope?.('microsoft-tts');
  ...
  async check(){
    ...
    if(!correct){const combination=this.item.tones.map(tone=>tone===5?'·':tone).join('–');s.combos[combination]=(s.combos[combination]||0)+1;}
  ...
  finish(reason='ui'){
    ...
    const score=Math.max(0,Math.min(100,Math.round(percent*.88+Math.min(12,s.bestStreak*2))));
    const hardestEntry=Object.entries(s.combos).sort((a,b)=>b[1]-a[1])[0];
    const hardest=hardestEntry?`${hardestEntry[0]} (${hardestEntry[1]})`:c.noErrors;
    try{window.hzStore?.saveSession?.('tone-sequence',{done:s.done,hits:s.hits,errors,percent,bestStreak:s.bestStreak,replays:s.replays,perTone:s.perTone,combos:s.combos,durationMs:duration,score,endedBy:reason});}catch{}
    const stats=[
    ...
    const restart=()=>{
      this.stats={done:0,hits:0,streak:0,bestStreak:0,replays:0,sessionStart:Date.now(),combos:{},perTone:{1:{seen:0,err:0},2:{seen:0,err:0},3:{seen:0,err:0},4:{seen:0,err:0},5:{seen:0,err:0}}};
    ...
    requestAnimationFrame(()=>showPracticeCelebration({title:c.sessionDone,score,percent,scoreLabel:String(score),subtitle:c.celSubtitle(s.done),stats,againLabel:c.restart,onAgain:restart,onBack:()=>window.hzBackToHub?.(),onClose:()=>window.hzBackToHub?.()}));
  }
}
...
function bootEnhancements(){
  installReaderCapture();installReaderSetting();installPracticeHub();patchProfileAvatar();patchScreenNavigation();
}
