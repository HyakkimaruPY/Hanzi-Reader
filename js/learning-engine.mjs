...
const text={
  pt:{
    practice:'Prática',toneListen:'Tons e escuta',production:'Produção e escrita',
    ...
    dockTitle:'Botão flutuante do leitor',dockSub:'Personalize apenas os controles sobre o texto.',dockSize:'Tamanho',dockOpacity:'Opacidade',auto:'Automático',small:'Pequeno',medium:'Médio',large:'Grande',
  ...
  en:{
    practice:'Practice',toneListen:'Tones and listening',production:'Production and writing',
    ...
    dockTitle:'Reader floating controls',dockSub:'Customize only the controls over the text.',dockSize:'Size',dockOpacity:'Opacity',auto:'Automatic',small:'Small',medium:'Medium',large:'Large',
...
function loadDockSettings(){
  const defaults={size:'auto',opacity:null};
  try{
    const parsed=JSON.parse(localStorage.getItem(DOCK_KEY)||'null');
    const size=['auto','small','medium','large'].includes(parsed?.size)?parsed.size:'auto';
    const opacity=Number.isFinite(Number(parsed?.opacity))?Math.max(.55,Math.min(1,Number(parsed.opacity))):null;
    return{size,opacity};
  }catch{return defaults;}
}
function saveDockSettings(value){try{localStorage.setItem(DOCK_KEY,JSON.stringify({...value,v:1,changedAt:Date.now()}));}catch{}}
function applyDockSettings(value=loadDockSettings()){
  const root=document.documentElement;
  root.dataset.readerDockSize=value.size||'auto';
  if(value.opacity==null)root.style.removeProperty('--hz-reader-dock-opacity');
  else root.style.setProperty('--hz-reader-dock-opacity',String(value.opacity));
  window.dispatchEvent(new CustomEvent('hz:reader-dock-change',{detail:value}));
}
...
function installReaderCapture(){
  ...
  const observer=new MutationObserver(records=>{
    if(queued||!records.some(record=>[...record.addedNodes].some(node=>node.nodeType===1&&(node.id==='read-play'||node.querySelector?.('#read-play')))))return;
    queued=true;requestAnimationFrame(()=>{queued=false;const button=$('#read-play');if(readerEnabled()&&button&&enhancedReader.state==='idle'&&!button.classList.contains('hzle-reader-main'))enhancedReader.update();});
  });
...
function installReaderSetting(){
  applyDockSettings();
  const host=$('#mo-style #style-scroll')||$('#mo-style .ms');if(!host||$('#hzle-reader-settings',host))return;
  const c=tr(),settings=loadDockSettings();
  const block=document.createElement('section');block.id='hzle-reader-settings';block.className='hzle-reader-settings';
  block.innerHTML=`<div class="style-row hzle-setting-card"><div><div class="style-lbl">${esc(c.enhanced)}</div><div class="style-sub">${esc(c.enhancedSub)}</div></div><button class="stog hzle-toggle" type="button" aria-pressed="false"></button></div><div class="hzle-dock-setting"><div class="hzle-dock-title"><strong>${esc(c.dockTitle)}</strong><span>${esc(c.dockSub)}</span></div><div class="hzle-setting-line"><span>${esc(c.dockSize)}</span><div class="hzle-size-options" role="group" aria-label="${esc(c.dockSize)}">${[['auto',c.auto],['small',c.small],['medium',c.medium],['large',c.large]].map(([value,label])=>`<button type="button" data-dock-size="${value}">${esc(label)}</button>`).join('')}</div></div><label class="hzle-setting-line hzle-opacity-line"><span>${esc(c.dockOpacity)} <output id="hzle-opacity-value"></output></span><input id="hzle-dock-opacity" type="range" min="55" max="100" step="5"></label></div>`;
  ...
  const sync=()=>{
    const current=loadDockSettings(),enabled=readerEnabled();
    toggle.classList.toggle('on',enabled);toggle.setAttribute('aria-pressed',String(enabled));toggle.setAttribute('aria-label',enabled?c.enabled:c.disabled);
    block.querySelectorAll('[data-dock-size]').forEach(button=>button.classList.toggle('on',button.dataset.dockSize===current.size));
    const effective=current.opacity??responsiveDefault();range.value=String(Math.round(effective*100));output.value=`${Math.round(effective*100)}%`;output.textContent=output.value;
  };
  toggle.addEventListener('click',()=>{const on=!readerEnabled();saveReaderEnabled(on);if(!on)enhancedReader.reset();sync();});
  block.addEventListener('click',event=>{const button=event.target.closest('[data-dock-size]');if(!button)return;const current=loadDockSettings();current.size=button.dataset.dockSize;saveDockSettings(current);applyDockSettings(current);sync();});
  range.addEventListener('input',()=>{const current=loadDockSettings();current.opacity=Number(range.value)/100;saveDockSettings(current);applyDockSettings(current);output.value=`${range.value}%`;output.textContent=output.value;});
  sync();
}
...
function installPracticeHub(){
  ...
  if(hub.dataset.hzLearningOrganized){
    const lateWriting=$('#hzp-writing',hub),target=$('[data-hzle-skill="production"] .hzle-practice-list',hub);
    if(lateWriting&&target&&lateWriting.parentElement!==target){lateWriting.querySelector('.hzp-lbl').innerHTML=`${esc(tr().writing)}<br><span class="hzp-sub">${esc(tr().writingSub)}</span>`;target.appendChild(lateWriting);}return;
  }
  ...
  const listening=group(tr().toneListen),production=group(tr().production);listening.dataset.hzleSkill='listening';production.dataset.hzleSkill='production';
  oldTone.remove();oldTone.querySelector('.hzp-lbl').innerHTML=`${esc(tr().toneGame)}<br><span class="hzp-sub">${esc(tr().toneGameSub)}</span>`;listening.lastElementChild.appendChild(oldTone);
  ...
  shell.append(listening,production);hub.replaceChildren(shell);
}

class ToneSequenceGame{
  constructor(panel){this.panel=panel;this.state=loadLearning();this.round=0;this.total=8;this.score=0;this.item=null;this.answer=[];this.replays=0;this.startedAt=0;this.locked=false;this.introduced=false;}
...
const observer=new MutationObserver(records=>{
  if(records.some(record=>record.target?.closest?.('#mo-style')||[...record.addedNodes].some(node=>node.nodeType===1&&(node.matches?.('#hz-sp-hub,#spf,#mo-style,#read-play')||node.querySelector?.('#hz-sp-hub,#spf,#mo-style,#read-play')))))queueEnhancements();
});
function boot(){bootEnhancements();observer.observe(document.body,{childList:true,subtree:true});setTimeout(queueEnhancements,700);}
