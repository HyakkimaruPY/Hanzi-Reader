...
class ToneSequenceGame{
  ...
  finish(){
    audioService.cancel();const c=tr(),percent=Math.round(this.score/Math.max(1,this.total)*100);
    this.panel.innerHTML=`<div class="hzts-finish"><div class="hzts-finish-mark">${this.score}/${this.total}</div><h1>${esc(c.sessionDone)}</h1><p>${esc(c.sessionSummary)}</p><button type="button" class="pri" id="hzts-restart">${esc(c.restart)}</button><button type="button" id="hzts-finish-back">${esc(c.back)}</button></div>`;
    const restart=()=>{this.round=0;this.score=0;this.showIntro();};
    $('#hzts-restart',this.panel).addEventListener('click',restart);$('#hzts-finish-back',this.panel).addEventListener('click',()=>window.hzBackToHub?.());
    requestAnimationFrame(()=>showPracticeCelebration({title:c.sessionDone,score:this.score,total:this.total,percent,scoreLabel:`${this.score}/${this.total}`,subtitle:c.sessionSummary,stats:[{label:isEnglish()?'Challenges':'Desafios',value:this.total},{label:isEnglish()?'Correct':'Acertos',value:this.score}],onAgain:restart,onClose:()=>window.hzBackToHub?.()}));
  }
}
