...
const DOCK_DEFAULTS={size:'auto',opacity:null};
...
function persistDockSettings(){
  ...
  try{localStorage.setItem(DOCK_KEY,JSON.stringify({...value,v:1,changedAt:Date.now()}));}catch{}
}
...
function updateDockSettings(patch,{persist=true}={}){
  ...
  // Somente duas propriedades CSS são alteradas; aplicar de imediato evita
  // a sensação de atraso sem reconstruir leitor, modal ou conteúdo do livro.
  applyDockSettings(dockSettingsCache);
...
function installReaderSetting(){
  ...
  const sync=(value=loadDockSettings())=>{
    ...
    const effective=value.opacity??responsiveDefault();const percent=Math.round(effective*100);
    if(document.activeElement!==range)range.value=String(percent);output.value=`${percent}%`;output.textContent=output.value;
  };
  toggle.addEventListener('click',()=>{const on=!readerEnabled();saveReaderEnabled(on);if(!on)enhancedReader.reset();sync();});
  block.addEventListener('click',event=>{
    const button=event.target.closest('button[data-dock-size]');if(!button||!block.contains(button))return;
    event.preventDefault();const next=updateDockSettings({size:button.dataset.dockSize});sync(next);
  });
  range.addEventListener('input',()=>{
    const opacity=Number(range.value)/100;updateDockSettings({opacity});
    output.value=`${range.value}%`;output.textContent=output.value;
  },{passive:true});
  range.addEventListener('change',()=>{flushDockSettings();sync();});
  sync();
}
