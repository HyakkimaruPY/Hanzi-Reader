...
function getManualSearchSeedData() {
  // Recognition seed data is intentionally created lazily.  Keeping these
  // arrays inside this function prevents the browser from allocating the
  // handwriting database as soon as the main site opens.
  const SEED = [
 ...
 ['爱','PDDPDZHPZD',80],['愛','PDDPDZDZDHPZD',1800],['语','DDHHSHSZH',220],['語','DDHHSHSZH',2300],['汉','DDHZD',221],['漢','DDHHSSHSZHPD',2301],['字','DDZZSH',222],['吗','SZHZZH',223],['媽','ZPHHHSHHHSZ',2302],['呢','SZHZHPPZ',224],['吗','SZHZZH',225],['哪','SZHZHHSPZ',226],['谁','DDHPSDHSHH',227],['誰','DDHPSDHSHH',2303],['请','DDHHHSHSZHH',228],['請','DDHHHSHSZHH',2304],['谢谢','DDHHSZHHSZH',229],['谢','DDHPSZHHHPHSD',230],['謝','DDHPSZHHHPHSD',2305]
  ...
  return { SEED, COMMON_EXTRA, VARIANTS };
}



// Internal state for recognition.  The database is built lazily, only after the user draws.
let db = null;
let dbMap = null;
let dbReady = false;
 ...
 * initial load, this function waits for the screen lifecycle event and
 * initialises the UI exactly once.
 */
export function initManualSearch(options = {}) {
  // Wait until the dictionary search bar exists. The dictionary page
  // is added after the initial shell, so use its lifecycle event. If the
  ...
  const tryInit = () => {
    ...
    setupUI(searchWrap, options);
  };
  const install = () => {
    if (tryInit() !== false) return true;
    return Boolean(document.getElementById('dict-hand-btn'));
  };
  const start = () => {
    if (install()) return;
    const onScreenReady = event => {
      if (event.detail?.id === 'sx' && install()) {
        document.removeEventListener('hz:screen-ready', onScreenReady);
      }
    };
    // A tela do dicionário possui um evento de ciclo próprio; observar o body
    // inteiro faria cada card e cada bloco do leitor acordar este módulo.
    document.addEventListener('hz:screen-ready', onScreenReady, { passive: true });
  };
 ...
 */
function setupUI(searchWrap, options = {}) {
  ...
  // Insert the handwriting button before the existing search button.  After
  // insertion, reposition the search button to the end of the container
  // to ensure the order becomes: input → pencil → search.  Without
  // repositioning the existing button might remain at the first
  // column due to legacy CSS rules.  Moving the element ensures it
  // flows after the pencil in the DOM order.
  searchWrap.insertBefore(handBtn, searchBtn);
  searchWrap.appendChild(searchBtn);

  // Force the search row to remain a single line.  The existing
  // dictionary CSS declares `.dict-search` as a grid with a different
  // column count in some theme patches.  That is what was pushing the
  // search button to a second line.  To make this resilient against
  // later style injections, we apply an inline flex layout with
  // `!important` and also reapply it on mutations/short delays.
  ...
  function enforceSearchLayout() {
    // DOM order: optional back button → input → handwriting → search.
    ...
    if (searchBtn && searchBtn.parentNode === searchWrap) searchWrap.appendChild(searchBtn);
  ...
  enforceSearchLayout();
  // Reaplica somente quando a estrutura da própria barra muda. A versão
  // anterior usava quatro timers de boot e repintava a barra já visível.
  let layoutFrame = 0;
  const queueSearchLayout = () => {
    if (layoutFrame) return;
    layoutFrame = requestAnimationFrame(() => { layoutFrame = 0; enforceSearchLayout(); });
  };
  new MutationObserver(queueSearchLayout).observe(searchWrap, { childList: true });
  document.addEventListener('hz:screen-ready', event => {
    if (event.detail?.id === 'sx') queueSearchLayout();
  }, { passive: true });
          ...
          <div class="hand-color">
            <button type="button" id="hand-color-toggle" title="Escolher cor" class="hand-icon-btn hand-color-toggle" aria-label="Escolher cor">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <path d="M7 3.5 16.5 13"></path>
                <path d="M5.7 8.2 13.8 16.3c.7.7 1.8.7 2.5 0l2.8-2.8c.7-.7.7-1.8 0-2.5L11 2.9"></path>
                <path d="M4 20h16"></path>
                <path d="M18.8 16.8c1.1 1.2 1.7 2.2 1.7 3 0 1-.8 1.7-1.7 1.7s-1.7-.8-1.7-1.7c0-.8.6-1.8 1.7-3Z"></path>
              </svg>
  ...
  .hand-color-toggle { position: relative; overflow: visible; border-color: var(--active-color, #ffffff) !important; }
  .hand-color-current {
    position: absolute;
    right: 3px;
    bottom: 3px;
  ...
  function resetSeed() {
    // Build the recognition database on demand.  This mirrors the standalone
    // recognizer's lightweight local index while avoiding any startup RAM cost
    // inside the full hanzi-reader app.
    db = [];
    dbMap = new Map();
    const { SEED, COMMON_EXTRA, VARIANTS } = getManualSearchSeedData();
  ...
  function ensureDb() {
    if (!dbReady || !Array.isArray(db) || !dbMap) resetSeed();
  }
  function releaseDb() {
    db = null;
    dbMap = null;
  ...
  function openModal() {
    // Reset strokes and canvas for fresh drawing
    clearCanvas(true);
  ...
  function closeModal() {
    ...
    clearCanvas(true);
    if (colorOptions) colorOptions.classList.remove('open');
    // Free recognition index after use on memory-constrained devices.
    releaseDb();
  }

  // Do not build the recognition database here.  It is created lazily on
  // the first recognition attempt, so opening the site does not allocate
  // the handwriting index or consume RAM unnecessarily.
  updateComposition();
  resizeCanvas();
