...
const copy=()=>uiLang()==='en'?{
  ...
  album:'Guzheng',artist:'Fu Na · 發燒古箏',library:'Biblioteca musical',tracks:'42 pistas',search:'Buscar música…',pick:'Elige una pista',instrument:'Cuerdas tradicionales chinas',empty:'No se encontró música.',mini:'Reproductor de música',close:'Cerrar reproductor',minimize:'Minimizar reproductor',expand:'Expandir reproductor',previous:'Anterior',next:'Siguiente',play:'Reproducir',pause:'Pausar',shuffle:'Aleatorio',repeat:'Repetir',volume:'Volumen',favorite:'Favorito'
}:{
...
const state={
  ...
  drag:null,position:null,positionDragged:false,wasPlaying:false,screenHandler:null
};
const refs={};
...
function open(){
  ...
  renderList();updateUI();
  // v5.1: nenhum input recebe foco ao abrir — o teclado virtual só aparece
  // quando o usuário toca explicitamente no campo de pesquisa. O foco
  // inicial vai para o próprio diálogo (título/container), mantendo a
  // navegação por Tab acessível no desktop.
  requestAnimationFrame(()=>{
    const dialog=refs.modal.querySelector('.hz-music-dialog')||refs.modal;
...
function removeMini(clearPosition){const mini=$('#hz-music-mini');if(mini){mini.removeEventListener('click',onMiniClick);mini.removeEventListener('pointerdown',startDrag);mini.removeEventListener('pointermove',moveDrag);mini.removeEventListener('pointerup',endDrag);mini.removeEventListener('pointercancel',endDrag);mini.remove();}uninstallResizeClamp();state.drag=null;if(clearPosition){state.position=null;state.positionDragged=false;try{localStorage.removeItem(POSITION_KEY);}catch{}}}
...
function compactViewport(){return viewport().width<=700;}
function boundsFor(mini){
  const vp=viewport(),rect=mini.getBoundingClientRect(),active=document.querySelector('.screen.active'),nav=active?.querySelector('.bnav:not(.rbnav)');
  const navRect=nav?.getBoundingClientRect(),navTop=navRect&&navRect.height>0?navRect.top:vp.top+vp.height;
  const side=Math.max(10,Math.min(18,vp.width*.035));
  return{minX:vp.left+side,maxX:Math.max(vp.left+side,vp.left+vp.width-rect.width-side),minY:vp.top+side,maxY:Math.max(vp.top+side,Math.min(navTop-side,vp.top+vp.height-side)-rect.height)};
}
function clamp(value,min,max){return Math.min(max,Math.max(min,value));}
function clampMini(mini,x,y,{save=false,dragged=state.positionDragged}={}){
  const b=boundsFor(mini),nx=clamp(x,b.minX,b.maxX),ny=clamp(y,b.minY,b.maxY);
  mini.classList.add('is-positioned');mini.style.left=`${Math.round(nx)}px`;mini.style.top=`${Math.round(ny)}px`;mini.style.right='auto';mini.style.bottom='auto';
  state.position={x:nx,y:ny,dragged:Boolean(dragged),viewportWidth:viewport().width};state.positionDragged=Boolean(dragged);
  if(save){try{localStorage.setItem(POSITION_KEY,JSON.stringify(state.position));}catch{}}
}
function defaultMiniPosition(mini){const b=boundsFor(mini),vp=viewport(),rect=mini.getBoundingClientRect();return{x:compactViewport()?vp.left+(vp.width-rect.width)/2:b.maxX,y:b.maxY};}
function restoreMiniPosition(mini){
  const saved=state.position||readJSON(POSITION_KEY,null);requestAnimationFrame(()=>{
    const valid=saved&&Number.isFinite(saved.x)&&Number.isFinite(saved.y)&&saved.dragged===true;
    if(valid){state.positionDragged=true;clampMini(mini,saved.x,saved.y,{dragged:true});}
    else{state.positionDragged=false;const pos=defaultMiniPosition(mini);clampMini(mini,pos.x,pos.y,{dragged:false});}
  });
}
function startDrag(event){
  ...
  const rect=mini.getBoundingClientRect();state.drag={pointerId:event.pointerId,dx:event.clientX-rect.left,dy:event.clientY-rect.top};state.positionDragged=true;mini.classList.add('dragging');mini.setPointerCapture?.(event.pointerId);mini.addEventListener('pointermove',moveDrag);mini.addEventListener('pointerup',endDrag,{once:true});mini.addEventListener('pointercancel',endDrag,{once:true});event.preventDefault();
}
function moveDrag(event){const mini=event.currentTarget;if(!state.drag||event.pointerId!==state.drag.pointerId)return;clampMini(mini,event.clientX-state.drag.dx,event.clientY-state.drag.dy,{dragged:true});event.preventDefault();}
function endDrag(event){const mini=event.currentTarget;mini.removeEventListener('pointermove',moveDrag);mini.classList.remove('dragging');try{mini.releasePointerCapture?.(event.pointerId);}catch{}if(state.position)clampMini(mini,state.position.x,state.position.y,{save:true,dragged:true});state.drag=null;}
function installResizeClamp(){
  if(state.resizeHandler)return;
  state.resizeHandler=()=>{if(state.resizeFrame)return;state.resizeFrame=requestAnimationFrame(()=>{state.resizeFrame=0;const mini=$('#hz-music-mini');if(!mini)return;const rect=mini.getBoundingClientRect();if(!state.positionDragged){const pos=defaultMiniPosition(mini);clampMini(mini,pos.x,pos.y,{dragged:false});}else clampMini(mini,state.position?.x??rect.left,state.position?.y??rect.top,{dragged:true});});};
  state.screenHandler=()=>state.resizeHandler?.();
  window.addEventListener('resize',state.resizeHandler,{passive:true});window.visualViewport?.addEventListener('resize',state.resizeHandler,{passive:true});document.addEventListener('hz:screen-change',state.screenHandler,{passive:true});
}
...
function boot(){
  install();
  const service={
    ensureAudio,playTrack,togglePlay,next,previous,close,
    get audio(){return state.audio;},
    getState:()=>({currentId:state.currentId,minimized:state.minimized,playing:Boolean(state.audio&&!state.audio.paused),position:state.audio?.currentTime||0,volume:state.audio?.volume??initialVolume()})
  };
  window.MusicPlaybackService=service;window.hzOpenMusic=open;
  window.hzMusicPlayer={open,minimize,minimizeIfExpanded,close,isExpanded,isPlaying:()=>Boolean(state.audio&&!state.audio.paused),getState:service.getState,playTrack};
  window.hzMusicController={stop:close,openPicker:open,isPlaying:()=>Boolean(state.audio&&!state.audio.paused)};
  if(refs.modal?.classList.contains('open'))open();
}
