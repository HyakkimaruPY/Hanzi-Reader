...
function openToneDrawIntro({ firstRun = false, onStart = null } = {}) {
  if (window.hzPracticeUI?.showHelp) {
    const steps=TX('toneSteps'),language=uiLang();
    const titles=language==='en'?['Goal','Listen to the audio','Draw the tones','Undo or clear','Scoring','Finish or leave']
      :language==='es'?['Objetivo','Escuchar el audio','Dibujar los tonos','Deshacer o limpiar','Puntuación','Concluir o salir']
      :['Objetivo','Ouvir o áudio','Desenhar os tons','Desfazer ou limpar','Pontuação','Concluir ou sair'];
    const topics=[
      {title:titles[0],detail:TX('toneIntroLead')},
      {title:titles[1],detail:steps[0]},
      {title:titles[2],detail:`${steps[1]} ${steps[2]}`},
      {title:titles[3],detail:steps[3]},
