...
// Música é inicializada sob demanda. Isso evita baixar áudio durante o boot e
// preserva a thread principal para a interface, o leitor e o dicionário.
function v43PlayCelebrationTrack(){
  ...
  audio.preload='auto';audio.src=track.url;
  v43CelebrationMode=true;v43CurrentTrackId=trackId;
  const doPlay=()=>{try{audio.currentTime=start;}catch{}audio.play().catch(()=>{});};
  ...
  v43CelebEndTimer=setTimeout(()=>{try{audio.pause();}catch{}v43CelebrationMode=false;v43CurrentTrackId=null;v43CelebEndTimer=null;},V43_SEG_LEN*1000+300);
...
let v43MusicReturnTab='sl';
function v43SetNavActive(tab){
  document.querySelectorAll('.ni[data-tab]').forEach(node=>node.classList.toggle('on',node.dataset.tab===tab));
}
function v43CurrentNavTab(){
  const id=document.querySelector('.screen.active')?.id||'sl';
  if(id==='sp')return'practice';if(id==='spf')return'profile';
  return['sl','sw','sx','ss','sd'].includes(id)?id:'sl';
}
function v43CloseMusic(){v43StopMusic();closeModals();v43SetNavActive(v43MusicReturnTab||v43CurrentNavTab());}
document.getElementById('v43-music-close')?.addEventListener('click',v43CloseMusic);
...
document.body.addEventListener('click',e=>{
  ...
  if(tab==='music'){window.hzOpenMusic?.();return;}
...
function v29NavHTML(active='sl'){
 const tabs=[['sl','Leitura',v29Svg('book')],['sw','Flash Cards',v29Svg('word')],['sx','Dicionário',v29Svg('dict')],['practice','Prática',v29Svg('practice')],['music','Música',v29Svg('music')],['profile','Meu Perfil',v29Svg('profile')]];
...
window.hzOpenMusic=function(){
  try{
    v43MusicReturnTab=v43CurrentNavTab();v43SetNavActive('music');
    showModal('mo-music');v43RenderTrackList(document.getElementById('v43-search')?.value||'');
  }catch(e){}
};
