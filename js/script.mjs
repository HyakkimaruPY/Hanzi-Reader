...
(function(){
  ...
  async function h52RenderDictWords(q,out){
    ...
    else out.innerHTML='<div class="dict-empty"><div class="spin sm"></div><small>Buscando palavras relacionadas…</small></div>';
  ...
  async function h52RenderDictSentences(q,out){
    ...
    else out.innerHTML='<div class="dict-empty"><div class="spin sm"></div><small>Buscando frases…</small></div>';
  ...
  async function h53ResolveDictionaryEntry(rawTerm){
    ...
    if(!h53HasDefs(result)){
      const packaged=(await h53FallbackEntries())[term];
      if(packaged)result={term,pinyin:packaged.pinyin||'',hsk:packaged.hsk||null,src:'Dicionário local',defs:[{pos:'tradução',defs:[...(packaged.pt||[]),...(packaged.en||[])].map(text=>({text,ex:[]}))}]};
    }
    ...
    h53DictionaryCache.set(term,{at:Date.now(),value:result});
    return result;
  }
