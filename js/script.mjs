...
function v55IsBadSentenceTranslation(value){
  const text=String(value||'').replace(/\s+/g,' ').trim();
  if(!text)return true;
  if(/(?:https?:\/\/|www\.|\[[^\]]+\]\s*\(|URL\s*Source|Source\s*:|Markdown\s*Content|Published\s*Time|Selected?\s*:|Youdao|Jukuu|dict\.youdao|jukuu\.com|r\.jina\.ai)/i.test(text))return true;
...
async function lookupTatoebaExamples(word,limit=6){
  ...
  const fetchJson=async url=>{
    ...
    try{const r=await fetch(url,{cache:'no-store',signal:ctl?.signal,headers:{Accept:'application/json'}});return r.ok?await r.json():null;}
    catch{return null;}
    finally{if(timer)clearTimeout(timer);}
  };
  const normalizePayload=json=>{
    const input=Array.isArray(json?.data)?json.data:Array.isArray(json?.results)?json.results:Array.isArray(json)?json:[];
  ...
  const addRows=rows=>{
    for(const row of rows||[]){
      ...
      seen.add(row.text);collected.push(row);
      if(collected.length>=safeLimit)break;
    }
  };
  for(const url of sources){
    const json=await fetchJson(url);
    addRows(normalizePayload(json));
    if(collected.length>=safeLimit)break;
  }

  // O Tatoeba pode responder 500 na API v1 e 302 no endpoint legado, como
  // ocorre em alguns navegadores móveis. Reutiliza então o leitor textual que
  // já funcionava nas versões antigas (Jukuu/Youdao via r.jina.ai), sem exigir
  // alteração de backend e sem bloquear a interface quando o Tatoeba cai.
  if(collected.length<safeLimit){
    const readerUrls=[
      `https://r.jina.ai/http://www.jukuu.com/search.php?q=${encodeURIComponent(word)}`,
      `https://r.jina.ai/http://dict.youdao.com/example/blng/eng/${encodeURIComponent(word)}/`
    ];
    for(const url of readerUrls){
      try{
        ...
        const timer=ctl?setTimeout(()=>ctl.abort(),8500):0;
        let text='';
        try{const r=await fetch(url,{cache:'no-store',signal:ctl?.signal});if(r.ok)text=await r.text();}
        finally{if(timer)clearTimeout(timer);}
        if(!text)continue;
        const lines=text.split(/\n+/).map(x=>String(x||'').replace(/[#*_`>]/g,'').trim()).filter(Boolean);
        const rows=[];
        for(let i=0;i<lines.length&&rows.length<safeLimit;i++){
          const match=lines[i].match(/[\u3400-\u9fff][\u3400-\u9fff，。！？、；：“”《》（）\s]{2,100}/);
          if(!match)continue;
          const zh=v40ToSimplified(match[0].replace(/\s+/g,''));
          if(!zh.includes(word)||seen.has(zh))continue;
          let tr='';
          for(let j=i+1;j<Math.min(i+7,lines.length);j++){
            const candidate=String(lines[j]||'').replace(/^[-–—•>\s]+/,'').trim();
...
async function onTap(el){
  ...
  let result=null;
  try{result=window.resolveDictionaryEntry?await window.resolveDictionaryEntry(initialWord):await lookupAll(initialWord);}catch{}
  try{const extra=await lookupAll(initialWord);if(extra){if(!result)result=extra;else{if(!result.tatoeba?.length&&extra.tatoeba?.length)result.tatoeba=extra.tatoeba;if(!result.sogou?.length&&extra.sogou?.length)result.sogou=extra.sogou;if(!result.charDefs?.length&&extra.charDefs?.length)result.charDefs=extra.charDefs;}}}catch{}
...
async function drillChar(ch,py){
  ...
  try{defDefs=window.resolveDictionaryEntry?await window.resolveDictionaryEntry(ch):await lookupAll(ch);}catch{defDefs=null;}
  try{const extra=await lookupAll(ch);if(extra&&defDefs){defDefs.tatoeba=defDefs.tatoeba?.length?defDefs.tatoeba:extra.tatoeba;defDefs.sogou=defDefs.sogou?.length?defDefs.sogou:extra.sogou;}else if(extra)defDefs=extra;}catch{}
...
function v37CaptureSelection(immediate=false){
  if(!document.getElementById('sr').classList.contains('active'))return;
  if(selCaptureScheduled&&!immediate)return;
  ...
  requestAnimationFrame(()=>{
    ...
    if(immediate)paint();else selTimer=setTimeout(paint,70);
  });
}
document.addEventListener('selectionchange',()=>v37CaptureSelection(false),{passive:true});
document.addEventListener('pointerup',()=>v37CaptureSelection(true),{passive:true});
document.addEventListener('touchend',()=>v37CaptureSelection(true),{passive:true});
document.addEventListener('keyup',event=>{if(event.key.startsWith('Arrow')||event.key==='Shift')v37CaptureSelection(true);},{passive:true});
...
function v29Boot(){try{v29InstallShell();v29InstallModals();v29Bind();document.querySelectorAll('.mo').forEach(m=>{if(!m._v29back){m.addEventListener('click',e=>{if(e.target===m)closeModals();});m._v29back=true;}});renderDiscover();renderLib();document.dispatchEvent(new CustomEvent('hz:app-shell-mounted'));}catch(error){try{console.warn('[boot] interface base parcial',error);}catch{}}finally{try{window.hzAppReady&&window.hzAppReady();}catch{}}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',v29Boot,{once:true});else v29Boot();
...
const HZ_LOOKUP_INFLIGHT=new Map();
...
function hzLookupCorePromise(word){
  const q=String(word||'').trim();
  if(!q)return Promise.resolve(null);
  const cached=hzDictCacheRead('lookupAll',q);
  if(cached)return Promise.resolve(cached);
  const active=HZ_LOOKUP_INFLIGHT.get(q);
  if(active)return active;
  const task=Promise.resolve().then(()=>HZ_LOOKUP_ALL_CORE(q)).then(res=>{
    if(res)hzDictCacheWrite('lookupAll',q,res);
    return res||null;
  }).catch(()=>null).finally(()=>{if(HZ_LOOKUP_INFLIGHT.get(q)===task)HZ_LOOKUP_INFLIGHT.delete(q);});
  HZ_LOOKUP_INFLIGHT.set(q,task);
  return task;
}
lookupAll=async function(word){
  const q=String(word||'').trim();
  ...
  const core=hzLookupCorePromise(q);
  const budget=local?450:2800;
  const res=await Promise.race([core,new Promise(resolve=>setTimeout(()=>resolve(null),budget))]);
  return res||local;
};
// Resolve a consulta até o fim, reutilizando a mesma promessa para dicionário e
// pop-up. Assim o resultado tardio pode ser injetado sem nova interação.
window.hzLookupAllComplete=async function(word){
  const q=String(word||'').trim();
  if(!q)return null;
  const cached=hzDictCacheRead('lookupAll',q);
  if(cached)return cached;
  return (await hzLookupCorePromise(q))||hzFastLocalLookup(q)||null;
};
...
document.addEventListener('hz:app-shell-mounted',v37Boot,{once:true,passive:true});
})();
...
document.addEventListener('hz:app-shell-mounted',v38Boot,{once:true,passive:true});
})();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',hr39Install,{once:true});else hr39Install();
})();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',h41Boot,{once:true});else h41Boot();
})();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',h42Patch,{once:true});else h42Patch();
})();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',H44_boot,{once:true});else H44_boot();
})();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',H45_boot,{once:true});else H45_boot();
})();
...
function H48_scheduleSelection(){clearTimeout(H48_sel.timer);H48_sel.timer=setTimeout(()=>{const text=H48_selectionText();if(H48_cjkCount(text)<1)return;H48_sel.text=text;H48_updateSelectionBar('Trecho selecionado');},90);}
...
function hzGameDoc(){try{const bin=atob(HZP_GAME_B64);const bytes=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);const doc=new TextDecoder('utf-8').decode(bytes);return typeof window.hzPrepareToneGameDoc==='function'?window.hzPrepareToneGameDoc(doc):doc;}catch(e){return'';}}
...
function hzUnmountPracticeActivity(){
  ...
  try{window.hzPracticeAudio?.cancel();}catch(e){}
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',hzBoot,{once:true});else hzBoot();
})();
...
document.addEventListener('hz:app-shell-mounted',boot,{passive:true});
})();
...
function playBlob(blob){
  if(window.hzPracticeAudio?.playBlob)return window.hzPracticeAudio.playBlob(blob,{interrupt:true,scope:'microsoft-tts'});
  return new Promise((resolve,reject)=>{
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',patchTradSimpFlow,{once:true});else patchTradSimpFlow();
})();
...
(function(){
  ...
  function simplifyReaderNow(){
    try{
      ...
      const alreadyRendered=Boolean(root.querySelector('.wunit,[data-tid],.pyrow'));
      const tipOpen=Boolean(document.getElementById('tip')?.classList.contains('open'));
      // O pop-up é apenas uma camada visual. Enquanto estiver aberto, não toca
      // no DOM do leitor — nem reconstruindo HTML, nem percorrendo text nodes.
      // Isso evita o quadro em branco observado no Chrome móvel.
      if(tipOpen)return;
      if(raw&&typeof buildHTML==='function'&&!alreadyRendered){
...
(function(){
  ...
  function h56BeginDictView(tab,query,out){
    const token=`${++h56DictEpoch}:${tab}:${query}`;
    if(out){out.dataset.h56DictToken=token;out.dataset.h56DictTab=tab;out.dataset.h56DictQuery=query;}
    return token;
  }
  function h56DictViewCurrent(out,token,tab,query){
    if(!out||!out.isConnected||out.dataset.h56DictToken!==token)return false;
    let activeTab='',activeQuery='';
    try{activeTab=v29DictTab||'';}catch{}
    try{activeQuery=h52ToSimpSync(h52Cjk(v29DictTerm)||v29DictTerm).trim();}catch{}
    return activeTab===tab&&activeQuery===query;
  }
  function h56SetActiveDictTab(tab){
    document.querySelectorAll('.dict-tab').forEach(x=>x.classList.toggle('on',x.dataset.dtab===tab));
  }
  ...
  async function h52RenderDictWords(q,out){
    ...
    if(!out)return;
    const viewToken=h56BeginDictView('words',q,out);
    if(!q){out.innerHTML='<div class="dict-empty">Pesquise uma palavra ou ideograma.</div>';return;}
    ...
    void Promise.allSettled([
      ...
      if(!h56DictViewCurrent(out,viewToken,'words',q))return;
  ...
  async function h52Tatoeba(q){
    ...
    // estável v1 diretamente em hospedagens estáticas. Não use api_v0 aqui:
  ...
  function h52PaintDictSentences(q,out,sents){
    if(!out)return;
    sents=[...(sents||[])].sort((a,b)=>v55SentenceCjkLength(a.text)-v55SentenceCjkLength(b.text)||a.text.length-b.text.length);
    if(!sents.length){out.innerHTML='<div class="dict-empty">Não encontrei frases que contenham exatamente este termo agora.</div>';return;}
    const playIcon='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 010 7.07"/></svg>';
    out.innerHTML='<div class="dict-results-lexi"><div class="dict-subtitle">Frases com “'+h52Esc(q)+'”</div>'+sents.map((s,i)=>{
      ...
      const cleanTranslations=(s.translations||[]).filter(t=>!v55IsBadSentenceTranslation(t));
      return `<div class="sent-card v37"><div class="sent-top"><div><div class="sent-zh">${zh}</div><div class="sent-py"><b>${h52Esc(py)}</b></div></div><div class="hz-sent-actions"><button class="dict-audio v34-svg-only" data-sent-idx="${i}">${playIcon}</button>${save}</div></div><div class="sent-tr">${cleanTranslations.length?h52Esc(cleanTranslations[0]):'Tradução indisponível nesta fonte.'}</div><div class="sent-src"><b>${h52Esc(s.src||'Banco de frases')}</b> • contém “${h52Esc(q)}”</div></div>`;
    }).join('')+'</div>';
  ...
  async function h52RenderDictSentences(q,out){
    ...
    void (async()=>{
      try{for(const row of await h52Tatoeba(q)||[])await push(row);}catch{}
      if(!h56DictViewCurrent(out,viewToken,'sents',q))return;
  ...
  function h53RememberTerm(term){
    try{if(window.hzStore?.rememberDictionarySearch)return window.hzStore.rememberDictionarySearch(term);const current=JSON.parse(localStorage.getItem('hzDictionaryRecent.v1')||'[]');const next=[...(Array.isArray(current)?current:[]).filter(item=>item!==term),term].slice(-30);localStorage.setItem('hzDictionaryRecent.v1',JSON.stringify(next));}catch{}
  }
  async function h53ResolveDictionaryEntry(rawTerm){
    ...
    else h53DictionaryCache.delete(term);
    return result;
  }
  window.resolveDictionaryEntry=h53ResolveDictionaryEntry;

  function h56MergeDictionaryResults(base,extra){
    if(!base&&!extra)return null;
    if(!base)return h53StandardizeDefs({...extra});
    if(!extra)return h53StandardizeDefs({...base});
    const merged={...extra,...base};
    const groups=[];const seenDefs=new Set();
    for(const source of [base,extra]){
      for(const group of source?.defs||[]){
        const defs=[];
        for(const raw of group?.defs||[]){
          const text=String(typeof raw==='string'?raw:raw?.text||'').trim();
          if(!text||seenDefs.has(text))continue;
          seenDefs.add(text);defs.push(typeof raw==='string'?{text,ex:[]}:{...raw,text});
        }
        if(defs.length)groups.push({pos:group?.pos||'',defs});
      }
    }
    merged.defs=groups;
    const mergeArray=(key,id)=>{
      const map=new Map();
      for(const row of [...(base?.[key]||[]),...(extra?.[key]||[])]){
        const k=id(row);if(k&&!map.has(k))map.set(k,row);
      }
      merged[key]=[...map.values()];
    };
    mergeArray('tatoeba',row=>row?.text||row?.zh||'');
    mergeArray('sogou',row=>row?.word||row?.k||'');
    mergeArray('charDefs',row=>row?.ch||'');
    merged.src=base.src||extra.src||'';
    merged.pinyin=base.pinyin||extra.pinyin||'';
    merged.traditional=base.traditional||extra.traditional||'';
    return h53StandardizeDefs(merged);
  }

  function h54CommitDictDefs(q,out,viewToken,result,{final=false}={}){
    if(!h56DictViewCurrent(out,viewToken,'defs',q))return false;
    const root=out.querySelector('.h54-dict-final');
    const defsSlot=root?.querySelector('.h54-defs-slot');
    if(!root||!defsSlot)return false;
    const source=root.querySelector('.lexi-source-label');
    if(source)source.textContent=result?.src||'Dicionário local';
    const pyEl=root.querySelector('.lexi-py');
    if(pyEl&&result?.pinyin)pyEl.textContent=result.pinyin;
    if(!h53HasDefs(result)&&!final)return false;
    let html='';
    if(h53HasDefs(result)){
      const hints=new Set();
      for(const group of result.defs||[])for(const def of group.defs||[])if(def.pyHint)hints.add(def.pyHint);
      let index=0;
      for(const group of (result.defs||[]).slice(0,8)){
        const rows=(group.defs||[]).slice(0,8);if(!rows.length)continue;
        html+='<div class="lexi-entry">';
        if(group.pos)html+=`<div class="lexi-pos">${h52Esc(group.pos)}</div>`;
        for(const def of rows){
          index++;
          const reading=hints.size>1&&def.pyHint?` <span class="lexi-def-reading">— ${h52Esc(q)}: ${h52Esc(def.pyHint)}</span>`:'';
          html+=`<div class="lexi-def"><div class="lexi-def-label">Definição ${index}${reading}</div>${typeof v39TransButton==='function'?v39TransButton(def.text):h52Esc(def.text)}</div>`;
        }
        html+='</div>';
      }
    }else{
      html='<div class="lexi-entry"><div class="dict-empty">Não encontrei uma definição para este termo nas fontes disponíveis.</div></div>';
    }
    try{
      const grammar=v42FindGrammar(q);
      if(grammar?.length)html+=grammar.map(g=>`<div class="lexi-entry"><div class="lexi-section-title">Gramática — ${h52Esc(g.title)}</div><div class="lexi-grammar-pattern">${h52Esc(g.pattern)}</div><div class="lexi-def" style="border-bottom:0">${h52Esc(g.explanation)}</div></div>`).join('');
    }catch{}
    if(result?.charDefs?.length){
      html+='<div class="lexi-entry"><div class="lexi-section-title">Ideograma por ideograma</div>'+result.charDefs.map(c=>`<div class="lexi-def"><b>${h52Esc(c.ch)}</b> — ${c.text?(typeof v39TransButton==='function'?v39TransButton(c.text):h52Esc(c.text)):'—'}</div>`).join('')+'</div>';
    }
    const related=(result?.sogou||[]).map(item=>item.word).filter(Boolean).slice(0,10);
    if(related.length){
      html+=`<div class="lexi-entry"><div class="lexi-section-title">Termos relacionados</div><div class="lexi-acc-hint">Toque numa palavra para ver a definição completa</div>${related.map((word,i)=>`<div class="lexi-acc-row" data-acc-idx="${i}"><div class="lexi-acc-row-label">${typeof v40WordDisplayHtml==='function'?v40WordDisplayHtml(word):h52Esc(word)}</div>${typeof v39SaveButtonHtml==='function'?v39SaveButtonHtml(word):''}<svg class="lexi-acc-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></div><div class="lexi-acc-panel" id="acc-panel-${i}"></div>`).join('')}</div>`;
    }
  ...
  async function h54RenderDictDefs(rawQuery,out){
    ...
    const viewToken=h56BeginDictView('defs',q,out);
    if(!q){out.innerHTML=H52_EMPTY_DICT;return;}
    await h52EnsureTradSimp();
    if(!h56DictViewCurrent(out,viewToken,'defs',q))return;
    let py=(typeof getWordPY==='function'?getWordPY(q):'')||'';
    if(!py&&typeof waitPinyin==='function'){
      void Promise.resolve().then(()=>waitPinyin()).then(()=>{
        if(!h56DictViewCurrent(out,viewToken,'defs',q))return;
        const readyPy=(typeof getWordPY==='function'?getWordPY(q):'')||'';
        const el=out.querySelector('.h54-dict-final .lexi-py');
        if(el&&readyPy)el.textContent=readyPy;
      }).catch(()=>{});
    }
    const playIcon='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 010 7.07"/></svg>';
    ...
    if([...q].filter(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}}).length===1){
      void (async()=>{
        ...
        if(!h56DictViewCurrent(out,viewToken,'defs',q))return;
        const currentRoot=out.querySelector('.h54-dict-final');
        const slot=currentRoot?.querySelector('.h54-stroke-slot');if(!slot)return;
        if(typeof window.hzMountStrokePanel==='function'){
          window.hzMountStrokePanel({slot,stepsHost:currentRoot.querySelector('.h54-steps-slot'),stroke,char:q});
        }else if(stroke?.gif){
          slot.innerHTML=`<button class="lexi-stroke-card h54-stroke-button" type="button"><img src="${h52Esc(stroke.gif)}" alt="Ordem dos traços"></button>`;
          slot.querySelector('button').onclick=()=>{try{v41OpenGifModal(stroke.gif,q);}catch{}};
        }
      })();
    }
    ...
    let aggregate=null;
    try{
      const quick=hzFastLocalLookup(q);
      if(h53HasDefs(quick)){aggregate=h56MergeDictionaryResults(aggregate,quick);h54CommitDictDefs(q,out,viewToken,aggregate);}
    }catch{}

    const jobs=[];
    jobs.push(Promise.resolve().then(()=>h53ResolveDictionaryEntry(q)));
    if(typeof window.hzLookupAllComplete==='function')jobs.push(Promise.resolve().then(()=>window.hzLookupAllComplete(q)));
    let pending=jobs.length;
    const settle=async promise=>{
      let result=null;try{result=await promise;}catch{}
      if(result){
        aggregate=h56MergeDictionaryResults(aggregate,result);
        if(h53HasDefs(aggregate)){
          h53DictionaryCache.set(q,{at:Date.now(),value:aggregate});
          try{window.hzStore?.dictPut?.(q,aggregate);}catch{}
          h54CommitDictDefs(q,out,viewToken,aggregate);
        }
      }
      pending--;
      if(pending===0&&!h53HasDefs(aggregate))h54CommitDictDefs(q,out,viewToken,aggregate,{final:true});
    };
    jobs.forEach(job=>{void settle(job);});
  }
  h54RenderDictDefs.__h52Final=true;h54RenderDictDefs.__tradSimpV2=true;

  function h56QuickDictionaryResult(term){
    try{
      const entry=v34EntryLocal(term);
      const result=entry?h53EntriesToResult(term,[{...entry,src:'Dicionário local'}]):null;
      if(h53HasDefs(result))return result;
    }catch{}
    try{const result=hzFastLocalLookup(term);if(h53HasDefs(result))return result;}catch{}
    return null;
  }

  function h56InstallFastReaderPopup(){
    if(typeof onTap!=='function'||onTap.__h56Fast)return;
    onTap=async function(el){
      try{const activeSel=(window.getSelection()?.toString()||'').trim();if(activeSel)return;}catch{}
      const tid=parseInt(el?.dataset?.tid||'-1',10);
      const tok=readerTokens?.[tid];
      if(!tok)return;
      const epoch=++h56TipEpoch;
      const initialWord=h52ToSimpSync(tok.word||tok.char||'').trim();
      if(!initialWord)return;
      const current=()=>epoch===h56TipEpoch&&Boolean(document.getElementById('tip')?.classList.contains('open'));
      setTipWord(initialWord,tok.naturalPy||tok.py||getWordPY(initialWord),tok.toneInfo);
      tipLoading();showTip(el);defDefs=null;
      let aggregate=null,termBoxStarted=false;
      const commit=result=>{
        if(!current()||!result)return;
        aggregate=h56MergeDictionaryResults(aggregate,result);
        if(!aggregate)return;
        defDefs=aggregate;
        renderTipDefs(aggregate);
        requestAnimationFrame(()=>{if(current())positionTip(el);});
        if(!termBoxStarted&&typeof hr39RenderTermBox==='function'){
          termBoxStarted=true;
          setTimeout(()=>{if(current())hr39RenderTermBox(tid);},80);
        }
      };

      const quick=h56QuickDictionaryResult(initialWord);
      if(quick)commit(quick);

      const jobs=[Promise.resolve().then(()=>h53ResolveDictionaryEntry(initialWord))];
      if(typeof window.hzLookupAllComplete==='function')jobs.push(Promise.resolve().then(()=>window.hzLookupAllComplete(initialWord)));
      const settled=jobs.map(job=>Promise.resolve(job).then(result=>{if(result)commit(result);return result;}).catch(()=>null));
      await Promise.allSettled(settled);
      if(!current())return;

      if(!h53HasDefs(aggregate)){
        let nearby=null;
        try{nearby=await tryNearby(tid);}catch{}
        if(!current())return;
        if(nearby?.word&&nearby.word.length>1){
          const word=h52ToSimpSync(nearby.word);
          setTipWord(word,nearby.py||getWordPY(word),applyToneSandhi(word,nearby.py||getWordPY(word)));
          commit({defs:nearby.defs,src:nearby.src||'CC-CEDICT',pinyin:nearby.py||getWordPY(word)});
        }
      }
      if(!h53HasDefs(aggregate)&&initialWord.length===1){
        let charResult=null;
        try{charResult=await window.hzLookupAllComplete?.(tok.char||initialWord);}catch{}
        if(current()&&charResult)commit(charResult);
      }
      if(current()&&!h53HasDefs(aggregate)){defDefs=aggregate;renderTipDefs(aggregate);requestAnimationFrame(()=>positionTip(el));}
    };
    onTap.__h56Fast=true;
    onTap._hr39=true;
    window.onTap=onTap;

    if(typeof hideTip==='function'&&!hideTip.__h56Fast){
      const oldHide=hideTip;
      hideTip=function(){h56TipEpoch++;return oldHide.apply(this,arguments);};
      hideTip.__h56Fast=true;
      window.hideTip=hideTip;
    }
  }

  function h56SanitizeDictionarySentences(result){
    if(!result)return result;
    const clean={...result};
    clean.tatoeba=(result.tatoeba||[]).map(row=>{
      const translations=(Array.isArray(row?.translations)?row.translations:[row?.translations]).flat(Infinity).map(value=>typeof value==='string'?value:String(value?.text||'')).map(value=>value.trim()).filter(value=>!v55IsBadSentenceTranslation(value));
  ...
  function h52Patch(){
    ...
    try{
      if(typeof renderTipDefs==='function'&&!renderTipDefs.__h52Final){
        ...
        renderTipDefs=function(result){const clean=h56SanitizeDictionarySentences(result);const r=old.call(this,clean);h52PostProcessTipSentences();return r;};
