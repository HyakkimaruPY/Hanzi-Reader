...
async function lookupTatoebaExamples(word,limit=6){
  ...
  const fetchJson=async url=>{
    ...
    const timer=ctl?setTimeout(()=>ctl.abort(),7500):0;
    try{const r=await fetch(url,{cache:'no-store',signal:ctl?.signal});return r.ok?await r.json():null;}
    catch{return null;}
    finally{if(timer)clearTimeout(timer);}
  };
  // Em Vercel a ponte same-origin evita CORS e preserva a rota estável da API.
  // Hospedagens estáticas continuam com o fallback direto para a mesma API v1.
  let json=await fetchJson(`/api/tatoeba?q=${encodeURIComponent(word)}&limit=${safeLimit}`);
  if(!json){
    const p=new URLSearchParams();
    p.set('lang','cmn');p.set('q',word);p.set('showtrans','all');p.set('is_unapproved','no');p.set('sort','relevance');p.set('limit',String(safeLimit));
    json=await fetchJson(`https://api.tatoeba.org/v1/sentences?${p.toString()}`);
  }
  return rows(json).map(row=>{
    const translations=flat(row.translations).filter(t=>t&&t.text&&!t.is_unapproved)
      .sort((a,b)=>translationRank(a)-translationRank(b)).map(t=>t.text).filter(Boolean);
    return{text:v40ToSimplified(row.text||''),translations:[...new Set(translations)].slice(0,2),id:row.id||null};
  }).filter(e=>e.text&&v41SentenceLooksReadable(e.text)).slice(0,safeLimit);
}
