...
function v55IsBadSentenceTranslation(value){
  ...
  if(!text)return true;
  if(/(?:https?:\/\/|www\.|\[[^\]]+\]\s*\(|URL\s*Source|Source\s*:|Youdao|Jukuu|dict\.youdao|jukuu\.com|r\.jina\.ai)/i.test(text))return true;
...
(function(){
  ...
  function simplifyReaderNow(){
    try{
      ...
      if(raw&&typeof buildHTML==='function'&&!alreadyRendered&&!tipOpen){
...
(function(){
  ...
  async function h54RenderDictDefs(rawQuery,out){
    ...
    // O módulo de pinyin é carregado de forma assíncrona. Em celulares rápidos,
    // o cartão podia ser montado antes de a biblioteca ficar pronta e permanecia
    // vazio. Atualiza somente a linha de pinyin assim que a biblioteca assentar.
