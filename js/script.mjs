...
let v43Audio=null,v43CurrentTrackId=null,v43IsShuffled=false,v43IsRepeating=false,v43CelebrationMode=false,v43CelebEndTimer=null,v43CelebSegEnd=0,v43DuckTimer=null,v43VolumeBeforeDuck=1,v43Ducked=false;
...
function v43StopMusic(){
  if(v43DuckTimer){clearInterval(v43DuckTimer);v43DuckTimer=null;}
  v43Ducked=false;
  if(v43Audio){try{v43Audio.pause();v43Audio.volume=Math.max(0,Math.min(1,v43VolumeBeforeDuck||1));}catch{}}
...
function v43SetDucked(on){
  const audio=v43Audio;if(!audio)return;
  on=!!on;
  if(on&&!v43Ducked)v43VolumeBeforeDuck=Math.max(.05,Math.min(1,Number(audio.volume)||1));
  v43Ducked=on;
  const target=on?Math.min(.14,v43VolumeBeforeDuck):v43VolumeBeforeDuck;
  if(v43DuckTimer)clearInterval(v43DuckTimer);
  v43DuckTimer=setInterval(()=>{
    const delta=target-audio.volume;
    if(Math.abs(delta)<.015){audio.volume=target;clearInterval(v43DuckTimer);v43DuckTimer=null;return;}
    audio.volume=Math.max(0,Math.min(1,audio.volume+delta*.28));
  },35);
}
...
window.hzMusicController={
  start(){try{const audio=v43GetAudioEl();if(v43CurrentTrackId==null)v43PlayFullTrack(0);else audio.play().catch(()=>{});return true;}catch{return false;}},
  stop(){try{v43StopMusic();}catch{}},
  setDucked(on){try{v43SetDucked(on);}catch{}},
  openPicker(){try{window.hzOpenMusic();}catch{}},
  isPlaying(){try{return !!(v43Audio&&!v43Audio.paused);}catch{return false;}}
};
...
function hzRewriteHelp(){
  ...
  const docs=pt?{
   ...
   pratica:'<h2>Prática e Áudio</h2><p><b>Prática:</b> atividades organizadas por habilidade, incluindo tons, sequência tonal e escrita de Hanzi. Cada desafio usa recuperação ativa e registra o progresso local.</p><p>A trilha de guzheng é uma ambientação opcional e fica desligada por padrão nos exercícios auditivos. A voz de leitura pode ser ajustada em Ajustes → Voz.</p>'
  }:{
   ...
   pratica:'<h2>Practice & Audio</h2><p><b>Practice:</b> skill-based activities including tones, tone sequence and Hanzi writing. Challenges use active recall and save progress locally.</p><p>Guzheng ambience is optional and stays off by default in listening exercises. Tune the reading voice in Settings → Voice.</p>'
  };
...
(function(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',h52Boot);else h52Boot();
  setTimeout(()=>h52Patch(),1200);
})();
