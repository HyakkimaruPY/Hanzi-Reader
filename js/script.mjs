...
function v43StopMusic(){
  ...
  v43CelebrationMode=false;v43CurrentTrackId=null;
  document.getElementById('v43-mini-player')?.remove();
}
function v43OnTrackEnded(){if(v43CelebrationMode){v43CelebrationMode=false;v43CurrentTrackId=null;}}
function v43OnTimeUpdate(){
  if(!v43CelebrationMode||!v43Audio)return;
  const fill=document.getElementById('v43-mini-fill'),timeEl=document.getElementById('v43-mini-time');
...
// A celebração reutiliza apenas um trecho curto. O player completo vive em
// js/music-player.mjs e não mantém renderizadores legados conectados ao DOM.
function v43PlayCelebrationTrack(){
  const audio=v43GetAudioEl();
  const trackId=Math.floor(Math.random()*V43_TRACKS.length),track=V43_TRACKS[trackId],start=v43PickSegmentStart(trackId);
  audio.preload='auto';audio.src=track.url;v43CelebrationMode=true;v43CurrentTrackId=trackId;
  const doPlay=()=>{try{audio.currentTime=start;}catch{}audio.play().catch(()=>{});};
  if(audio.readyState>=1)doPlay();else{const onMeta=()=>{doPlay();audio.removeEventListener('loadedmetadata',onMeta);};audio.addEventListener('loadedmetadata',onMeta);}
...
function showScreen(id){
  ...
  if(active.length===1&&active[0]===next)return;
  active.forEach(screen=>{if(screen!==next)screen.classList.remove('active');});
  next.classList.add('active');
  document.documentElement.dataset.activeScreen=id;
  document.dispatchEvent(new CustomEvent('hz:screen-change',{bubbles:true,detail:{id,previous:active[0]?.id||''}}));
}
