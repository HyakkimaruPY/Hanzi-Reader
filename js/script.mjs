    ...
    if([...q].filter(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}}).length===1){
      void (async()=>{
        ...
        if(stroke?.gif){
          ...
          if(toggle&&stepsSlot){toggle.onclick=async()=>{
            const open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));
            if(open){stepsSlot.classList.remove('open');stepsSlot.innerHTML='';toggle.textContent=stepsLabel;return;}
            toggle.textContent=(window.hzT?.('loading')||'Carregando…');
            const imgs=await v41SliceStrokeGuide(stroke.strokeDiagram,parseInt(stroke.strokeCount,10));
            if(!out.isConnected||out.dataset.h54DefsRequest!==requestId)return;
            toggle.textContent=(window.hzT?.('hideSteps')||'Recolher');
            stepsSlot.className='h54-steps-slot lexi-steps-block open';
            stepsSlot.innerHTML=imgs.length?`<div class="lexi-steps-grid">${imgs.map((src,i)=>`<button class="lexi-step-card" type="button" data-i="${i}"><img src="${src}" alt="${h52Esc(stepsLabel)} ${i+1}"><span>${i+1}</span></button>`).join('')}</div>`:`<div class="dict-empty">${h52Esc(window.hzT?.('strokeStepsUnavailable')||'Passos indisponíveis para este ideograma.')}</div>`;
            stepsSlot.querySelectorAll('.lexi-step-card').forEach(btn=>btn.onclick=()=>{try{v41OpenStepModal(imgs,Number(btn.dataset.i)||0);}catch{}});
          };}
        }
      })();
    }
