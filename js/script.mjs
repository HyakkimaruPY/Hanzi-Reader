...
(function(){
  ...
  async function h52RenderDictWords(q,out){
    ...
    const words=new Set();
    try{(v34CandidateWords(q)||[]).forEach(w=>{w=h52ToSimpSync(w);if(w&&w!==q&&h52Contains(w,q))words.add(w);});}catch{}
    try{[...HSK_LEVEL.keys(),...SEG_WORDS].forEach(w=>{w=h52ToSimpSync(w);if(w&&w!==q&&w.length>1&&qChars.some(ch=>w.includes(ch)))words.add(w);});}catch{}
    try{(await lookupSogouSuggestions(q)||[]).forEach(s=>{const w=h52ToSimpSync(s.word||s.k||'');if(w&&w!==q&&h52Contains(w,q))words.add(w);});}catch{}
    try{(await v34CedictEntries(q)||[]).forEach(e=>{const w=h52ToSimpSync(e.simp||e.word||'');if(w&&w!==q&&h52Contains(w,q))words.add(w);});}catch{}
    const combined=[...words].slice(0,70);
    if(!combined.length){out.innerHTML='<div class="dict-empty">Não encontrei palavras relacionadas a este termo.</div>';return;}
    const playIcon='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 010 7.07"/></svg>';
  ...
  async function h52Tatoeba(q){
    ...
    if(!n)return found;
    try{
      const url='https://tatoeba.org/en/api_v0/search?from=cmn&query='+encodeURIComponent(n)+'&trans_to=eng&sort=relevance&orphans=no&word_count_max=32';
      const r=await fetch(url,{signal:AbortSignal.timeout?AbortSignal.timeout(7500):undefined});
      if(r.ok){
        const d=await r.json();
        for(const x of d.results||[]){
          const zh=await h52SimplifySentenceAsync(x.text||'');
          if(!h52Contains(zh,n))continue;
          let en='';
          try{const flat=(x.translations||[]).flat(Infinity);en=(flat.find(t=>t&&t.lang==='eng')||flat.find(t=>t&&t.text)||{}).text||'';}catch{}
          found.push({text:zh,translations:en?[en]:[],src:'Tatoeba'});
        }
      }
    }catch{}
    return found;
  ...
  async function h52RenderDictSentences(q,out){
    ...
    if(!out)return;
    if(!q){out.innerHTML='<div class="dict-empty">Pesquise uma palavra ou ideograma.</div>';return;}
    out.innerHTML='<div class="dict-empty"><div class="spin sm"></div><small>Convertendo e buscando frases…</small></div>';
    const map=new Map();
    const push=async s=>{
    ...
    try{(await h52Tatoeba(q)).forEach(s=>{if(s.text&&!map.has(s.text))map.set(s.text,s);});}catch{}
    try{if(typeof hr39SentenceSearch==='function'){for(const s of await hr39SentenceSearch(q)||[])await push({text:s.zh||s.text,translations:s.en?[s.en]:s.translations,src:s.src||'externa'});}}catch{}
    const sents=[...map.values()].slice(0,22);
    if(!sents.length){out.innerHTML='<div class="dict-empty">Não encontrei frases que contenham exatamente este termo agora.</div>';return;}
    const playIcon='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 010 7.07"/></svg>';
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{h52Patch();h52EnsureTradSimp().then(()=>{h52Patch();h52MigrateStoredLibrary();});});
  else{h52Patch();h52EnsureTradSimp().then(()=>{h52Patch();h52MigrateStoredLibrary();});}
  [500,1400,3200,7000].forEach(ms=>setTimeout(()=>{h52Patch();h52EnsureTradSimp().then(()=>h52MigrateStoredLibrary());},ms));
})();
