...
function hzYieldMainThread(timeout=180){
  return new Promise(resolve=>{
    if(typeof requestIdleCallback==='function')requestIdleCallback(()=>resolve(),{timeout});
    else setTimeout(resolve,0);
  });
}
try{window.hzYieldMainThread=hzYieldMainThread;}catch{}

// Executa inicializadores uma única vez quando a árvore está pronta. As versões
// anteriores acumulavam timers de 0,1 a 5,2 s que repintavam telas já visíveis,
// substituíam renderizadores durante a navegação e provocavam cards tardios.
function hzAfterDomReady(task){
  const run=()=>queueMicrotask(()=>{try{task();}catch(e){try{console.warn('[bootstrap]',e);}catch{}}});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});
  else run();
}
try{window.hzAfterDomReady=hzAfterDomReady;}catch{}
...
const HZ_LEGACY_MEMORY_DB={books:new Map(),words:new Map()};
function hzLegacyStore(st){return HZ_LEGACY_MEMORY_DB[st]||(HZ_LEGACY_MEMORY_DB[st]=new Map());}
function initDB(){return new Promise(res=>{
  let settled=false,timer=0;
  const finish=(nextDb,fallback)=>{
    if(settled){try{nextDb?.close?.();}catch{}return;}
    settled=true;clearTimeout(timer);db=nextDb||null;window.__hzLegacyDbFallback=Boolean(fallback);res();
  };
  if(!('indexedDB' in globalThis)){finish(null,true);return;}
  let r;try{r=indexedDB.open(DB,DBV);}catch(e){finish(null,true);return;}
  timer=setTimeout(()=>finish(null,true),2500);
  ...
  r.onsuccess=e=>finish(e.target.result,false);
  r.onerror=()=>finish(null,true);
  r.onblocked=()=>finish(null,true);
});}
const dbtx=(st,m,fn)=>new Promise((res,rej)=>{
  if(!db){res(undefined);return;}
  try{const t=db.transaction(st,m),store=t.objectStore(st),r=fn(store);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error);}catch(e){rej(e);}
});
const dbAll=st=>db?dbtx(st,'readonly',s=>s.getAll()):Promise.resolve([...hzLegacyStore(st).values()]);
const dbPut=(st,v)=>{if(!db){if(v&&v.id!=null)hzLegacyStore(st).set(v.id,typeof structuredClone==='function'?structuredClone(v):v);return Promise.resolve(v&&v.id);}return dbtx(st,'readwrite',s=>s.put(v));};
const dbDel=(st,id)=>{if(!db){hzLegacyStore(st).delete(id);return Promise.resolve();}return dbtx(st,'readwrite',s=>s.delete(id));};
const dbClr=st=>{if(!db){hzLegacyStore(st).clear();return Promise.resolve();}return dbtx(st,'readwrite',s=>s.clear());};
...
let v37PinyinLayoutGeneration=0;
function v37FixPinyinOverlap(scope){
  const root=scope||document.getElementById('rtext');
  if(!root||!root.isConnected)return;
  const generation=++v37PinyinLayoutGeneration;
  ...
  // Textos grandes usam uma escala global conservadora. A versão anterior lia
  // getBoundingClientRect() milhares de vezes em vários frames, provocando
  // layout forçado concorrente com rolagem e seleção.
  if(chars.length>1400){
    root.style.setProperty('--reader-py-scale','.88');
    for(const c of chars)c.style.removeProperty('--pyscale');
    return;
  }
  root.style.removeProperty('--reader-py-scale');
  const canvas=v37FixPinyinOverlap.canvas||(v37FixPinyinOverlap.canvas=document.createElement('canvas'));
  const ctx=canvas.getContext('2d');
  if(!ctx)return;
  const style=getComputedStyle(document.documentElement);
  ctx.font=`600 ${curFs*.52}px ${style.getPropertyValue('--pyf')||'sans-serif'}`;
  const CHUNK=180;
  let i=0;
  const run=()=>{
    if(generation!==v37PinyinLayoutGeneration||!root.isConnected)return;
    // Não disputa a thread principal durante gesto de rolagem ou seleção ativa.
    const selection=window.getSelection?.();
    if(document.documentElement.classList.contains('hz-reader-scrolling')||(selection&&!selection.isCollapsed&&root.contains(selection.anchorNode))){
      setTimeout(run,90);return;
    }
    const end=Math.min(i+CHUNK,chars.length);
    for(;i<end;i++){
      const el=chars[i],py=el.dataset.py||'';
      const width=py?ctx.measureText(py).width:0;
      const scale=width>curFs*.94?Math.max(.72,(curFs*.94)/width):1;
      if(scale<.999)el.style.setProperty('--pyscale',scale.toFixed(3));
      else el.style.removeProperty('--pyscale');
    }
    if(i<chars.length)requestAnimationFrame(run);
  };
  requestAnimationFrame(run);
}
function applyPinyin(scope){
  const root=scope||document.getElementById('rtext')||document;
  root.querySelectorAll('ruby rt').forEach(rt=>{rt.style.visibility=showPinyin?'':'hidden';rt.style.height=showPinyin?'auto':'0';rt.style.marginBottom=showPinyin?'6px':'0';});
  root.querySelectorAll('.wunit[data-tid]').forEach(el=>{const tok=readerTokens[parseInt(el.dataset.tid)];const sh=wordShouldShowPinyin(tok);el.classList.toggle('pyhide',!sh);el.classList.toggle('pytarget',showPinyin&&pinyinLevelMode&&sh);});
  if(!scope)syncSettingControls();
  requestAnimationFrame(()=>v37FixPinyinOverlap(root));
}
...
function overlayDictSegments(run,base){
  // Todos os índices expostos pelo leitor são índices de pontos de código.
  // A versão antiga misturava esses índices com offsets UTF-16 de startsWith(),
  // deslocando a seleção depois de caracteres CJK fora do BMP (Ext. B+).
  const chars=Array.from(run||'');
  const unitOffsets=new Array(chars.length+1);let unitPos=0;
  for(let cp=0;cp<chars.length;cp++){unitOffsets[cp]=unitPos;unitPos+=chars[cp].length;}
  unitOffsets[chars.length]=unitPos;
  const startMap=new Map();let pos=0;
  for(const w of base||[]){startMap.set(pos,w);pos += Array.from(w).length;}
  const out=[];let i=0;
  while(i<chars.length){
    const hit=bestDictAt(run,unitOffsets[i]);
    const b=startMap.get(i);
    if(hit&&(!b||Array.from(hit).length>Array.from(b).length)){out.push(hit);i += Array.from(hit).length;continue;}
    if(b){out.push(b);i += Array.from(b).length;continue;}
    if(hit){out.push(hit);i += Array.from(hit).length;continue;}
    out.push(chars[i]);i++;
  }
  return out;
}
...
function buildHTML(text,options={}){
  text=v40NormalizeText(String(text||''));
  const append=Boolean(options&&options.append);
  if(!append){
    const fullText=options&&options.fullText!=null
      ?v40NormalizeText(String(options.fullText))
      :text;
    window.__rtextRaw=fullText;
    readerTokens=[];readerCharRefs=[];
  }
  const paragraphs=text.split('\n');
  let ci=Number(options&&options.startOffset)||0;
  const paraHtml=[];
...
function v37RawSelectionSlice(sel){
  try{
    ...
    if(b<=a)return'';
    // data-ci é contado por pontos de código; Array.from mantém o mesmo
    // sistema de índices mesmo quando o texto contém emoji ou caracteres fora do BMP.
    return Array.from(raw).slice(a,b).join('');
  }catch(e){return'';}
}
...
document.addEventListener('selectionchange',()=>{
  ...
  const run=()=>{
    ...
    // O reconhecimento visual acompanha a seleção no mesmo ciclo lógico. A
    // espera por frame + timer da versão anterior deixava a interface atrás do
    // seletor nativo, sobretudo quando outro trabalho de layout ocupava a fila.
    const dock=document.getElementById('mini-dock-flag');
    dock?.classList.toggle('show',ok);
...
document.getElementById('bback').addEventListener('click',()=>{document.getElementById('mini-dock-flag')?.classList.remove('show');showScreen('sl');renderLib();});
...
document.body.addEventListener('click',e=>{
  ...
  showScreen(tab);
  // Os caches já são carregados no bootstrap e atualizados nas operações de
  // escrita. Reabrir IndexedDB em toda troca de aba atrasava os cards.
  if(tab==='sw'&&!document.getElementById('wc')?.querySelector('.wcard'))renderWords();
  if(tab==='sl'&&!document.getElementById('bc')?.children.length)renderLib();
});
...
(async()=>{
  ...
  try{
    await initDB();
    document.dispatchEvent(new CustomEvent('hz:legacy-db-ready',{detail:{fallback:Boolean(window.__hzLegacyDbFallback)}}));
    // Biblioteca e Flash Cards são montados ainda sob o overlay inicial. Isso
    // evita liberar a tela vazia e injetar cards alguns frames depois.
    const sourceReady=window.hsrcReadyPromise||Promise.resolve();
    await Promise.all([loadLib(),loadWords(),sourceReady]);
  }catch(error){
    try{console.warn('[bootstrap] armazenamento em fallback:',error);}catch{}
    try{renderLib();renderWords();}catch{}
  }finally{
    try{window.hzAppReady&&window.hzAppReady();}catch{}
  }
})();
...
function v29Svg(name){
 const icons={
  ...
  word:'<rect x="4" y="6" width="14" height="11" rx="2"/><path d="M7 6V4.8A1.8 1.8 0 018.8 3h9.4A1.8 1.8 0 0120 4.8v8.4a1.8 1.8 0 01-1.8 1.8H18"/><path d="M8 10h6M8 13h4"/>',
...
function v29Boot(){v29InstallShell();v29InstallModals();v29Bind();document.querySelectorAll('.mo').forEach(m=>{if(!m._v29back){m.addEventListener('click',e=>{if(e.target===m)closeModals();});m._v29back=true;}});window.__hzShellReady=true;}
hzAfterDomReady(v29Boot);
...
function v32Boot(){v32ApplyHsk30();v32UiCleanup();v32InstallReaderControls();}
hzAfterDomReady(v32Boot);
})();
...
if(typeof showScreen==='function'){const oldShowScreen=showScreen;showScreen=function(id){const r=oldShowScreen(id);v34PatchDictSearch();v34InstallFullscreen();return r;};}
...
function v34Boot(){try{const about=document.querySelector('#ss .ssub[style*="color:#8a8a8a"]');if(about)about.textContent=V34_VERSION;}catch{}v34PatchDictSearch();v34InstallFullscreen();v34InstallHelp();v34RemoveEmojiSymbols();}
hzAfterDomReady(v34Boot);
})();
...
hzAfterDomReady(hr35MarkSettings);
document.addEventListener('hz:screen-ready',event=>{if(event.detail?.id==='ss')hr35MarkSettings();},{passive:true});
...
// boot
hzAfterDomReady(()=>{try{h36EnsureDeck();h36InstallFlashHead();h36MigrateUnassignedCards().then(()=>loadWords());h36InstallNext();const about=document.querySelector('#ss .ssub[style*="color:#8a8a8a"]');if(about)about.textContent='v3.8';}catch(e){console.warn('v3.8 boot',e);}});
})();
...
function v37Boot(){v37InstallCss();v37InstallModeButtons();const about=document.querySelector('#ss .ssub[style*="color:#8a8a8a"]');if(about)about.textContent=V37_VERSION;}
hzAfterDomReady(v37Boot);
})();
...
async function v38MigrateStored(){
 try{if(!db){document.addEventListener('hz:legacy-db-ready',v38MigrateStored,{once:true});return;}if(localStorage.getItem('hmig38')==='1')return;let changed=false,processed=0;const list=await dbAll(STB);
   for(const b of list){if((h36BookKind(b)==='book')&&h36Chapters(b).length){let combined=v38CleanRaw(h36Chapters(b).map(c=>c.content||'').join('\n\n'),b.source||'');if(!combined)continue;const pages=v38SplitText(combined,b.title||'Página');if(pages.length<=1||v38CjkCount(combined)<=v38PageChars()*1.3){b.kind='simple';b.content=combined;b.type='source';delete b.chapters;b.progress=0;changed=true;await dbPut(STB,b);}else{b.chapters=pages.map((p,i)=>({id:(h36Chapters(b)[i]&&h36Chapters(b)[i].id)||v29NewId(),num:i+1,title:p.title,content:p.content,progress:0,addedAt:Date.now(),sourceUrl:b.source||''}));b.lastChapterIndex=Math.min(b.lastChapterIndex||0,b.chapters.length-1);changed=true;await dbPut(STB,b);}}
     ...
     if(++processed%4===0)await hzYieldMainThread();
   }
   localStorage.setItem('hmig38','1');if(changed){books=await dbAll(STB);}}
...
function v38Boot(){v38Style();v38ExpandSeg();v38InstallSources();v38PatchGlobals();v38InstallBackGuard();const about=document.querySelector('#ss .ssub[style*="color:#8a8a8a"]');if(about)about.textContent=V38_VERSION;v38MigrateStored();}
hzAfterDomReady(v38Boot);
})();
...
.reader-fullscreen #mini-dock-flag.hz-selecting #sel-translate,.reader-fullscreen #mini-dock-flag.hz-selecting #sel-read{display:inline-flex!important}
...
function H46_boot(){H46_patchGlobals();}
...
function H48_patchGlobals(){
 ...
 H48_updateMainButton();try{if(document.getElementById('sd')?.classList.contains('active'))(window.hsrcRenderDiscoverLocal||renderDiscover)();}catch{}
...
function H48_watchReader(){
  if(H48_watchReader.installed)return;H48_watchReader.installed=true;
  // O áudio é invalidado por uma mudança semântica do conteúdo, não por cada
  // nó anexado. Observar o subtree fazia cada lote do leitor reiniciar o TTS.
  document.addEventListener('hz:reader-content-changed',()=>H48_resetFull(),{passive:true});
}
function H48_boot(){H48_patchGlobals();H48_installSelection();H48_watchReader();}
hzAfterDomReady(H48_boot);
})();
...
document.addEventListener('selectionchange',()=>{
  try{
    ...
    document.getElementById('mini-dock-flag')?.classList.toggle('hz-selecting',!!has);
  }catch(e){}
});
document.addEventListener('click',e=>{
  const tr=e.target.closest&&e.target.closest('#sel-translate');
  const rd=e.target.closest&&e.target.closest('#sel-read');
  if(tr)setTimeout(()=>{document.getElementById('mini-dock-flag')?.classList.remove('hz-selecting');try{window.getSelection().removeAllRanges();}catch(err){}},650);
  if(rd)setTimeout(()=>{document.getElementById('mini-dock-flag')?.classList.remove('hz-selecting');try{window.getSelection().removeAllRanges();}catch(err){}},900);
},true);
...
const HZ_I18N={
 ...
 en:{reading:'Reading',words:'Flash Cards',dict:'Dictionary',sources:'Sources',practice:'Practice',music:'Music',
     ...
     searchPh:'字 / 词 / phrase'},
 es:{reading:'Lectura',words:'Tarjetas',dict:'Diccionario',sources:'Fuentes',practice:'Práctica',music:'Música',
     settings:'Ajustes',back:'Volver',stats:'Estadísticas',profile:'Mi perfil',guest:'Invitado',
     sec1:'Lector y lectura',sec2:'Idioma',sec3:'Tema y apariencia',sec4:'Ayuda y acerca de',sec5:'Almacenamiento y opciones avanzadas',
     voiceRow:'Voz y lectura en voz alta',voiceSub:'Voz, velocidad, tono y estilo de lectura',
     v2Row:'Voz Natural v2',v2Sub:'Opción predeterminada. Desactívala para usar la versión clásica.',
     ...
     flashDecks:'Mazos',flashLevels:'Mazos por niveles',flashTitle:'Tarjetas',
     dictEmptyT:'Busca una palabra o un carácter.',dictEmptyS:'El diccionario usa definiciones, palabras relacionadas, ejemplos y audio natural cuando está disponible.',
     searchPh:'字 / 词 / frase'}
};
function hzLang(){
...
function hzReorgSettings(){
  ...
  g2.insertAdjacentHTML('beforeend',
    '<div class="srow"><div><div class="slbl">'+T('langRow')+'</div><div class="ssub">'+T('langSub')+'</div></div><select class="lvlselect" id="hz-lang-sel"><option value="auto">'+T('langAuto')+'</option><option value="pt">Português</option><option value="en">English</option><option value="es">Español</option></select></div>');
  // G3 — tema e aparência
...
const hzReaderSettingsRoot=document.getElementById('style-scroll');
const hzReaderSettingsObserver=hzReaderSettingsRoot?new MutationObserver(()=>hzQueueReaderSettings()):null;
// O observer fica restrito ao modal de estilo. Observar document.body fazia
// cada card e cada bloco do leitor passar por esta rotina sem necessidade.
hzReaderSettingsObserver?.observe(hzReaderSettingsRoot,{childList:true,subtree:true});
...
document.addEventListener('hz:screen-change',event=>{
 if(event.detail?.id==='sd'&&!window.hsrcRenderDiscoverLocal)hzEnsureCompactDiscover();
},{passive:true});
if(document.querySelector('.screen.active')?.id==='sd'&&!window.hsrcRenderDiscoverLocal)hzEnsureCompactDiscover();
...
function boot(){
 try{ensureProfileScreen();}catch(e){}
 try{installImportOption();}catch(e){}
 // O adapter local é carregado antes do DOMContentLoaded e assume Fontes.
 // Evita montar cards legados para substituí-los imediatamente depois.
 if(!window.hsrcRenderDiscoverLocal)try{compactDiscover();}catch(e){}
}
hzAfterDomReady(boot);
})();
...
hzAfterDomReady(patchTradSimpFlow);
})();
...
(function(){
  ...
  async function migrateStoredLibrary(){
    try{
      ...
      let changedCount=0,processed=0;
      for(const b of arr||[]){
        let changed=false;
      ...
      if(changedCount){try{books=await dbAll(STB);}catch{}}
  ...
  function patchOnce(){
    ...
    try{
      if(typeof buildHTML==='function'&&!buildHTML.__tradSimpV2){
        const old=buildHTML;
        buildHTML=function(text,options){
          const nextOptions=options&&options.fullText!=null
            ?{...options,fullText:toSimp(options.fullText)}
            :options;
          return old.call(this,toSimp(text),nextOptions);
        };
    ...
    try{
      if(document.getElementById('sx')?.classList.contains('active'))simplifyTextNodes(document.getElementById('dict-results'));
    }catch{}
  }
...
(function(){
  ...
  async function h53ResolveDictionaryEntry(rawTerm){
    ...
    const cached=h53DictionaryCache.get(term);if(cached&&Date.now()-cached.at<(cached.value?600000:15000))return cached.value;
    let result=null;
  ...
  async function h54RenderDictDefs(rawQuery,out){
    ...
    if([...q].filter(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}}).length===1){
      void (async()=>{
        ...
        if(stroke?.gif){slot.innerHTML=`<button class="lexi-stroke-card h54-stroke-button" type="button"><img src="${h52Esc(stroke.gif)}" alt="Ordem dos traços"></button>`;slot.querySelector('button').onclick=()=>{try{v41OpenGifModal(stroke.gif,q);}catch{}};}
      })();
    }
...
/* ===== v5.0 refinement bridge: stroke reuse, progressive reader, language accordion ===== */
window.hzLookupStrokeOrder=lookupStrokeOrder;
window.hzSliceStrokeGuide=v41SliceStrokeGuide;
(function(){
  let renderToken=0;
  const READER_ATOMIC_LIMIT=2600;
  const READER_CHUNK_CHARS=240;
  const READER_MOUNT_BUDGET=2;
  const READER_KEEP_DISTANCE=4;
  let virtualReaderState=null;
  function readerChunks(text,target=READER_CHUNK_CHARS){
    const chars=Array.from(text);const chunks=[];let start=0;
    while(start<chars.length){
      let end=Math.min(chars.length,start+target);
      if(end<chars.length){
        let cut=end;
        const floor=start+Math.floor(target*.58);
        while(cut>floor&&chars[cut-1]!=='\n')cut--;
        if(cut<=floor){cut=end;const ceiling=Math.min(chars.length,start+Math.floor(target*1.22));while(cut<ceiling&&chars[cut-1]!=='\n')cut++;}
        end=Math.max(start+1,cut);
      }
      chunks.push({text:chars.slice(start,end).join(''),start,html:'',ready:false,mounted:false,slot:null});start=end;
    }
    return chunks.length?chunks:[{text:'',start:0,html:'',ready:false,mounted:false,slot:null}];
  }
  function htmlFragment(html){const template=document.createElement('template');template.innerHTML=html;return template.content;}
  function readerSelectionActive(root){try{const sel=window.getSelection?.();return Boolean(sel&&!sel.isCollapsed&&sel.anchorNode&&root.contains(sel.anchorNode));}catch{return false;}}
  function readerScrolling(){return document.documentElement.classList.contains('hz-reader-scrolling');}
  function readerInteracting(root){return readerScrolling()||readerSelectionActive(root);}
  function scheduleReaderWork(fn){
    if(globalThis.scheduler?.postTask)return scheduler.postTask(fn,{priority:'background'}).catch(()=>setTimeout(fn,0));
    if('requestIdleCallback' in globalThis)return requestIdleCallback(()=>fn(),{timeout:180});
    setTimeout(fn,0);
  }
  function estimateChunkHeight(scroll,text){
    const fs=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--fs'))||38;
    const width=Math.max(220,(scroll?.clientWidth||innerWidth)-44);
    const perLine=Math.max(7,Math.floor(width/(fs*1.04)));
    const chars=Array.from(text).length;
    const lines=Math.max(1,Math.ceil(chars/perLine));
    const paragraphs=Math.max(0,(text.match(/\n/g)||[]).length);
    // Placeholders baixos demais expandiam durante a rolagem e deslocavam o
    // ponto visível. Uma estimativa levemente conservadora reduz esse salto.
    return Math.max(fs*1.75,Math.ceil(lines*fs*1.72+paragraphs*fs*.32));
  }
  function dispatchReaderEvent(name,detail={}){document.dispatchEvent(new CustomEvent(name,{bubbles:true,detail}));}
  function cancelVirtualReader(){
    const state=virtualReaderState;if(!state)return;
    state.cancelled=true;
    try{state.observer?.disconnect();}catch{}
    try{state.scroll?.removeEventListener('scroll',state.onScroll);}catch{}
    try{state.resize?.disconnect();}catch{}
    clearTimeout(state.retryTimer);
    virtualReaderState=null;
  }
  function slotNearViewport(state,slot,multiplier=1.8){
    if(!slot?.isConnected||!state.scroll)return false;
    const rr=state.scroll.getBoundingClientRect(),sr=slot.getBoundingClientRect();
    const margin=Math.max(rr.height,360)*multiplier;
    return sr.bottom>=rr.top-margin&&sr.top<=rr.bottom+margin;
  }
  function viewportChunkIndex(state){
    const chunks=state?.chunks;if(!chunks?.length||!state.scroll)return 0;
    const rr=state.scroll.getBoundingClientRect();
    const target=rr.top+Math.max(1,rr.height)*.5;
    let lo=0,hi=chunks.length-1,best=0;
    while(lo<=hi){
      const mid=(lo+hi)>>1,slot=chunks[mid]?.slot;
      if(!slot){hi=mid-1;continue;}
      const sr=slot.getBoundingClientRect();best=mid;
      if(sr.bottom<target)lo=mid+1;
      else if(sr.top>target)hi=mid-1;
      else return mid;
    }
    return Math.max(0,Math.min(chunks.length-1,lo<chunks.length?lo:best));
  }
  function preserveScrollForResize(state,slot,oldHeight,oldTop){
    if(!state.scroll)return;
    const newHeight=slot.getBoundingClientRect().height;
    const delta=newHeight-oldHeight;
    if(Math.abs(delta)>.5&&oldTop<state.scroll.scrollTop+2)state.scroll.scrollTop=Math.max(0,state.scroll.scrollTop+delta);
  }
  function materializeReaderChunk(state,index,force=false){
    const chunk=state.chunks[index],slot=chunk?.slot;
    if(!chunk||!slot||chunk.mounted||!chunk.ready||state.cancelled)return false;
    if(!force&&!slotNearViewport(state,slot))return false;
    // A seleção nativa tem prioridade absoluta. Nem mesmo um bloco visível é
    // substituído enquanto o usuário mantém um range ativo.
    if(readerSelectionActive(state.root)){
      state.pending.add(index);
      clearTimeout(state.retryTimer);
      state.retryTimer=setTimeout(()=>scanVirtualReader(state),90);
      return false;
    }
    const oldHeight=slot.getBoundingClientRect().height,oldTop=slot.offsetTop;
    slot.style.visibility='hidden';
    slot.replaceChildren(htmlFragment(chunk.html));
    slot.classList.remove('is-placeholder');slot.classList.add('is-mounted');
    slot.removeAttribute('aria-hidden');slot.style.height='auto';
    slot.style.minHeight=Math.max(1,oldHeight)+'px';
    preserveScrollForResize(state,slot,oldHeight,oldTop);
    slot.style.visibility='visible';
    chunk.mounted=true;state.pending.delete(index);state.mounted++;
    try{applyPinyin(slot);}catch{}
    return true;
  }
  function unmountReaderChunk(state,index){
    const chunk=state.chunks[index],slot=chunk?.slot;
    if(!chunk?.mounted||!slot||state.cancelled||state.pinAll||readerSelectionActive(state.root))return false;
    // Conserva a altura real do bloco antes de liberar seus milhares de nós.
    // Isso mantém a posição do scroll estável quando o usuário volta ao trecho.
    const height=Math.max(1,slot.getBoundingClientRect().height);
    slot.style.height=height+'px';slot.style.minHeight='0';
    slot.replaceChildren();
    slot.classList.remove('is-mounted');slot.classList.add('is-placeholder');
    slot.setAttribute('aria-hidden','true');
    chunk.mounted=false;state.mounted=Math.max(0,state.mounted-1);
    return true;
  }
  function scanVirtualReader(state){
    if(!state||state.cancelled||state.scanFrame)return;
    state.scanFrame=requestAnimationFrame(()=>{
      state.scanFrame=0;if(state.cancelled)return;
      if(readerSelectionActive(state.root)){
        clearTimeout(state.retryTimer);
        state.retryTimer=setTimeout(()=>scanVirtualReader(state),90);
        return;
      }
      const center=viewportChunkIndex(state);
      let mountedNow=0;
      // Usa a posição real dos slots; a antiga estimativa por porcentagem se
      // tornava inválida conforme blocos anteriores ajustavam a própria altura.
      const candidates=[];
      for(let distance=0;distance<=6;distance++){
        const before=center-distance,after=center+distance;
        if(before>=0&&!candidates.includes(before))candidates.push(before);
        if(after<state.chunks.length&&!candidates.includes(after))candidates.push(after);
      }
      if(center<2)candidates.push(0,1);
      for(const i of candidates){
        if(mountedNow>=READER_MOUNT_BUDGET)break;
        const chunk=state.chunks[i];
        if(!chunk?.ready||chunk.mounted)continue;
        const visibleCore=Math.abs(i-center)<=2;
        if(visibleCore||slotNearViewport(state,chunk.slot,2.1)){
          if(materializeReaderChunk(state,i,visibleCore))mountedNow++;
        }
      }
      if(state.pending.size&&mountedNow<READER_MOUNT_BUDGET){
        for(const i of [...state.pending]){
          if(mountedNow>=READER_MOUNT_BUDGET)break;
          if(materializeReaderChunk(state,i,true))mountedNow++;
        }
      }
      // Janela de DOM limitada: blocos distantes voltam a placeholders com a
      // mesma altura. O texto completo continua em memória, mas não no layout.
      if(!state.pinAll){
        for(let i=0;i<state.chunks.length;i++){
          if(Math.abs(i-center)>READER_KEEP_DISTANCE)unmountReaderChunk(state,i);
        }
      }
  ...
  function createVirtualSlots(state){
    const fragment=document.createDocumentFragment();
    state.chunks.forEach((chunk,index)=>{
      const slot=document.createElement('div');
      slot.className='hz-reader-virtual-chunk is-placeholder';slot.dataset.chunk=String(index);slot.setAttribute('aria-hidden','true');
      const estimated=estimateChunkHeight(state.scroll,chunk.text);slot.style.height=estimated+'px';slot.dataset.estimatedHeight=String(estimated);
      chunk.slot=slot;fragment.appendChild(slot);
    });
    state.root.replaceChildren(fragment);
  }
  function installVirtualObservers(state){
    state.onScroll=()=>scanVirtualReader(state);state.scroll?.addEventListener('scroll',state.onScroll,{passive:true});
    if('IntersectionObserver' in globalThis){
      state.observer=new IntersectionObserver(entries=>{
        for(const entry of entries)if(entry.isIntersecting)state.pending.add(Number(entry.target.dataset.chunk));
        scanVirtualReader(state);
      },{root:state.scroll,rootMargin:'120% 0px'});
      state.chunks.forEach(c=>state.observer.observe(c.slot));
    }
    if('ResizeObserver' in globalThis){state.resize=new ResizeObserver(()=>scanVirtualReader(state));state.resize.observe(state.scroll);}
  }
  async function materializeReaderAll(){
    const state=virtualReaderState;if(!state||state.cancelled)return;
    state.pinAll=true;
    while(!state.prepared&&!state.cancelled)await new Promise(r=>setTimeout(r,20));
    for(let i=0;i<state.chunks.length&&!state.cancelled;i++){
      materializeReaderChunk(state,i,true);
      if(i%2===1)await new Promise(r=>requestAnimationFrame(r));
    }
  }
  window.hzMaterializeReaderAll=materializeReaderAll;
  async function progressiveReader(rawText,progress=0){
    const root=document.getElementById('rtext'),scroll=document.getElementById('rscroll');if(!root)return;
    cancelVirtualReader();
    const token=++renderToken;
    const text=v40NormalizeText(String(rawText||''));
    root.classList.add('hz-reader-rendering');root.replaceChildren();
    dispatchReaderEvent('hz:reader-render-start',{token,chars:Array.from(text).length});
    const totalChars=Array.from(text).length;
    if(totalChars<=READER_ATOMIC_LIMIT){
      root.appendChild(htmlFragment(buildHTML(text,{fullText:text})));
      dispatchReaderEvent('hz:reader-content-changed',{token,reason:'open'});
      requestAnimationFrame(()=>{
        if(token!==renderToken)return;
        try{applyPinyin();}catch{}
        if(scroll){const max=scroll.scrollHeight-scroll.clientHeight;scroll.scrollTop=Number(progress||0)*Math.max(0,max);}
        root.classList.remove('hz-reader-rendering');
        dispatchReaderEvent('hz:reader-first-paint',{token,complete:true});
        dispatchReaderEvent('hz:reader-render-complete',{token,chars:totalChars,batches:1,virtualized:false});
      });
      return;
    }
    const chunks=readerChunks(text);
    const initialIndex=Math.max(0,Math.min(chunks.length-1,Math.round(Number(progress||0)*(chunks.length-1))));
    const state={token,text,root,scroll,chunks,cancelled:false,prepared:false,mounted:0,pending:new Set(),pinAll:false,scanFrame:0,observer:null,resize:null,onScroll:null,retryTimer:0};
    virtualReaderState=state;createVirtualSlots(state);
    const prime=[0,initialIndex,initialIndex-1,initialIndex+1].filter((v,i,a)=>v>=0&&v<chunks.length&&a.indexOf(v)===i);
    for(const i of prime){
      const chunk=chunks[i];
      chunk.html=buildHTML(chunk.text,{append:i!==0,fullText:text,startOffset:chunk.start});chunk.ready=true;
    }
    const initialMount=prime.filter(i=>Math.abs(i-initialIndex)<=1).slice(0,READER_MOUNT_BUDGET);
    for(const i of initialMount)materializeReaderChunk(state,i,true);
    installVirtualObservers(state);
    dispatchReaderEvent('hz:reader-content-changed',{token,reason:'open'});
    requestAnimationFrame(()=>{
      if(token!==renderToken||state.cancelled)return;
      if(scroll){const max=scroll.scrollHeight-scroll.clientHeight;scroll.scrollTop=Number(progress||0)*Math.max(0,max);}
      root.classList.remove('hz-reader-rendering');scanVirtualReader(state);
      dispatchReaderEvent('hz:reader-first-paint',{token,complete:false,virtualized:true});
    });
    let index=1;
    const prepare=()=>{
      if(token!==renderToken||state.cancelled||!root.isConnected||!document.getElementById('sr')?.classList.contains('active'))return;
      const started=performance.now();
      while(index<chunks.length&&performance.now()-started<9){
        const chunk=chunks[index];
        if(!chunk.ready){
          chunk.html=buildHTML(chunk.text,{append:true,fullText:text,startOffset:chunk.start});chunk.ready=true;
          if(slotNearViewport(state,chunk.slot,2.4))state.pending.add(index);
        }
        index++;
      }
      scanVirtualReader(state);
      if(index<chunks.length){scheduleReaderWork(prepare);return;}
      state.prepared=true;
      dispatchReaderEvent('hz:reader-render-complete',{token,chars:totalChars,batches:chunks.length,virtualized:true,mounted:state.mounted});
    };
    scheduleReaderWork(prepare);
  }
  window.hzProgressiveReader=progressiveReader;
  const oldSimple=window.v29OpenSimpleReading||v29OpenSimpleReading;
  v29OpenSimpleReading=async function(id){curBook=books.find(b=>b.id===id);if(!curBook)return;showScreen('sr');document.getElementById('rsrc').textContent=curBook.source||curBook.title;document.getElementById('rpct').textContent=Math.round((curBook.progress||0)*100)+'%';await progressiveReader(curBook.content||'',curBook.progress||0);const rs=document.getElementById('rscroll');rs.onscroll=()=>v29SaveReaderProgress(rs,null);curBook.lastRead=Date.now();dbPut(STB,curBook).catch(()=>{});};
  v29OpenBookChapter=async function(id,idx){curBook=books.find(b=>b.id===id)||await v29LoadBook(id);if(!curBook)return;const c=v29GetChapters(curBook)[idx];if(!c)return;curBook._readingChapterIndex=idx;showScreen('sr');document.getElementById('rsrc').textContent=(c.title||('Capítulo '+(idx+1)))+' • '+(curBook.title||'Livro');document.getElementById('rpct').textContent=Math.round((c.progress||0)*100)+'%';await progressiveReader(c.content||'',c.progress||0);const rs=document.getElementById('rscroll');rs.onscroll=()=>v29SaveReaderProgress(rs,idx);curBook.lastRead=Date.now();curBook.lastChapterIndex=idx;dbPut(STB,curBook).catch(()=>{});};
  window.addEventListener('hz:screen-change',e=>{if(e.detail?.id!=='sr'){renderToken++;cancelVirtualReader();}},{passive:true});
  function languageAccordion(){
    const select=document.getElementById('hz-lang-sel');
    if(!select||document.getElementById('hz-language-options'))return false;
    const row=select.closest('.srow');if(!row)return false;
    select.hidden=true;select.setAttribute('aria-hidden','true');
    const trigger=document.createElement('button');trigger.type='button';trigger.className='hz-language-trigger';trigger.setAttribute('aria-expanded','false');trigger.setAttribute('aria-controls','hz-language-options');
    const translate=window.hzT||((key)=>({langAuto:'Automático',langRow:'Idioma da interface'})[key]||key);
    const labels={pt:'Português',en:'English',es:'Español',auto:translate('langAuto')};
    const current=hzLang();trigger.innerHTML=`<span>${labels[current]}</span><span aria-hidden="true">⌄</span>`;
    const box=document.createElement('div');box.id='hz-language-options';box.className='hz-language-options';box.hidden=true;box.setAttribute('role','listbox');box.setAttribute('aria-label',translate('langRow'));
    box.innerHTML=['pt','en','es'].map(code=>`<button type="button" data-lang-code="${code}" role="option" aria-selected="${code===current}">${labels[code]}</button>`).join('');
    select.insertAdjacentElement('afterend',trigger);row.insertAdjacentElement('afterend',box);
    const setOpen=open=>{box.hidden=!open;trigger.setAttribute('aria-expanded',String(open));if(open)box.querySelector('[aria-selected="true"]')?.focus();};
    trigger.addEventListener('click',()=>setOpen(box.hidden));
    trigger.addEventListener('keydown',e=>{if(e.key==='ArrowDown'){e.preventDefault();setOpen(true);}});
    box.addEventListener('keydown',e=>{const buttons=[...box.querySelectorAll('button')],i=buttons.indexOf(document.activeElement);if(e.key==='Escape'){setOpen(false);trigger.focus();}else if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();buttons[(i+(e.key==='ArrowDown'?1:-1)+buttons.length)%buttons.length]?.focus();}});
    box.addEventListener('click',e=>{const b=e.target.closest('[data-lang-code]');if(!b)return;try{localStorage.setItem('hzLang',b.dataset.langCode);localStorage.setItem('hzLanguage',b.dataset.langCode);}catch{}setOpen(false);location.reload();});
    return true;
  }
  const installLanguage=()=>languageAccordion();
  hzAfterDomReady(()=>{
    if(installLanguage())return;
    const settingsRoot=document.getElementById('ss');
    const observer=settingsRoot?new MutationObserver(()=>{if(installLanguage())observer.disconnect();}):null;
    if(observer)observer.observe(settingsRoot,{childList:true,subtree:true});
    window.addEventListener('hz:screen-visible',event=>{if(event.detail?.id==='ss'&&installLanguage())observer?.disconnect();},{passive:true});
  });
})();
