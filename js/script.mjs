...
import { setupTtsBridge } from './static/tts-bridge.mjs';
import { TSVG, DISC } from './static/ui-static.mjs';
import { HSK_WORDS, HSK_EXTRA_WORDS, SEGMENTATION_EXTRA_WORDS } from './static/hsk-data.mjs';
import { V40_TRAD_TO_SIMP, V42_GRAMMAR_DICT, V43_TRACKS, V30_LOCAL_DICT, V34_DICT, HZ_EN_MAP, V32_HSK30, V33_HSK30_MORE, H46_VOICES, HZ_I18N, V34_SOURCES, HR36_SOURCES, V38_EXTRA_SOURCES } from './static/app-data.mjs';
...
let fontSize=24,showPinyin=true,pinyinLevelMode=false,pinyinMinLevel=2;
...
Object.entries(HSK_EXTRA_WORDS).forEach(([w,lv])=>HSK_LEVEL.set(w,lv));
const SEG_WORDS=[...HSK_LEVEL.keys(),...SEGMENTATION_EXTRA_WORDS].sort((a,b)=>b.length-a.length);
...
function loadSettings(){try{fontSize=parseInt(localStorage.getItem('hfs')||'24');showPinyin=localStorage.getItem('hspy')!=='0';pinyinLevelMode=localStorage.getItem('hpl')==='1';pinyinMinLevel=parseInt(localStorage.getItem('hplv')||'2');}catch{}applyFontSize();applyPinyin();}
...
function v37FixPinyinOverlap(){
  ...
  const curFs=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--fs'))||24;
...
function pushWordToken(word,ci){
  ...
  return`<span class="wunit ${lvcls}${hidden}${target}${uncls}" data-tid="${tid}" data-lv="${level>=1&&level<=9?level:'x'}" data-ci="${ci}" data-cilen="${[...word].length}"><span class="hzrow">${buildHanziCells(word,pyInfo.parts)}</span></span>`;
}
...
function buildHTMLProgressiveSlice(paragraphs,start,end,state,total){
  const paraHtml=[];
  for(let p=start;p<end;p++){
    const para=paragraphs[p]||'';
    if(!para.trim()){
      state.pendingGap=true;state.ci+=[...para].length;if(p<total-1)state.ci++;continue;
    }
    const paraStart=state.ci;let html='',run='';
    const flush=()=>{if(!run)return;const words=segmentChineseRun(run);for(const w of words){html+=pushWordToken(w,state.ci);state.ci+=[...w].length;}run='';};
    const chars=[...para];
    for(let i=0;i<chars.length;i++){
      const ch=chars[i];
      if(isCJK(ch)){run+=ch;continue;}
      flush();if(ch==='\r')continue;
      if(/\s/.test(ch)){const spStart=state.ci;let n=1;while(i+1<chars.length&&/\s/.test(chars[i+1])&&chars[i+1]!=="\r"){n++;i++;state.ci++;}html+=tokenSpace(n,spStart);state.ci++;continue;}
      html+=tokenPunct(ch,state.ci);state.ci++;
    }
    flush();
    if(html)paraHtml.push(`<p class="rpara${state.pendingGap?' rpara-gap':''}" data-ci="${paraStart}" data-cilen="${state.ci-paraStart}">${html}</p>`);
    state.pendingGap=false;if(p<total-1)state.ci++;
  }
  return paraHtml.join('');
}
...
function hzReaderIdle(cb){
  if('requestIdleCallback' in window)return window.requestIdleCallback(()=>cb(),{timeout:120});
  return requestAnimationFrame(()=>cb());
}
let hzReaderRenderJob=null;
function hzCancelReaderRender(){
  if(hzReaderRenderJob)hzReaderRenderJob.cancelled=true;
  hzReaderRenderJob=null;
}
function hzApplyPinyinDeferred(){
  if(typeof applyPinyin!=='function')return;
  requestAnimationFrame(()=>{try{applyPinyin();}catch{}});
}
async function hzRenderReaderTextProgressive(text,{loadLabel='Processando texto...',firstBatch=10,batchSize=18}={}){
  const root=document.getElementById('rtext');
  if(!root){return buildHTML(text);}
  hzCancelReaderRender();
  const job={cancelled:false,started:performance.now(),nodes:0};
  hzReaderRenderJob=job;
  text=v40NormalizeText(String(text||''));
  window.__rtextRaw=text;
  root.replaceChildren();
  readerTokens=[];readerCharRefs=[];
  root.classList.add('hz-progressive-reader');
  showLoad?.(loadLabel);
  await frame();
  const paragraphs=text.split('\n');
  const state={ci:0,pendingGap:false};
  const renderSlice=(start,end)=>{
    if(job.cancelled||!root.isConnected)return;
    const html=buildHTMLProgressiveSlice(paragraphs,start,end,state,paragraphs.length);
    if(html){root.insertAdjacentHTML('beforeend',html);job.nodes+=end-start;}
  };
  const firstEnd=Math.min(paragraphs.length,Math.max(1,firstBatch));
  renderSlice(0,firstEnd);
  hzApplyPinyinDeferred();
  document.dispatchEvent(new CustomEvent('hz:reader-first-paint',{detail:{ms:Math.round(performance.now()-job.started),paragraphs:firstEnd}}));
  let index=firstEnd;
  const step=()=>{
    if(job.cancelled||hzReaderRenderJob!==job||!root.isConnected)return;
    const screen=document.getElementById('sr');
    if(screen&&!screen.classList.contains('on')){hzCancelReaderRender();return;}
    const end=Math.min(paragraphs.length,index+batchSize);
    renderSlice(index,end);index=end;
    if(index<paragraphs.length)hzReaderIdle(step);else{hzApplyPinyinDeferred();root.classList.remove('hz-progressive-reader');document.dispatchEvent(new CustomEvent('hz:reader-render-complete',{detail:{ms:Math.round(performance.now()-job.started),paragraphs:paragraphs.length,nodes:job.nodes}}));}
  };
  if(index<paragraphs.length)hzReaderIdle(step);else root.classList.remove('hz-progressive-reader');
  return {firstPaint:true,paragraphs:firstEnd,total:paragraphs.length};
}
(function installReaderDelegation(){
  if(window.__hzReaderDelegation)return;window.__hzReaderDelegation=true;
  document.addEventListener('click',event=>{
    const unit=event.target?.closest?.('#rtext .wunit[data-tid]');
    if(unit&&typeof window.onTap==='function'){event.preventDefault();window.onTap(unit);}
  },{passive:false});
})();
window.hzRenderReaderTextProgressive=hzRenderReaderTextProgressive;
window.hzCancelReaderRender=hzCancelReaderRender;

try{Object.assign(window,{lookupStrokeOrder,v41SliceStrokeGuide,v41OpenStepModal,v41OpenGifModal});}catch{}
...
async function v37ShowTranslate(text){
  ...
  await Promise.all(words.map(async(w,i)=>{
    let r=null;
    try{ r=window.resolveDictionaryEntry?await window.resolveDictionaryEntry(w):await lookupAll(w); }catch{}
    if((!r||!r.defs||!r.defs.length)&&[...String(w)].filter(ch=>{try{return isCJK(ch)}catch{return /[\u3400-\u9fff]/.test(ch)}}).length>1){
      for(const ch of [...String(w)].filter(ch=>{try{return isCJK(ch)}catch{return /[\u3400-\u9fff]/.test(ch)}})){
        try{const cr=window.resolveDictionaryEntry?await window.resolveDictionaryEntry(ch):await lookupAll(ch);if(cr&&cr.defs&&cr.defs.length){r=cr;break;}}catch{}
      }
    }
    wordResults[i]=r;
...
function v29Svg(name){
 const icons={
  ...
  word:'<rect x="4" y="7" width="12" height="9" rx="1.8"/><rect x="7" y="5" width="12" height="9" rx="1.8"/><rect x="9" y="9" width="11" height="8" rx="1.8"/>',
  ...
  practice:'<path d="M4 16.5h14.5a1.5 1.5 0 001.5-1.5V9.5"/><path d="M4 16.5l14.8-4.2"/><path d="M7 15.6V18M11 14.5v3.1M15 13.4v3.6"/><path d="M6.5 8.2c1.6-1.1 3.2-1.1 4.8 0 1.6 1.1 3.2 1.1 4.8 0"/><path d="M18.8 5.6c1.1.8 1.7 1.8 1.9 3"/><path d="M15.9 4.4c1.4 1.3 2.1 2.8 2.1 4.4"/>',
...
async function v29OpenSimpleReading(id){curBook=books.find(b=>b.id===id);if(!curBook)return;showScreen('sr');document.getElementById('rsrc').textContent=curBook.source||curBook.title;document.getElementById('rpct').textContent=Math.round((curBook.progress||0)*100)+'%';document.querySelector('#sr .rtop')?.classList.remove('clickable');document.getElementById('rtext').innerHTML='';try{showLoad('Aguardando pinyin...');await waitPinyin();showLoad('Processando texto...');await hzRenderReaderTextProgressive(curBook.content||'',{loadLabel:'Processando texto...',firstBatch:12,batchSize:20});}catch(e){toast('Erro: '+e.message);document.getElementById('rtext').innerHTML=buildHTML(curBook.content||'');try{applyPinyin();}catch{}}finally{hideLoad();}await frame();const rs=document.getElementById('rscroll');const maxS=rs.scrollHeight-rs.clientHeight;rs.scrollTop=(curBook.progress||0)*maxS;rs.onscroll=()=>v29SaveReaderProgress(rs,null);curBook.lastRead=Date.now();await dbPut(STB,curBook);}
async function v29OpenBookChapter(id,idx){curBook=books.find(b=>b.id===id)||await v29LoadBook(id);if(!curBook)return;const ch=v29GetChapters(curBook);const c=ch[idx];if(!c)return;curBook._readingChapterIndex=idx;showScreen('sr');document.querySelector('#sr .rtop')?.classList.add('clickable');document.getElementById('rsrc').textContent=(c.title||('Capítulo '+(idx+1)))+' • '+(curBook.title||'Livro');document.getElementById('rpct').textContent=Math.round((c.progress||0)*100)+'%';document.getElementById('rtext').innerHTML='';try{showLoad('Aguardando pinyin...');await waitPinyin();showLoad('Processando capítulo...');await hzRenderReaderTextProgressive(c.content||'',{loadLabel:'Processando capítulo...',firstBatch:12,batchSize:20});}catch(e){toast('Erro: '+e.message);document.getElementById('rtext').innerHTML=buildHTML(c.content||'');try{applyPinyin();}catch{}}finally{hideLoad();}await frame();const rs=document.getElementById('rscroll');const maxS=rs.scrollHeight-rs.clientHeight;rs.scrollTop=(c.progress||0)*maxS;rs.onscroll=()=>v29SaveReaderProgress(rs,idx);curBook.lastRead=Date.now();curBook.lastChapterIndex=idx;await dbPut(STB,curBook);}
...
function v37PageChars(){
 const fs=parseInt(getComputedStyle(document.documentElement).getPropertyValue('--fs'))||24;
...
function hzReorgSettings(){
  ...
  g2.insertAdjacentHTML('beforeend',
    '<div class="srow hz-lang-row"><div><div class="slbl">'+T('langRow')+'</div><div class="ssub">'+T('langSub')+'</div></div><div class="hz-lang-accordion" id="hz-lang-accordion"><button type="button" class="lvlselect hz-lang-trigger" id="hz-lang-trigger" aria-expanded="false" aria-controls="hz-lang-options"><span id="hz-lang-current"></span><span aria-hidden="true">⌄</span></button><div class="hz-lang-options" id="hz-lang-options" role="listbox" hidden><button type="button" role="option" data-lang="pt">Português</button><button type="button" role="option" data-lang="en">English</button><button type="button" role="option" data-lang="es">Español</button></div></div></div>');
  // G3 — tema e aparência
  ...
  const langBox=document.getElementById('hz-lang-accordion');
  if(langBox){
    const labels={pt:'Português',en:'English',es:'Español'};
    const trigger=document.getElementById('hz-lang-trigger'),current=document.getElementById('hz-lang-current'),opts=document.getElementById('hz-lang-options');
    const sync=()=>{const lang=hzLang();if(current)current.textContent=labels[lang]||labels.pt;langBox.querySelectorAll('[data-lang]').forEach(b=>{const on=b.dataset.lang===lang;b.classList.toggle('on',on);b.setAttribute('aria-selected',String(on));});};
    const close=()=>{trigger?.setAttribute('aria-expanded','false');if(opts)opts.hidden=true;};
    const toggle=()=>{const open=trigger?.getAttribute('aria-expanded')==='true';trigger?.setAttribute('aria-expanded',String(!open));if(opts)opts.hidden=open;};
    trigger?.addEventListener('click',toggle);trigger?.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle();}});
    langBox.querySelectorAll('[data-lang]').forEach(btn=>{btn.addEventListener('click',()=>{localStorage.setItem('hzLang',btn.dataset.lang);sync();close();hzTranslateChrome();hzRewriteHelp();hzRetitleSections();document.dispatchEvent(new CustomEvent('hz:lang-change',{detail:{lang:btn.dataset.lang}}));});btn.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();btn.click();}});});
    sync();
  }
}
...
(function(){
  ...
  async function h53ResolveDictionaryEntry(rawTerm){
    ...
    // Fallback específico para Hanzi simples: não descarta caracteres isolados
    // nem grava falhas temporárias como ausência permanente.
    if(!h53HasDefs(result)&&[...term].filter(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}}).length===1){
      try{
        const ch=term,entry=h52LocalEntry(ch)||v34EntryLocal(ch)||null;
        if(entry)result=h53EntriesToResult(ch,[{...entry,src:'Dicionário local · caractere'}]);
      }catch{}
      if(!h53HasDefs(result)){
        try{const entries=await v34LookupEntries(term);result=h53EntriesToResult(term,entries)||result;}catch{}
      }
    ...
    if(h53HasDefs(result)||result?.sogou?.length||result?.charDefs?.length)h53DictionaryCache.set(term,{at:Date.now(),value:result});
    return result;
  }
  window.resolveDictionaryEntry=h53ResolveDictionaryEntry;

  function h56InferPosLabel(text,groupPos=''){
    const t=String(text||'').toLowerCase(),g=String(groupPos||'').toLowerCase();
    if(/surname|proper noun|nome próprio|apellido/.test(t+g))return 'nome próprio';
    if(/(to|make|do|be|have|go|come|give|take|see|look|speak|say|ask|teach|learn|read|write|eat|drink|approach|despise|belittle)/.test(t)||/^(to\s+)/.test(t)||/(verbo|verb)/.test(g))return 'verbo';
    if(/(adverb|adv\.|advérbio|adverbio)/.test(t+g)||/ly/.test(t))return 'advérbio';
    if(/(adj\.|adjective|adjetivo)/.test(t+g)||/(thin|weak|light|cold|distant|similar|good|bad|new|old|big|small)/.test(t))return 'adjetivo';
    if(/(preposition|prep\.|preposição|preposicion)/.test(t+g))return 'preposição';
    if(/(conjunction|conj\.|conjunção|conjuncion)/.test(t+g))return 'conjunção';
    if(/(classifier|measure word|mw|contador|clasificador)/.test(t+g))return 'classificador';
    if(/(pronoun|pronome|pronombre)/.test(t+g))return 'pronome';
    if(/(noun|subst\.|substantivo|sustantivo)/.test(t+g))return 'substantivo';
    if(/(person|thing|place|way|path|road|method|name|country|city|book|word|character|language)/.test(t))return 'substantivo';
    return '';
  }
  ...
  async function h54RenderDictDefs(rawQuery,out){
    ...
    if([...q].filter(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}}).length===1){
      void (async()=>{
        ...
        if(stroke?.gif){
          const stepsLabel=(window.hzT?.('strokeSteps')||'Passos'),orderLabel=(window.hzT?.('strokeOrder')||'Ordem dos traços');
          slot.innerHTML=`<div class="h54-stroke-wrap"><button class="lexi-stroke-card h54-stroke-button" type="button" aria-label="${h52Esc(orderLabel)}"><img src="${h52Esc(stroke.gif)}" alt="${h52Esc(orderLabel)}"></button>${stroke.strokeDiagram&&stroke.strokeCount?`<button class="lexi-steps-btn h54-steps-btn" type="button" aria-expanded="false">${h52Esc(stepsLabel)}</button>`:''}</div>`;
          slot.querySelector('.h54-stroke-button').onclick=()=>{try{v41OpenGifModal(stroke.gif,q);}catch{}};
          const toggle=slot.querySelector('.h54-steps-btn'),stepsSlot=root.querySelector('.h54-steps-slot');
          if(toggle&&stepsSlot){let imgs=null,expanded=false;
            const bindCards=()=>stepsSlot.querySelectorAll('[data-i]').forEach(btn=>btn.onclick=()=>{try{v41OpenStepModal(imgs||[],Number(btn.dataset.i)||0);}catch{}});
            const renderRow=()=>{expanded=false;stepsSlot.className='h54-steps-slot lexi-steps-block open';stepsSlot.innerHTML=imgs&&imgs.length?`<button class="lexi-steps-expand-btn" type="button" id="h54-steps-expand">${h52Esc(window.hzT?.('expand')||'Expandir')}<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button><div class="lexi-steps-row">${imgs.map((src,i)=>`<button class="lexi-step-card row" type="button" data-i="${i}"><img src="${src}" alt="${h52Esc(stepsLabel)} ${i+1}"><span>${i+1}</span></button>`).join('')}</div>`:`<div class="dict-empty">${h52Esc(window.hzT?.('strokeStepsUnavailable')||'Passos indisponíveis para este ideograma.')}</div>`;const ex=stepsSlot.querySelector('#h54-steps-expand');if(ex)ex.onclick=renderGrid;bindCards();};
            const renderGrid=()=>{expanded=true;stepsSlot.className='h54-steps-slot lexi-steps-block open';stepsSlot.innerHTML=imgs&&imgs.length?`<button class="lexi-steps-expand-btn" type="button" id="h54-steps-collapse">${h52Esc(window.hzT?.('hideSteps')||'Recolher')}<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="14" y1="10" x2="21" y2="3"/><line x1="3" y1="21" x2="10" y2="14"/></svg></button><div class="lexi-steps-grid h54-three-col">${imgs.map((src,i)=>`<button class="lexi-step-card" type="button" data-i="${i}"><img src="${src}" alt="${h52Esc(stepsLabel)} ${i+1}"><span>${i+1}</span></button>`).join('')}</div>`:`<div class="dict-empty">${h52Esc(window.hzT?.('strokeStepsUnavailable')||'Passos indisponíveis para este ideograma.')}</div>`;const co=stepsSlot.querySelector('#h54-steps-collapse');if(co)co.onclick=renderRow;bindCards();};
            toggle.onclick=async()=>{
              const open=toggle.getAttribute('aria-expanded')==='true';toggle.setAttribute('aria-expanded',String(!open));
              if(open){stepsSlot.classList.remove('open');stepsSlot.innerHTML='';toggle.classList.remove('open');toggle.textContent=stepsLabel;return;}
              toggle.classList.add('open');toggle.textContent=(window.hzT?.('loading')||'Carregando…');
              if(!imgs)imgs=await v41SliceStrokeGuide(stroke.strokeDiagram,parseInt(stroke.strokeCount,10));
              if(!out.isConnected||out.dataset.h54DefsRequest!==requestId)return;
              toggle.textContent=stepsLabel;
              renderRow();
            };}
        }
      })();
    ...
    if(h53HasDefs(result)){
      ...
      for(const group of (result.defs||[]).slice(0,8)){
        ...
        for(const def of rows){index++;const reading=hints.size>1&&def.pyHint?` <span class="lexi-def-reading">— ${h52Esc(q)}: ${h52Esc(def.pyHint)}</span>`:'';const pos=h56InferPosLabel(def.text,group.pos);const posChip=pos?` <span class="lexi-pos-chip">${h52Esc(pos)}</span>`:'';html+=`<div class="lexi-def"><div class="lexi-def-label">Definição ${index}${reading}${posChip}</div>${typeof v39TransButton==='function'?v39TransButton(def.text):h52Esc(def.text)}</div>`;}
        html+='</div>';
      }
...
(function hzUnifiedStorageLayer(){
  if(window.hzStorage)return;
  const PREF_PREFIX='hzPref.';
  const prefCache=new Map();
  const safeParse=(v,f=null)=>{try{return v==null?f:JSON.parse(v);}catch{return f;}};
  const PreferencesStore={
    get(key,fallback=null){if(prefCache.has(key))return prefCache.get(key);let raw=null;try{raw=localStorage.getItem(PREF_PREFIX+key);if(raw==null)raw=localStorage.getItem(key);}catch{}const value=safeParse(raw,raw??fallback);prefCache.set(key,value);return value;},
    set(key,value){prefCache.set(key,value);try{localStorage.setItem(PREF_PREFIX+key,JSON.stringify(value));}catch(e){console.warn('[hzStorage] preference write failed',e?.message||e);}return value;},
    remove(key){prefCache.delete(key);try{localStorage.removeItem(PREF_PREFIX+key);localStorage.removeItem(key);}catch{}},
    clearCache(){prefCache.clear();}
  };
  const StorageFallback={memory:new Map(),async get(store,key){return this.memory.get(store+'::'+key)||null;},async put(store,value,key){const id=key||value?.id||String(Date.now());this.memory.set(store+'::'+id,{...value,id});return id;},async all(store){const prefix=store+'::';return [...this.memory.entries()].filter(([k])=>k.startsWith(prefix)).map(([,v])=>v);}};
  async function openHzDb(){if(!('indexedDB' in window))throw new Error('indexedDB unavailable');return new Promise((resolve,reject)=>{const req=indexedDB.open('hanzi_reader_content',1);req.onupgradeneeded=()=>{const db=req.result;['practiceSessions','dictionaryCache','analysisCache','importedContent'].forEach(name=>{if(!db.objectStoreNames.contains(name))db.createObjectStore(name,{keyPath:'id',autoIncrement:true});});};req.onerror=()=>reject(req.error);req.onsuccess=()=>resolve(req.result);});}
  const ContentDatabase={
    async tx(store,mode,work){try{const db=await openHzDb();return await new Promise((resolve,reject)=>{const t=db.transaction(store,mode);const st=t.objectStore(store);let res;try{res=work(st);}catch(e){reject(e);return;}t.oncomplete=()=>{db.close();resolve(res)};t.onerror=()=>{db.close();reject(t.error)};});}catch(e){console.warn('[hzStorage] IndexedDB fallback:',e?.message||e);return null;}},
    async put(store,value,key){const v={...(value||{})};if(key!=null)v.id=key;if(v.id==null)v.id=Date.now()+'-'+Math.random().toString(36).slice(2);const ok=await this.tx(store,'readwrite',st=>st.put(v));if(ok===null)return StorageFallback.put(store,v,v.id);return v.id;},
    async get(store,key){const res=await this.tx(store,'readonly',st=>st.get(key));return res===null?StorageFallback.get(store,key):res;},
    async all(store){const res=await this.tx(store,'readonly',st=>st.getAll());return res===null?StorageFallback.all(store):res;}
  };
  const StorageMigration={async run(){const known=['hzLang','hzVoiceV2'];for(const key of known){try{const v=localStorage.getItem(key);if(v!=null&&!localStorage.getItem(PREF_PREFIX+key))PreferencesStore.set(key,safeParse(v,v));}catch{}}return true;}};
  window.hzStorage={PreferencesStore,ContentDatabase,StorageMigration,StorageFallback};
  hzReaderIdle?.(()=>StorageMigration.run());
})();

/* ===== h55-render-stability-fix ===== */
/* Corrige piscadas/reinserções tardias ao navegar, e estabiliza leitor/seleção. */
(function(){
'use strict';
if(window.__h55RenderStabilityFix)return;window.__h55RenderStabilityFix=true;

const H55={activeRender:0,readerJob:null,lastLibKey:'',lastScreen:''};
const q=id=>document.getElementById(id);
const raf=()=>new Promise(r=>requestAnimationFrame(r));
const idle=cb=>('requestIdleCallback' in window?requestIdleCallback(cb,{timeout:80}):setTimeout(cb,16));
const esc2=s=>{try{return esc(s);}catch{return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}};
const kindOf=b=>{try{return h36BookKind(b);}catch{return b?.kind||(b?.chapters?'book':'simple');}};
const chaptersOf=b=>{try{return h36Chapters(b);}catch{return Array.isArray(b?.chapters)?b.chapters:[];}};
const pctOf=b=>{try{return h36Progress(b);}catch{return Math.round(((b?.progress)||0)*100);}};
const svgBook=()=>{try{return SVG.book;}catch{return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M5 4h10.5A2.5 2.5 0 0 1 18 6.5V20H7.5A2.5 2.5 0 0 1 5 17.5V4z"/><path d="M8 7h6"/><path d="M8 10h5"/></svg>';}};
const iconBtn=(cls,icon,title)=>{try{return h36IconBtn(cls,icon,title);}catch{return `<button class="${cls}" type="button" title="${esc2(title)}">${icon||''}</button>`;}};
function renderCardFragment(list,mode){
  const frag=document.createDocumentFragment();
  if(mode==='simple'){
    const tools=document.createElement('div');tools.className='lib-tools';tools.innerHTML=`<div class="lib-title">${esc2(window.hzT?.('simpleReading')||'Leitura simples')}</div><button class="lib-chip" id="simple-import-chip" type="button">${esc2(window.hzT?.('import')||'Importar')}</button>`;frag.appendChild(tools);
    const wrap=document.createElement('div');wrap.className='simple-list';wrap.id='simple-list';frag.appendChild(wrap);
    if(!list.length){wrap.innerHTML=`<div class="emptyx"><b>${esc2(window.hzT?.('noSimpleReadings')||'Nenhuma leitura simples.')}</b><br>${esc2(window.hzT?.('importHint')||'Toque em Importar ou adicione uma source.')}</div>`;return frag;}
    const items=document.createDocumentFragment();
    list.forEach(b=>{
      const pct=Math.round((b.progress||0)*100);const el=document.createElement('div');el.className='card has-actions h55-ready';
      el.innerHTML=`<div class="card-actions">${iconBtn('ana-btn',(typeof SVG!=='undefined'?SVG.ana:''),'Analytics')}${iconBtn('del-btn',(typeof SVG!=='undefined'?SVG.trash:''),'Excluir')}</div><div class="thumb">${svgBook()}</div><div class="bi"><div class="bt">${esc2(b.title||'Sem título')}</div><div class="bs">${esc2(b.source||'Leitura')}</div><div class="bm">${typeof timeAgo==='function'?timeAgo(b.lastRead):''}</div><div class="bpb"><div class="bpf" style="width:${pct}%"></div></div></div>`;
      el.querySelector('.del-btn')?.addEventListener('click',e=>{e.stopPropagation();try{confirmDelBook(b.id);}catch{}});
      el.querySelector('.ana-btn')?.addEventListener('click',e=>{e.stopPropagation();try{h36ShowAnalytics(b.title||'Leitura',b.content||'');}catch{}});
      el.addEventListener('click',e=>{if(e.target.closest('button'))return;try{openBook(b.id);}catch{}});
      try{addLP(el,()=>confirmDelBook(b.id));}catch{}
      items.appendChild(el);
    });
    wrap.appendChild(items);return frag;
  }
  const tools=document.createElement('div');tools.className='lib-tools';tools.innerHTML=`<div class="lib-title">${esc2(window.hzT?.('books')||'Livros')}</div><button class="lib-chip" id="book-new-chip" type="button">${esc2(window.hzT?.('newBook')||'Novo livro')}</button><button class="lib-chip ${v29BookView==='cover'?'on':''}" id="view-cover" type="button">${esc2(window.hzT?.('covers')||'Capas')}</button><button class="lib-chip ${v29BookView==='list'?'on':''}" id="view-list" type="button">${esc2(window.hzT?.('list')||'Lista')}</button>`;frag.appendChild(tools);
  const wrap=document.createElement('div');wrap.className=v29BookView==='cover'?'lib-grid':'simple-list book-list';wrap.id='book-wrap';frag.appendChild(wrap);
  if(!list.length){wrap.innerHTML=`<div class="emptyx"><b>${esc2(window.hzT?.('noBooks')||'Nenhum livro.')}</b><br>${esc2(window.hzT?.('newBookHint')||'Toque em Novo livro ou adicione uma source de livro.')}</div>`;return frag;}
  const items=document.createDocumentFragment();
  list.forEach(b=>{
    const ch=chaptersOf(b);const pct=pctOf(b);const el=document.createElement('div');el.className='book-card h55-ready';
    let coverStyle='';try{coverStyle=v29CoverStyle(b);}catch{}
    el.innerHTML=`<div class="book-actions">${iconBtn('book-ana',(typeof SVG!=='undefined'?SVG.ana:''),'Analytics')}${iconBtn('book-edit2',(typeof SVG!=='undefined'?SVG.edit:''),'Editar')}${iconBtn('book-del2',(typeof SVG!=='undefined'?SVG.trash:''),'Excluir')}</div><div class="book-cover ${b.cover?'':'gen'}" style="${coverStyle}"></div><div><div class="book-name">${esc2(b.title||'Sem título')}</div><div class="book-syn">${esc2(b.synopsis||'Sem sinopse')}</div><div class="book-meta"><span>${ch.length} cap.</span><span>${pct}%</span></div></div>`;
    el.querySelector('.book-del2')?.addEventListener('click',e=>{e.stopPropagation();try{confirmDelBook(b.id);}catch{}});
    el.querySelector('.book-edit2')?.addEventListener('click',e=>{e.stopPropagation();try{v29OpenBookEditor(b.id);}catch{}});
    el.querySelector('.book-ana')?.addEventListener('click',e=>{e.stopPropagation();try{showAnalyticsForBook(b.id);}catch{}});
    el.addEventListener('click',e=>{if(e.target.closest('button'))return;if(ch.length)openBook(b.id);else try{toast('Adicione pelo menos um capítulo');}catch{}});
    try{addLP(el,()=>v29OpenChapterPicker(b.id));}catch{}
    items.appendChild(el);
  });wrap.appendChild(items);return frag;
}
function stableRenderLib(force=false){
  const bc=q('bc');if(!bc)return;
  const mode=(window.v29LibMode||v29LibMode)==='books'?'books':'simple';
  const src=Array.isArray(window.books)?window.books:(typeof books!=='undefined'?books:[]);
  const search=(typeof searchQ!=='undefined'?String(searchQ||''):'').toLowerCase();
  const base=src.filter(b=>mode==='books'?kindOf(b)==='book':kindOf(b)!=='book');
  const list=search?base.filter(b=>(b.title||'').toLowerCase().includes(search)||(b.source||'').toLowerCase().includes(search)||(b.synopsis||'').toLowerCase().includes(search)):base;
  const key=mode+'|'+search+'|'+list.map(b=>`${b.id}:${b.updatedAt||b.lastRead||b.addedAt||0}:${b.progress||0}:${chaptersOf(b).length}`).join(',')+'|'+(window.v29BookView||v29BookView||'');
  if(!force&&H55.lastLibKey===key&&bc.dataset.h55Stable==='1')return;
  H55.lastLibKey=key;
  q('mode-simple')?.classList.toggle('on',mode==='simple');q('mode-books')?.classList.toggle('on',mode==='books');q('plus-simple')?.classList.toggle('on',mode==='simple');q('plus-books')?.classList.toggle('on',mode==='books');
  const active=document.activeElement;const activeId=active?.id||'';const top=bc.scrollTop;
  const frag=renderCardFragment(list,mode);
  bc.classList.add('h55-rendering');
  bc.replaceChildren(frag);
  bc.dataset.h55Stable='1';bc.scrollTop=top;
  q('simple-import-chip')?.addEventListener('click',()=>{try{v29ImportContext='simple';v29PrepImport('Adicionar leitura simples');showModal('mo-import');}catch{}});
  q('book-new-chip')?.addEventListener('click',()=>{try{v29OpenBookEditor();}catch{}});
  q('view-cover')?.addEventListener('click',()=>{try{v39SetBookView('cover');}catch{}});
  q('view-list')?.addEventListener('click',()=>{try{v39SetBookView('list');}catch{}});
  if(activeId)q(activeId)?.focus?.({preventScroll:true});
  requestAnimationFrame(()=>bc.classList.remove('h55-rendering'));
}
function patchLib(){
  try{renderLib=function(){return stableRenderLib(true);};window.renderLib=renderLib;}catch{}
  let resizeTimer=0;window.addEventListener('resize',()=>{clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>stableRenderLib(false),120);},{passive:true});
}
function cancelReaderJob(){try{if(H55.readerJob)H55.readerJob.cancelled=true;if(window.hzCancelReaderRender)window.hzCancelReaderRender();}catch{}H55.readerJob=null;}
function syncBuildReader(text){
  try{return buildHTML(text);}catch(e){return esc2(text).replace(/\n{2,}/g,'</p><p>').replace(/^/,'<p class="rpara">').replace(/$/,'</p>');}
}
async function stableReaderRender(text,{loadLabel='Processando texto...'}={}){
  const root=q('rtext');if(!root)return syncBuildReader(text);
  cancelReaderJob();const token=++H55.activeRender;const job={cancelled:false,started:performance.now()};H55.readerJob=job;
  const raw=(typeof v40NormalizeText==='function'?v40NormalizeText(String(text||'')):String(text||''));window.__rtextRaw=raw;
  root.classList.add('h55-reader-building');root.replaceChildren();
  try{readerTokens=[];readerCharRefs=[];}catch{}
  try{showLoad?.(loadLabel);}catch{}
  await raf();if(job.cancelled||token!==H55.activeRender)return {cancelled:true};
  // Monta fora do fluxo e faz um único commit. Isso evita parágrafos "aparecendo" durante scroll/seleção.
  const html=syncBuildReader(raw);
  if(job.cancelled||token!==H55.activeRender)return {cancelled:true};
  const tpl=document.createElement('template');tpl.innerHTML=html;
  root.replaceChildren(tpl.content);
  root.classList.remove('h55-reader-building','hz-progressive-reader');
  try{applyPinyin();}catch{}
  document.dispatchEvent(new CustomEvent('hz:reader-render-complete',{detail:{ms:Math.round(performance.now()-job.started),paragraphs:root.querySelectorAll('.rpara').length,nodes:root.querySelectorAll('.wunit').length,mode:'single-commit'}}));
  H55.readerJob=null;return {firstPaint:true,total:root.querySelectorAll('.rpara').length,mode:'single-commit'};
}
function patchReader(){
  window.hzRenderReaderTextProgressive=stableReaderRender;
  try{hzRenderReaderTextProgressive=stableReaderRender;}catch{}
  const openStable=async(id,chapterIdx)=>{
    let b=(books||[]).find(x=>x.id===id);if(!b&&typeof v29LoadBook==='function')b=await v29LoadBook(id);if(!b)return;
    const isChapter=chapterIdx!=null;const ch=isChapter?chaptersOf(b)[chapterIdx]:null;curBook=b;if(isChapter)curBook._readingChapterIndex=chapterIdx;
    showScreen('sr');q('sr')?.classList.add('h55-reader-open');document.querySelector('#sr .rtop')?.classList.toggle('clickable',!!isChapter);
    q('rsrc').textContent=isChapter?((ch.title||('Capítulo '+(chapterIdx+1)))+' • '+(b.title||'Livro')):(b.source||b.title||'Leitura');
    q('rpct').textContent=Math.round(((isChapter?ch?.progress:b.progress)||0)*100)+'%';
    const rs=q('rscroll');if(rs){rs.onscroll=null;rs.scrollTop=0;}
    try{await waitPinyin?.();}catch{}
    await stableReaderRender(isChapter?(ch?.content||''):(b.content||''),{loadLabel:isChapter?'Montando capítulo...':'Montando leitura...'});
    try{hideLoad?.();}catch{}
    await raf();
    if(rs){const max=Math.max(0,rs.scrollHeight-rs.clientHeight);rs.scrollTop=((isChapter?ch?.progress:b.progress)||0)*max;rs.onscroll=()=>{try{v29SaveReaderProgress(rs,isChapter?chapterIdx:null);}catch{}};}
    b.lastRead=Date.now();if(isChapter)b.lastChapterIndex=chapterIdx;try{await dbPut(STB,b);}catch{}
  };
  try{v29OpenSimpleReading=function(id){return openStable(id,null);};window.v29OpenSimpleReading=v29OpenSimpleReading;}catch{}
  try{v29OpenBookChapter=function(id,idx){return openStable(id,idx);};window.v29OpenBookChapter=v29OpenBookChapter;}catch{}
  try{openBook=async function(id){const b=(books||[]).find(x=>x.id===id);if(!b)return;if(kindOf(b)==='book'){const ch=chaptersOf(b);return openStable(id,Math.max(0,Math.min(b.lastChapterIndex||0,ch.length-1)));}return openStable(id,null);};window.openBook=openBook;}catch{}
}
function patchSelection(){
  let scheduled=false,lastKey='';
  function capture(){
    scheduled=false;const sr=q('sr');if(!sr?.classList.contains('active'))return;
    const sel=window.getSelection?.();const dom=(sel?String(sel):'').trim();const ok=!!dom&&[...dom].some(ch=>{try{return isCJK(ch);}catch{return /[\u3400-\u9fff]/.test(ch);}});const key=ok?dom+'|'+(sel?.rangeCount||0):'';if(key===lastKey&&!ok)return;lastKey=key;
    try{if(ok){const raw=(typeof v37RawSelectionSlice==='function'?v37RawSelectionSlice(sel):'')||dom;try{selRaw=raw;selTxt=dom;}catch{}window.selRaw=raw;window.selTxt=dom;}else{try{selRaw='';selTxt='';}catch{}window.selRaw='';window.selTxt='';}}catch{}
    document.body.classList.toggle('hz-selecting',ok);
    q('mini-dock-flag')?.classList.toggle('show',ok);
    const cjk=ok?[...dom].filter(ch=>/[\u3400-\u9fff]/.test(ch)).length:0;const trans=q('sel-translate');if(trans)trans.style.display=cjk>=2?'inline-flex':'none';
  }
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(capture);} 
  document.addEventListener('selectionchange',schedule,{passive:true});
  ['pointerup','touchend','keyup'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(capture,0),{passive:true,capture:true}));
}
function patchScreen(){
  const old=window.showScreen||showScreen;
  window.showScreen=showScreen=function(id){
    const prev=H55.lastScreen||document.querySelector('.screen.active')?.id||'';
    if(prev==='sr'&&id!=='sr')cancelReaderJob();
    const r=old.call(this,id);
    H55.lastScreen=id;
    if(id==='sl')requestAnimationFrame(()=>stableRenderLib(false));
    requestAnimationFrame(()=>document.dispatchEvent(new CustomEvent('hz:screen-stabilized',{detail:{id,previous:prev}})));
    return r;
  };
}
function boot(){patchLib();patchReader();patchSelection();patchScreen();try{stableRenderLib(false);}catch{}}
