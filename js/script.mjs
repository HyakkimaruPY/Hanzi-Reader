...
function hzDictCacheRead(kind,word){
  // Cache apenas em memória: buscas do dicionário não devem ocupar localStorage.
  const k=hzDictCacheKey(kind,word);
  return HZ_DICT_MEM_CACHE.has(k)?HZ_DICT_MEM_CACHE.get(k):null;
}
function hzDictCacheWrite(kind,word,val){
  // Cache apenas na sessão atual. Persistência antiga é limpa no patch H50.
  const k=hzDictCacheKey(kind,word);
...
lookupAll=async function(word){
  ...
  const res=await HZ_LOOKUP_ALL_CORE(q);
...
function h36RenderFlash(){h36EnsureDeck();const wc=document.getElementById('wc');if(!wc)return;const tab=(()=>{const t=localStorage.getItem('h36FlashTab');return(t==='decks'||t==='levels')?t:'decks';})();wc.innerHTML=`<div class="flash-tabs"><button class="flash-tab ${tab==='decks'?'on':''}" data-ftab="decks">Baralhos</button><button class="flash-tab ${tab==='levels'?'on':''}" data-ftab="levels">Baralhos por níveis</button></div><div id="flash-body"></div>`;wc.querySelectorAll('[data-ftab]').forEach(b=>b.onclick=()=>{localStorage.setItem('h36FlashTab',b.dataset.ftab);h36RenderFlash();});const body=document.getElementById('flash-body');
  if(tab==='decks'){
    ...
    const order=[1,2,3,4,5,6,7,8,9,99,'phrase'];
    const labels={1:'HSK 1',2:'HSK 2',3:'HSK 3',4:'HSK 4',5:'HSK 5',6:'HSK 6',7:'HSK 7',8:'HSK 8',9:'HSK 9',99:'Fora da base',phrase:'Frases'};
...
function h36LevelKeyOf(w){return w.type==='phrase'?'phrase':((w.level>=1&&w.level<=9)?w.level:99);}
...
(function(){
const HZP_GAME_B64='';
...
window.hzOpenMusic=function(){try{showModal('mo-music');v43RenderTrackList('');}catch(e){}};
...
const SVG_GUZHENG='<svg fill="currentColor" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1536 1536"><path d="M975,1233L974,1231L968,1235L964,1235L939,1245L901,1256L922,1252Z M990,1225L979,1229L979,1231Z M1004,1219L1001,1219L993,1224L996,1224Z M1058,1211L1057,1207L1055,1207L1034,1220L993,1241L940,1261L870,1278L823,1284L745,1285L689,1279L632,1267L590,1254L561,1242L558,1243L558,1246L617,1267L677,1281L741,1289L812,1289L852,1285L914,1273L950,1262L1013,1236Z M1028,1206L1017,1212Z M1150,1122L1152,1125L1152,1122Z M730,1090L727,1105L725,1105L724,1109L713,1110L714,1112L725,1113L730,1132L733,1116L736,1116L739,1112L748,1110L738,1109L736,1107L736,1103L732,1103Z M729,1107L731,1107L734,1111L731,1114L729,1114L726,1110Z M186,987L188,1043L191,1046L365,1154L420,1186L420,1124L398,1099L364,1066L326,1036L301,1020L273,1006L250,997L225,990L205,986Z M195,991L225,995L268,1009L301,1026L340,1052L371,1079L414,1125L413,1174L198,1042L195,1038Z M385,976L397,976L409,982L412,980L415,982L411,985L412,990L420,992L426,988L434,987L435,981L433,975L444,976L450,987L463,991L464,988L460,985L459,978L462,977L469,980L475,977L476,974L472,970L465,969L461,974L453,976L447,973L449,972L450,968L453,970L453,964L451,963L445,964L440,972L434,972L432,970L434,959L421,948L415,947L413,949L406,949L402,952L401,963L404,970L401,973L389,973Z M417,983L421,984L419,989L415,987
652,181L608,192L606,194L567,206L518,228L474,253L428,285L399,309L359,348L332,379L300,423L267,479L251,513L234,557L233,565L228,576L219,609L211,652L206,695L205,759L208,796L218,855L228,891L230,892L232,888L220,844L211,784L209,761L209,709L214,659L224,606L236,564L254,517L274,475L300,431L323,398L364,349L400,314L450,274L477,256L523,230L568,210L606,197L643,187L705,176L770,172L802,173L849,177L897,186L934,196L993,218L1028,235L1065,257L1090,274L1128,304L1181,356L1205,386L1210,390Z" fill="#e1c8ac" fill-rule="evenodd" stroke="none"/></svg>';
...
#sp .hzp-card{backdrop-filter:blur(3px)}
...
function hzEnsureScreen(){
 ...
 sec.innerHTML=`<div class="bc" id="hz-sp-hub" style="flex:1;overflow-y:auto;padding:calc(var(--st) + 26px) 18px 26px">
   ...
   <div class="hzp-card" id="hzp-music"><div class="hzp-ico">${SVG_GUZHENG}</div><div class="hzp-lbl">Músicas · Guzheng · <span style="font-family:var(--rf)">古筝</span></div><div class="hzp-chev"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 6 15 12 9 18"/></svg></div></div>
 ...
 hzSyncNav();
 document.getElementById('hzp-music').addEventListener('click',()=>{window.hzOpenMusic();});
...
function hzShowGame(){
 ...
 if(!hub||!host)return;
 if(!document.getElementById('hz-sp-frame')){
...
function hzUnloadGame(){
 const fr=document.getElementById('hz-sp-frame');
 if(fr){try{fr.src='about:blank';}catch(e){}fr.remove();}
...
(function(){
  ...
  function toSimp(x){
    try{ return v40ToSimplified(asText(x)); }
  ...
  async function bootTradSimpV2(){
    patchOnce();
    await ensureTradSimpDb();
    patchOnce();
    simplifyReaderNow();
    migrateStoredLibrary();
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bootTradSimpV2);else bootTradSimpV2();
  [250,900,1800,3600,6500].forEach(ms=>setTimeout(()=>{patchOnce();ensureTradSimpDb().then(()=>{patchOnce();simplifyReaderNow();simplifyTextNodes(document.getElementById('dict-results'));});},ms));
})();


/* ===== h50-runtime-polish-trad-dict-ui =====
   Correção cumulativa:
   - Tradicional -> simplificado aplicado depois do DB carregar e depois de qualquer render tardio.
   - Dicionário sem cache persistente em localStorage.
   - Botão de busca nos cards de palavras/termos/frases.
   - Oculta UIs legadas fantasma até a interface atual estar pronta. */
(function(){
'use strict';
const H50_TSDB_VER='20260709-h50-runtime';
const CJK_RE=/[\u3400-\u9fff\uf900-\ufaff]/;
const CJK_RUN_RE=/[\u3400-\u9fff\uf900-\ufaff]+/;
const H50_FALLBACK={
  '歡':'欢','營':'营','臺':'台','灣':'湾','裡':'里','裏':'里','頁':'页','燈':'灯','聲':'声','鳥':'鸟','館':'馆','們':'们','個':'个','來':'来','時':'时','會':'会','為':'为','這':'这','說':'说','對':'对','國':'国','學':'学','過':'过','還':'还','後':'后','發':'发','開':'开','關':'关','門':'门','邊':'边','樹':'树','銅':'铜','紋':'纹','鷺':'鹭','書':'书','讀':'读','寫':'写','語':'语','聽':'听','愛':'爱','氣':'气','飛':'飞','風':'风','雲':'云','魚':'鱼','龍':'龙','體':'体','習':'习','歐':'欧','廣':'广','東':'东','電':'电','腦':'脑','網':'网','醫':'医','師':'师','樂':'乐','點':'点','難':'难','長':'长','買':'买','賣':'卖','錢':'钱','問':'问','應':'应','該':'该','總':'总','無':'无','從':'从','讓':'让','當':'当','間':'间','樣':'样','歲':'岁','麼':'么','萬':'万','權':'权','責':'责','務':'务','負':'负','擔':'担','險':'险','試':'试','驗':'验','測':'测','較':'较','確':'确','選':'选','擇':'择','觀':'观','視':'视','覽':'览','讚':'赞','贊':'赞','嘆':'叹','嗎':'吗','雖':'虽','盡':'尽','僅':'仅','雙':'双','與':'与','於':'于','將':'将','導':'导','歷':'历','傳':'传','統':'统','錄':'录','號':'号','標':'标','準':'准','質':'质','價':'价','產':'产','資':'资','層':'层','樓':'楼','橋':'桥','鐵':'铁','鋼':'钢','銀':'银','鑰':'钥','熱':'热','涼':'凉','溫':'温','濕':'湿','乾':'干','淨':'净','髒':'脏','臉':'脸','頭':'头','腳':'脚','髮':'发','顏':'颜','隱':'隐','趕':'赶','趨':'趋'
};
const H50_PHRASE_FALLBACK={'喜歡':'喜欢','露營':'露营','博物館':'博物馆','這裡':'这里','這樣':'这样','這麼':'这么','那裡':'那里','那樣':'那样','那麼':'那么','甚麼':'什么','怎麼':'怎么','為什麼':'为什么','因為':'因为','雖然':'虽然','儘管':'尽管','僅僅':'仅仅','對於':'对于','關於':'关于','關係':'关系','聯繫':'联系','資訊':'资讯','電腦':'电脑','會議':'会议','沒有':'没有','進行':'进行','連續':'连续','還是':'还是','起來':'起来','輕鬆':'轻松','負責':'负责','責任':'责任','買賣':'买卖','質量':'质量','傳統':'传统','將來':'将来','決定':'决定','活動':'活动','測驗':'测验','競賽':'竞赛','統一':'统一','總結':'总结','輸出':'输出','轉變':'转变','裏面':'里面','雞蛋':'鸡蛋','歐洲':'欧洲','沒有':'没有','狀態':'状态','緣分':'缘分','聯合':'联合','肅靜':'肃静','鄰居':'邻居'};
let h50DbPromise=null;
let h50ObserverBusy=false;
function h50Esc(s){try{return typeof esc==='function'?esc(s):String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}catch{return String(s??'');}}
function h50CjkOnly(s){return ([...String(s||'')].filter(ch=>CJK_RE.test(ch)).join(''));}
function h50ExtractTerm(el){
  try{
    const raw=(el?.dataset?.dictWord||el?.dataset?.v34Speak||el?.dataset?.word||el?.dataset?.zh||el?.getAttribute?.('data-v34-speak')||el?.querySelector?.('.ww,.zh,.lexi-acc-row-label,.sent-zh,.lexi-zh')?.textContent||el?.textContent||'');
    const m=String(raw).match(CJK_RUN_RE);
    return m?m[0]:String(raw).trim();
  }catch{return'';}
}
function h50SimpSync(text){
  let s=String(text??'');
  try{ if(typeof v40ToSimplified==='function') s=v40ToSimplified(s); }catch{}
  try{
    const chars=[...s];let out='',i=0;
    while(i<chars.length){let ok=false;for(let len=Math.min(8,chars.length-i);len>1;len--){const sub=chars.slice(i,i+len).join('');if(H50_PHRASE_FALLBACK[sub]){out+=H50_PHRASE_FALLBACK[sub];i+=len;ok=true;break;}}if(ok)continue;out+=H50_FALLBACK[chars[i]]||chars[i];i++;}
    return out;
  }catch{return s;}
}
async function h50EnsureTradDb(){
  if(h50DbPromise)return h50DbPromise;
  h50DbPromise=(async()=>{
    let loaded=false,lastErr=null;
    try{if(typeof window.loadTradSimpDb==='function'){loaded=await window.loadTradSimpDb();}}catch(e){lastErr=e;}
    if(!loaded){
      for(const url of [`db/traditional-simplified.json?v=${H50_TSDB_VER}`,'db/traditional-simplified.json']){
        try{const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const db=await r.json();if(typeof v40MergeTradSimpDb==='function')v40MergeTradSimpDb(db);loaded=true;break;}catch(e){lastErr=e;}
      }
    }
    try{window.HZ_TRAD_SIMP_DB_READY=!!loaded;}catch{}
    if(!loaded)try{console.warn('[H50 Trad->Simp] fallback interno usado:',lastErr);}catch{}
    return !!loaded;
  })();
  return h50DbPromise;
}
async function h50Simp(text){await h50EnsureTradDb();return h50SimpSync(text);}
function h50PatchAttrs(root){
  try{(root||document).querySelectorAll('[data-v34-speak],[data-sent-play],[data-auto-text],[data-dict-word],[data-word],[data-zh]').forEach(el=>{
    ['v34Speak','sentPlay','autoText','dictWord','word','zh'].forEach(k=>{if(el.dataset&&el.dataset[k]&&CJK_RE.test(el.dataset[k]))el.dataset[k]=h50SimpSync(el.dataset[k]);});
  });}catch{}
}
function h50SimplifyTextNodes(root){
  if(h50ObserverBusy||!root)return;
  h50ObserverBusy=true;
  try{
    const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,{acceptNode(n){const p=n.parentElement;if(!p||/^(SCRIPT|STYLE|TEXTAREA|INPUT)$/i.test(p.tagName))return NodeFilter.FILTER_REJECT;return CJK_RE.test(n.nodeValue||'')?NodeFilter.FILTER_ACCEPT:NodeFilter.FILTER_SKIP;}});
    const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
    nodes.forEach(n=>{const s=h50SimpSync(n.nodeValue);if(s!==n.nodeValue)n.nodeValue=s;});
    h50PatchAttrs(root);
  }catch{}finally{h50ObserverBusy=false;}
}
function h50OpenDict(term){
  term=h50SimpSync(term).trim();if(!term)return;
  try{if(typeof showScreen==='function')showScreen('sx');}catch{}
  try{v29DictTab='defs';}catch{}
  const inp=document.getElementById('dict-q');if(inp)inp.value=term;
  try{if(typeof v29RunDict==='function')return v29RunDict(term);}catch(e){try{console.warn('[H50] dict jump failed',e);}catch{}}
}
function h50IconSearch(){return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><circle cx="11" cy="11" r="7"/><line x1="20" y1="20" x2="16.2" y2="16.2"/></svg>';}
function h50MakeSearchButton(term){const b=document.createElement('button');b.type='button';b.className='hz-word-search-btn v34-svg-only';b.title='Pesquisar no dicionário';b.setAttribute('aria-label','Pesquisar no dicionário');b.dataset.hzSearchWord=h50SimpSync(term);b.innerHTML=h50IconSearch();b.onclick=e=>{e.preventDefault();e.stopPropagation();h50OpenDict(b.dataset.hzSearchWord||term);};return b;}
function h50InjectSearchButtons(root){
  root=root||document;
  try{
    root.querySelectorAll('.lexi-hero-actions').forEach(row=>{if(row.querySelector('.hz-word-search-btn'))return;const term=h50ExtractTerm(document.querySelector('.lexi-zh'))||document.getElementById('dict-q')?.value||'';row.insertBefore(h50MakeSearchButton(term),row.firstChild);});
    root.querySelectorAll('.lexi-acc-row').forEach(row=>{if(row.querySelector('.hz-word-search-btn'))return;const term=h50ExtractTerm(row);const before=row.querySelector('.lexi-save-btn,.dict-audio,.lexi-acc-chev');row.insertBefore(h50MakeSearchButton(term),before||row.lastChild);});
    root.querySelectorAll('.dict-item').forEach(row=>{if(row.querySelector('.hz-word-search-btn'))return;const term=h50ExtractTerm(row);const before=row.querySelector('.dict-audio,.lexi-audio,.v34-icon-btn');row.insertBefore(h50MakeSearchButton(term),before||null);});
    root.querySelectorAll('.wcard').forEach(card=>{if(card.querySelector('.hz-word-search-btn'))return;const term=h50ExtractTerm(card.querySelector('.ww')||card);let target=card.querySelector('.flash-word-row');if(target){target.classList.add('hz-word-row-actions');target.appendChild(h50MakeSearchButton(term));}else{let actions=card.querySelector('.hz-word-card-actions');if(!actions){actions=document.createElement('div');actions.className='hz-word-card-actions';card.appendChild(actions);}actions.appendChild(h50MakeSearchButton(term));card.classList.add('hz-word-card-has-actions');}});
    root.querySelectorAll('.tip-actions,#tip .tip-left').forEach(row=>{if(row.querySelector('.hz-word-search-btn'))return;const term=(typeof defWord!=='undefined'&&defWord)||document.getElementById('tip-wd')?.textContent||'';const b=h50MakeSearchButton(term);if(row.id==='tip-actions')row.insertBefore(b,row.firstChild);else row.appendChild(b);});
  }catch{}
}
function h50CleanupLocalStorageDict(){
  try{const del=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(/^hzdict\.cache\./.test(k))del.push(k);}del.forEach(k=>localStorage.removeItem(k));}catch{}
}
function h50PatchFunctions(){
  try{window.v40ToSimplified=h50SimpSync;window.v40NormalizeText=h50SimpSync;}catch{}
  try{if(typeof cleanRaw==='function'&&!cleanRaw.__h50){const old=cleanRaw;cleanRaw=function(raw){return h50SimpSync(old.call(this,raw));};cleanRaw.__h50=true;window.cleanRaw=cleanRaw;}}catch{}
  try{if(typeof buildHTML==='function'&&!buildHTML.__h50){const old=buildHTML;buildHTML=function(text){return old.call(this,h50SimpSync(text));};buildHTML.__h50=true;window.buildHTML=buildHTML;}}catch{}
  try{if(typeof getWordPY==='function'&&!getWordPY.__h50){const old=getWordPY;getWordPY=function(word){return old.call(this,h50SimpSync(word));};getWordPY.__h50=true;window.getWordPY=getWordPY;}}catch{}
  try{if(typeof lookupAll==='function'&&!lookupAll.__h50){const old=lookupAll;lookupAll=async function(word){return old.call(this,await h50Simp(word));};lookupAll.__h50=true;window.lookupAll=lookupAll;}}catch{}
  try{if(typeof lookupTatoebaExamples==='function'&&!lookupTatoebaExamples.__h50){const old=lookupTatoebaExamples;lookupTatoebaExamples=async function(word,limit){const q=await h50Simp(word);const rows=await old.call(this,q,limit);return (rows||[]).map(s=>({...s,text:h50SimpSync(s.text||s.zh||''),zh:h50SimpSync(s.zh||s.text||'')}));};lookupTatoebaExamples.__h50=true;window.lookupTatoebaExamples=lookupTatoebaExamples;}}catch{}
  try{if(typeof v29Tatoeba==='function'&&!v29Tatoeba.__h50){const old=v29Tatoeba;v29Tatoeba=async function(q){const qs=await h50Simp(q);const rows=await old.call(this,qs);return (rows||[]).map(s=>({...s,zh:h50SimpSync(s.zh||s.text||''),text:h50SimpSync(s.text||s.zh||'')}));};v29Tatoeba.__h50=true;window.v29Tatoeba=v29Tatoeba;}}catch{}
  try{if(typeof v29RunDict==='function'&&!v29RunDict.__h50){const old=v29RunDict;v29RunDict=async function(q){q=await h50Simp(q);const inp=document.getElementById('dict-q');if(inp)inp.value=q;try{v29DictTerm=q;}catch{}return old.call(this,q);};v29RunDict.__h50=true;window.v29RunDict=v29RunDict;}}catch{}
  ['v29RenderDictDefs','v29RenderDictWords','v29RenderDictSentences'].forEach(name=>{try{let fn=eval(name);if(typeof fn==='function'&&!fn.__h50){const old=fn;const wrap=async function(q,out){q=await h50Simp(q);const res=await old.call(this,q,out);h50SimplifyTextNodes(out||document.getElementById('dict-results'));h50InjectSearchButtons(out||document.getElementById('dict-results'));return res;};wrap.__h50=true;eval(name+'=wrap');window[name]=wrap;}}catch{}});
  try{if(typeof v30DictJump==='function'&&!v30DictJump.__h50){const old=v30DictJump;v30DictJump=function(w){return old.call(this,h50SimpSync(w));};v30DictJump.__h50=true;window.v30DictJump=v30DictJump;}}catch{}
  try{if(typeof speakWordMode==='function'&&!speakWordMode.__h50){const old=speakWordMode;speakWordMode=function(word,mode){return old.call(this,h50SimpSync(word),mode);};speakWordMode.__h50=true;window.speakWordMode=speakWordMode;}}catch{}
  try{if(typeof h36Speak==='function'&&!h36Speak.__h50){const old=h36Speak;h36Speak=function(text,opts){return old.call(this,h50SimpSync(text),opts);};h36Speak.__h50=true;window.h36Speak=h36Speak;}}catch{}
  try{if(window.v36Speak&&!window.v36Speak.__h50){const old=window.v36Speak;window.v36Speak=function(text,kind){return old.call(this,h50SimpSync(text),kind);};window.v36Speak.__h50=true;try{v36Speak=window.v36Speak;}catch{}}}catch{}
}
function h50PatchScreens(){
  try{document.body.classList.add('hz-ui-ready');}catch{}
  try{const sd=document.getElementById('sd');if(sd&&!sd.classList.contains('active'))sd.classList.add('hz-legacy-hidden');}catch{}
  h50SimplifyTextNodes(document.getElementById('rtext'));
  h50SimplifyTextNodes(document.getElementById('dict-results'));
  h50SimplifyTextNodes(document.getElementById('tip'));
  h50InjectSearchButtons(document);
}
function h50Observe(){
  try{
    const obs=new MutationObserver(muts=>{if(h50ObserverBusy)return;for(const m of muts){const n=m.target&&m.target.nodeType===1?m.target:(m.target&&m.target.parentElement);if(n){h50SimplifyTextNodes(n);h50InjectSearchButtons(n);}}});
    ['rtext','dict-results','tip','wc','flash-body'].forEach(id=>{const el=document.getElementById(id);if(el)obs.observe(el,{childList:true,subtree:true,characterData:true});});
  }catch{}
}
async function h50Boot(){
  h50CleanupLocalStorageDict();
  h50PatchFunctions();
  h50PatchScreens();
  h50Observe();
  await h50EnsureTradDb();
  h50PatchFunctions();
  h50PatchScreens();
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',h50Boot);else h50Boot();
[120,480,1100,2400,4600,7200,9800].forEach(ms=>setTimeout(()=>{h50PatchFunctions();h50PatchScreens();h50EnsureTradDb().then(()=>{h50PatchFunctions();h50PatchScreens();});},ms));
})();
