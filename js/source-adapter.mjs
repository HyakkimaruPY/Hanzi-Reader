...
let _showScreenRef = null;
let _renderLibRef = null;
let discoverSignature = '';
let discoverRenderPromise = null;
...
function renderDiscoverLoading(){
  ...
  if(!dc)return;
  dc.innerHTML='<div class="hsrc-wrap hsrc-loading" aria-busy="true"><div class="hsrc-head"><div><div class="hsrc-title">Fontes</div><div class="hsrc-sub">Carregando conteúdos locais…</div></div></div><div class="hsrc-loading-card"><span class="hsrc-loading-spin" aria-hidden="true"></span><span>Preparando a biblioteca de fontes</span></div></div>';
}

function commitDiscoverLocal(dc){
  const list = filtered();
  const signature=JSON.stringify([filters.hsk,filters.genre,filters.type,items.length,list.map(x=>x.id)]);
  if(signature===discoverSignature&&dc.querySelector('.hsrc-wrap:not(.hsrc-loading)'))return false;
  const template=document.createElement('template');
  template.innerHTML=`<div class="hsrc-wrap"><div class="hsrc-head"><div><div class="hsrc-title">Fontes</div><div class="hsrc-sub">Histórias progressivas em JSON, salvas dentro do projeto. Origem: Local.</div></div><div class="hsrc-head-actions"><span class="hsrc-badge">${items.length} conteúdos</span><button class="hsrc-close" type="button" data-hsrc-close aria-label="Fechar fontes">×</button></div></div>${renderFilters()}<div class="hsrc-grid">${list.map(card).join('') || '<div class="hsrc-empty">Nenhum conteúdo combina com os filtros ativos.</div>'}</div></div>`;
  dc.replaceChildren(template.content);
...
async function renderDiscoverLocal(){
  const dc = document.getElementById('dc');
  if(!dc) return;
  // Com o manifesto em memória, a montagem é atômica e síncrona. Assim a
  // tela nunca fica ativa exibindo o renderer legado ou um estado parcial.
  if(loaded){commitDiscoverLocal(dc);return;}
  if(discoverRenderPromise)return discoverRenderPromise;
  if(!dc.querySelector('.hsrc-wrap'))renderDiscoverLoading();
  discoverRenderPromise=(async()=>{
    try{
      await loadIndex();
      commitDiscoverLocal(dc);
    }catch(e){
      const error=document.createElement('div');error.className='hsrc-error';error.textContent='Erro ao carregar sources locais: '+(e.message||e);dc.replaceChildren(error);
    }finally{discoverRenderPromise=null;}
  })();
  return discoverRenderPromise;
}

function wire(root){
  root.querySelectorAll('[data-hsrc-close]').forEach(b=>b.onclick=()=>{try{showScreen('sl');}catch{}try{ensureLibraryImportUi();}catch{}});
  root.querySelectorAll('[data-hsrc-filter]').forEach(b=>b.onclick=()=>{const k=b.dataset.hsrcFilter;const v=b.dataset.val;filters[k]=String(filters[k]||'')===String(v)?null:v;discoverSignature='';renderDiscoverLocal();});
...
async function renderText(ctx,text,label,progress){
  ...
  try{
    if(typeof window.hzProgressiveReader==='function')await window.hzProgressiveReader(text||'',initialPct);
    else if(rtext){rtext.replaceChildren();rtext.insertAdjacentHTML('afterbegin',window.buildHTML?buildHTML(text||''):esc(text||'').replace(/\n/g,'<br>'));window.applyPinyin?.();}
...
async function addToLibrary(id){
  try{
    ...
    queueMicrotask(()=>{try{if(typeof loadLib==='function')void loadLib();else if(typeof renderLib==='function')renderLib();}catch{try{renderLib&&renderLib();}catch{}}});
  }catch(e){
...
function ensureReadSourcesButton(){
  ...
  if(!btn){
    ...
    div.innerHTML='<div class="iico url"><svg fill="none" height="20" stroke="white" stroke-width="2" viewBox="0 0 24 24" width="20"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"></path></svg></div><div><div class="ilbl">Fontes de leitura</div><div class="isub">Importar uma leitura local do projeto</div></div>';
...
function ensureLibraryImportUi(){
  try{
    ...
    if(mode==='books' && tools){
      ...
      // O renderizador principal recria este botão; por isso a ação local é
      // reassumida em toda renderização, inclusive quando o botão já existia.
      btn.onclick=(ev)=>{ev?.preventDefault?.();ev?.stopPropagation?.();openLocalSources('book');};
      btn.__hsrcFixed=true;
    }
    const simpleBtn=document.getElementById('simple-import-chip');
    if(simpleBtn && !simpleBtn.__hsrcFixed){
      const old=simpleBtn.onclick;
      simpleBtn.onclick=(ev)=>{try{v29ImportContext='simple';}catch{} if(typeof old==='function')old.call(simpleBtn,ev); requestAnimationFrame(()=>{restoreImportModalLinks();ensureReadSourcesButton();});};
...
function forceLocalDiscover(){
  ...
  const active=document.getElementById('sd')?.classList.contains('active');
  const dc=document.getElementById('dc');
  if(active&&dc&&!loaded&&!dc.querySelector('.hsrc-wrap'))renderDiscoverLoading();
}
function patchShowScreen(){
  const current = (typeof showScreen === 'function') ? showScreen : window.showScreen;
  if(typeof current !== 'function') return;
  if(current.__hsrc){_showScreenRef=current;return;}
  if(current===_showScreenRef)return;
  const patched=function(id){
    if(id==='sd'){
      if(loaded)void renderDiscoverLocal();
      else if(!document.getElementById('dc')?.querySelector('.hsrc-wrap'))renderDiscoverLoading();
    }
    ...
    queueMicrotask(()=>{
      ...
      if(id==='sd'&&!loaded)void renderDiscoverLocal();
    });
    return r;
  };
  patched.__hsrc = true;
  patched.__hsrcBase = current;
  try{window.showScreen = patched;}catch{}
  try{showScreen = patched;}catch{}
  _showScreenRef = patched;
}
function patchRenderLib(){
  const current = (typeof renderLib === 'function') ? renderLib : window.renderLib;
  if(typeof current !== 'function') return;
  if(current.__hsrc){_renderLibRef=current;return;}
  if(current===_renderLibRef)return;
  const patched=function(){
    let signature='';
    try{
      const list=(window.books||books||[]).map(book=>[book.id,book.updatedAt||book.lastRead||book.addedAt||0,book.progress||0,book.lastChapterIndex||0,book.order??'']);
      signature=JSON.stringify([window.v29LibMode||v29LibMode,window.v29BookView||v29BookView,window.searchQ||searchQ,list]);
      const host=document.getElementById('bc');
      if(signature===libraryRenderSignature&&host?.children.length){queueMicrotask(ensureLibraryImportUi);return;}
    }catch{}
    const r=current.apply(this,arguments);
    if(signature)libraryRenderSignature=signature;
    queueMicrotask(ensureLibraryImportUi);
    return r;
  };
  patched.__hsrc = true;
  patched.__hsrcBase = current;
  ...
  _renderLibRef = patched;
}
function boot(){
  forceLocalDiscover();patchShowScreen();patchRenderLib();
  restoreImportModalLinks();ensureReadSourcesButton();disableLegacyOnlineSources();ensureLibraryImportUi();
  // Pré-carrega apenas o manifesto. O bootstrap principal aguarda esta
  // promessa para não liberar a navegação antes de Fontes estar preparada.
  window.hsrcReadyPromise=loadIndex().catch(error=>{
    console.warn('[source-adapter] preload',error);
    return [];
  });
  if(document.getElementById('sd')?.classList.contains('active')){
    renderDiscoverLoading();void renderDiscoverLocal();
  }
}
window.hsrcRenderDiscoverLocal = renderDiscoverLocal;
window.hsrcOpenLocalSources = openLocalSources;
boot();
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});
window.addEventListener('hz:screen-visible',event=>{if(event.detail?.id==='sd')void renderDiscoverLocal();},{passive:true});
})();
