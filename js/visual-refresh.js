...
const copy=()=>lang()==='en'?{
  ...
  writing:'Escritura de Hanzi',writingSub:'Traza caracteres y practica la memoria motora',character:'Carácter para practicar',random:'Nuevo',undo:'Deshacer',clear:'Limpiar',done:'Concluir práctica',strokes:'trazos',saved:'Práctica registrada',
  profileSummary:'Estudio de hoy',goal:'de una meta diaria de 30 min',streak:'días de racha',best:'récord',activities:'Actividades',accumulated:'Estudio acumulado',last14:'Actividad · últimos 14 días',today:'Hoy',
  read:'Lectura',game:'Juego de tonos',dictionary:'Diccionario',review:'Repaso',audio:'Audio escuchado',searched:'Palabras buscadas',wordReview:'Palabras repasadas',sentenceReview:'Frases repasadas',minutes:'min'
}:{
...
const writingIcon='<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="8" y="8" width="48" height="48" rx="7" opacity=".58"/><path d="M32 8v48M8 32h48M14 14l36 36M50 14L14 50" stroke-dasharray="2.5 4" opacity=".28"/><path d="M23 23h17M27 23c0 9-3 15-8 19M29 34h15M36 27v18M28 45h17" opacity=".82"/><path d="M45 13l6 6-15 15-8 2 2-8 15-15z" fill="rgba(var(--ac-rgb),.08)"/><path d="M45 13l6 6M30 28l6 6"/></svg>';
function ensureWritingCard(){
  ...
  const production=$('[data-hzle-skill="production"] .hzle-practice-list');
  if(production)production.appendChild(card);else game.insertAdjacentElement('afterend',card);
  card.addEventListener('click',openWriting);
  document.dispatchEvent(new CustomEvent('hz:practice-card-ready',{bubbles:true,detail:{id:'hzp-writing'}}));
}
function openWriting(){
  // v5.1: a atividade foi reconstruída em js/hanzi-writing.mjs (busca de
  // ideograma, GIF + Passos, avaliação por traços e tela de conclusão).
  // O módulo antigo permanece apenas como fallback.
  if(typeof window.hzOpenWriting==='function'){window.hzOpenWriting();return;}
  try{window.hzPreloadCelebration?.();}catch{}
  const mount=root=>{
    const panel=document.createElement('div');panel.id='hz-writing-panel';panel.className='hanzi-writing-game';root.appendChild(panel);const c=copy();
    panel.innerHTML=`<div class="hzwr-shell"><div class="hzwr-head"><button class="hzwr-back" id="hzwr-back" aria-label="Voltar"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg></button><div class="hzwr-title"><h2>${esc(c.writing)}</h2><p>${esc(c.writingSub)}</p></div><div class="hzwr-session-score"><strong id="hzwr-completed">0</strong><span>${lang()==='en'?'written':'escritos'}</span></div></div><div class="hzwr-board"><div class="hzwr-controls"><input class="hzwr-input" id="hzwr-input" value="你" maxlength="2" aria-label="${esc(c.character)}"><button class="hzwr-random" id="hzwr-random">${esc(c.random)}</button></div><div class="hzwr-canvas-wrap"><canvas id="hz-writing-canvas"></canvas></div><div class="hzwr-actions"><button id="hzwr-undo">${esc(c.undo)}</button><button id="hzwr-clear">${esc(c.clear)}</button><button class="pri" id="hzwr-done">${esc(c.done)}</button></div><div class="hzwr-suggestions">${writingChars.map((ch,i)=>`<button class="hzwr-char ${i===0?'on':''}" data-char="${ch}">${ch}</button>`).join('')}</div><div class="hzwr-status"><span>${esc(c.character)}: <strong id="hzwr-current">你</strong></span><span><strong id="hzwr-count">0</strong>/<strong id="hzwr-target-count">7</strong> ${esc(c.strokes)}</span></div></div></div>`;
    const session=initWritingCanvas(panel);$('#hzwr-back',panel).onclick=()=>session.finish();return()=>{try{session.cleanup?.();}catch{}panel.remove();};
  };
  if(typeof window.hzMountPracticeActivity==='function'){window.hzMountPracticeActivity('hanzi-writing',mount);return;}
  const hub=$('#hz-sp-hub'),host=$('#hz-sp-host');if(!hub||!host)return;hub.style.display='none';host.style.display='flex';host.replaceChildren();mount(host);
}

function initWritingCanvas(panel){
  const canvas=$('#hz-writing-canvas',panel),wrap=canvas.parentElement,input=$('#hzwr-input',panel),count=$('#hzwr-count',panel),targetCount=$('#hzwr-target-count',panel),current=$('#hzwr-current',panel),completedEl=$('#hzwr-completed',panel);
  let paths=[],active=null,target='你',ctx,dpr=1,w=0,h=0,completed=0,attempts=0,finished=false;
  function expected(){return writingStrokeCounts[target]||Math.max(1,Array.from(target).length*4);}
  function syncCounts(){count.textContent=String(paths.length);targetCount.textContent=String(expected());completedEl.textContent=String(completed);$('#hzwr-done',panel).disabled=paths.length!==expected();}
  function setTarget(value){
    const chars=Array.from(String(value||'').trim());target=chars[0]||'你';
    input.value=target;current.textContent=target;paths=[];syncCounts();
    $$('.hzwr-char',panel).forEach(b=>b.classList.toggle('on',b.dataset.char===target));draw();
  }
  function resize(){
    const r=wrap.getBoundingClientRect();if(!r.width||!r.height)return;dpr=Math.min(2,window.devicePixelRatio||1);
    w=Math.max(260,Math.round(r.width));h=Math.max(260,Math.round(r.height));
    canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);canvas.style.width=w+'px';canvas.style.height=h+'px';ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);draw();
  }
  function grid(){
    ctx.clearRect(0,0,w,h);ctx.fillStyle='#f2e9dc';ctx.fillRect(0,0,w,h);ctx.strokeStyle='rgba(116,75,42,.22)';ctx.lineWidth=1;ctx.strokeRect(.5,.5,w-1,h-1);ctx.setLineDash([7,7]);
    [[w/2,0,w/2,h],[0,h/2,w,h/2],[0,0,w,h],[w,0,0,h]].forEach(a=>{ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(a[2],a[3]);ctx.stroke();});
    ctx.setLineDash([]);ctx.fillStyle='rgba(86,56,33,.14)';ctx.font=`${Math.round(w*.62)}px ${getComputedStyle(document.documentElement).getPropertyValue('--rf')||'serif'}`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(target,w/2,h/2+4);
  }
  function draw(){if(!ctx)return;grid();ctx.strokeStyle='#2e2820';ctx.lineWidth=Math.max(5,w*.018);ctx.lineCap='round';ctx.lineJoin='round';paths.forEach(path=>{if(path.length<2)return;ctx.beginPath();ctx.moveTo(path[0].x,path[0].y);path.slice(1).forEach(point=>ctx.lineTo(point.x,point.y));ctx.stroke();});}
  function point(e){const r=canvas.getBoundingClientRect();return{x:(e.clientX-r.left)*w/r.width,y:(e.clientY-r.top)*h/r.height};}
  const onDown=e=>{e.preventDefault();canvas.setPointerCapture?.(e.pointerId);active=[point(e)];paths.push(active);syncCounts();draw();};
  const onMove=e=>{if(!active)return;e.preventDefault();active.push(point(e));draw();};
  const onEnd=()=>{active=null;};
  canvas.addEventListener('pointerdown',onDown);canvas.addEventListener('pointermove',onMove);canvas.addEventListener('pointerup',onEnd);canvas.addEventListener('pointercancel',onEnd);
  input.addEventListener('input',()=>setTarget(input.value));$$('.hzwr-char',panel).forEach(button=>button.onclick=()=>setTarget(button.dataset.char));
  $('#hzwr-random',panel).onclick=()=>setTarget(writingChars[Math.floor(Math.random()*writingChars.length)]);
  $('#hzwr-clear',panel).onclick=()=>{paths=[];syncCounts();draw();};$('#hzwr-undo',panel).onclick=()=>{paths.pop();syncCounts();draw();};
  $('#hzwr-done',panel).onclick=()=>{
    attempts++;if(paths.length!==expected()){try{window.toast?.(lang()==='en'?'Match the exact reference stroke count first.':'Iguale primeiro a quantidade exata de traços de referência.');}catch{}return;}
    completed++;syncCounts();try{window.hzStat?.bump('wRev');}catch{}try{window.toast?.(copy().saved);}catch{}
    const next=writingChars[(writingChars.indexOf(target)+1+writingChars.length)%writingChars.length];setTarget(next);
  };
  function finish(){
    if(finished)return;finished=true;
    if(!completed){window.hzBackToHub?.();return;}
    const accuracy=Math.round(completed/Math.max(completed,attempts)*100);
    if(typeof window.hzShowPracticeCelebration==='function')window.hzShowPracticeCelebration({title:lang()==='en'?'Writing session complete!':'Sessão de escrita concluída!',score:completed,total:Math.max(completed,attempts),percent:accuracy,scoreLabel:String(completed),subtitle:lang()==='en'?`${completed} Hanzi completed by reference stroke count.`:`${completed} Hanzi concluído(s) pela contagem de traços de referência.`,stats:[{label:lang()==='en'?'Hanzi written':'Hanzi escritos',value:completed},{label:lang()==='en'?'Attempts':'Tentativas',value:Math.max(completed,attempts)}],onAgain:()=>{finished=false;completed=0;attempts=0;setTarget(target);},onClose:()=>window.hzBackToHub?.()});
    else window.hzBackToHub?.();
  }
  let resizeObserver=null;if('ResizeObserver' in window){resizeObserver=new ResizeObserver(()=>requestAnimationFrame(resize));resizeObserver.observe(wrap);}else window.addEventListener('resize',resize,{passive:true});
  requestAnimationFrame(()=>{resize();syncCounts();});
  function cleanup(){resizeObserver?.disconnect();window.removeEventListener('resize',resize);canvas.removeEventListener('pointerdown',onDown);canvas.removeEventListener('pointermove',onMove);canvas.removeEventListener('pointerup',onEnd);canvas.removeEventListener('pointercancel',onEnd);}
  return{cleanup,finish,get completed(){return completed;}};
}
...
function boot(){
  ...
  document.addEventListener('hz:lang-change',()=>{
    document.getElementById('hzp-writing')?.remove();ensureWritingCard();
    if($('#spf-stats'))renderProfileRefresh();
    queueRefresh();
  },{passive:true});
