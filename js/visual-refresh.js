  ...
  host.innerHTML=`
    <div class="hzvr-summary-grid">
      ...
      <section class="hzvr-summary-card hzvr-streak-card">
        <div class="hzvr-streak-top"><div class="hzvr-fire">${svgIcons.flame}</div><div class="hzvr-streak-count"><strong>${streak.current}</strong><span>${esc(streakUnit)}</span></div><div class="hzvr-streak-best"><strong>${streak.best}</strong><span>${esc(c.best)}</span></div></div>
        ${weekHTML(streak)}
      </section>
    </div>
...
const observer=new MutationObserver(muts=>{
  let relevant=false;
  for(const mutation of muts){for(const node of mutation.addedNodes){if(node.nodeType!==1)continue;scanSelects(node);if(node.matches?.('#hz-sp-hub,#spf,#mini-dock,.mo,select')||node.querySelector?.('#hz-sp-hub,#spf,#mini-dock,.mo,select'))relevant=true;}}
