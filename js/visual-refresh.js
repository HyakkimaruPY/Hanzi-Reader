...
function renderProfileRefresh(){
  ...
  host.innerHTML=`
    <div class="hzvr-profile-grid">
      ...
      <div class="hzvr-streak-card"><div class="hzvr-fire">${svgIcons.flame}</div><div class="hzvr-streak-main"><strong>${streak.current}</strong><span>${esc(c.streak)}</span>${weekHTML(streak)}</div><div class="hzvr-streak-best"><strong>${streak.best}</strong><span>${esc(c.best)}</span></div></div>
    </div>
