...
function openWriting(){
  const mount=root=>{
    ...
    const cleanup=initWritingCanvas(panel);$('#hzwr-back',panel).onclick=()=>window.hzBackToHub?.();return()=>{try{cleanup?.();}catch{}panel.remove();};
  };
...
function initWritingCanvas(panel){
  ...
  if('ResizeObserver' in window){resizeObserver=new ResizeObserver(resize);resizeObserver.observe(wrap);}else window.addEventListener('resize',resize,{passive:true});
  requestAnimationFrame(resize);
  return()=>{resizeObserver?.disconnect();window.removeEventListener('resize',resize);try{canvas.releasePointerCapture?.();}catch{}};
}
