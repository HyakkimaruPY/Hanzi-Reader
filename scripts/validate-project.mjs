...
if(!script.includes("['music','Música',v29Svg('music')]")||!script.includes('window.hzOpenMusic')||learning.includes('hzPracticeAmbience.v1'))bad('Música não foi restaurada como funcionalidade própria.');
else ok('Música restaurada como aba independente, sem ambientação na Prática');
if(!learning.includes('class CentralAudioService')||!learning.includes('class ToneSequenceGame'))bad('áudio central ou jogo Sequência tonal ausente.');
else ok('Sequência tonal reutiliza camada central de áudio');
if(!learning.includes('this.queue=[]')||!learning.includes('pumpQueue()')||!learning.includes('activePriority'))bad('camada central de áudio não possui fila e prioridade explícitas.');
else ok('camada central de áudio possui fila, prioridade e cancelamento');
...
for(const rel of ['db/hsk-expanded.json','db/grammar-helpers.json','db/traditional-simplified.json','api/tts-edge.js','assets/panda-manhua-logo.svg','css/learning-engine.css','js/learning-core.mjs','js/learning-engine.mjs']){
