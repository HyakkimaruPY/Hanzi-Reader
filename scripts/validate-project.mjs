...
const tatoebaApi=read('api/tatoeba.js');
...
if(!script.includes('/api/tatoeba')||!tatoebaApi.includes("showtrans', 'all'"))bad('rota same-origin de frases ou consulta estável do Tatoeba ausente.');
else if(tatoebaApi.includes("trans:lang"))bad('rota de frases voltou a filtrar somente traduções antes de obter os exemplos.');
else ok('frases usam ponte same-origin e API Tatoeba v1 sem filtro restritivo');
