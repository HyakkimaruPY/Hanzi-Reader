...
async function H46_audioFromSsmlDirect(ssml,o=H46_getSettings()){const endpoint=await H46_endpoint();const url=`https://${endpoint.r}.tts.speech.microsoft.com/cognitiveservices/v1`;const headers={'Authorization':endpoint.t,'Content-Type':'application/ssml+xml','User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36 Edg/127.0.0.0','X-Microsoft-OutputFormat':o.format||'audio-24khz-48kbitrate-mono-mp3'};H46_log('edge.tts.h46','request SSML direto',{url,method:'POST',headers:{...headers,Authorization:String(endpoint.t||'').slice(0,48)+'...'},settings:o,ssml:H46_short(ssml,8000)},'');let response;try{response=await fetch(url,{method:'POST',headers,body:ssml});}catch(e){H46_log('edge.tts.h46','fetch SSML direto falhou',{error:e.message||String(e)},'error');throw e;}if(!response.ok){const text=await response.text().catch(()=> '');H46_log('edge.tts.h46','response erro Microsoft direto',{status:response.status,headers:H46_headersObj(response.headers),body:text},'error');throw new Error('Serviço de voz direto: '+response.status+' — '+text);}const blob=await response.blob();H46_log('edge.tts.h46','response áudio Microsoft direto',{status:response.status,headers:H46_headersObj(response.headers),blob:{size:blob.size,type:blob.type}},'ok');return blob;}
async function H46_audioFromSsmlApi(ssml,o=H46_getSettings()){if(!(location.protocol==='http:'||location.protocol==='https:'))throw new Error('API Edge TTS indisponível fora de HTTP/HTTPS.');const payload={ssml,format:o.format||'audio-24khz-48kbitrate-mono-mp3',quality:o.format||'audio-24khz-48kbitrate-mono-mp3',voice:o.voice||'zh-CN-XiaoxiaoNeural',style:o.style||'general',degree:o.degree||1.35,role:o.role||'',speed:o.speed||1,pitch:o.pitch||0,volume:o.volume||0,chunkMode:o.chunkMode||'line',linePause:o.linePause??450};H46_log('edge.api.h46','request /api/edge-tts',{url:'/api/edge-tts',method:'POST',payload:{...payload,ssml:H46_short(ssml,8000)}},'');let response;try{response=await fetch('/api/edge-tts',{method:'POST',headers:{'Content-Type':'application/json','X-HZ-App':'1'},body:JSON.stringify(payload),cache:'no-store'});}catch(e){H46_log('edge.api.h46','fetch /api/edge-tts falhou',{error:e.message||String(e)},'error');throw e;}if(!response.ok){const text=await response.text().catch(()=> '');H46_log('edge.api.h46','response erro /api/edge-tts',{status:response.status,headers:H46_headersObj(response.headers),body:H46_short(text,4000)},'error');throw new Error('API Edge TTS: '+response.status+' — '+text);}const blob=await response.blob();H46_log('edge.api.h46','response áudio /api/edge-tts',{status:response.status,headers:H46_headersObj(response.headers),blob:{size:blob.size,type:blob.type}},'ok');return blob;}
async function H46_audioFromSsml(ssml,o=H46_getSettings()){let last=null;if(location.protocol==='http:'||location.protocol==='https:'){try{return await H46_audioFromSsmlApi(ssml,o);}catch(e){last=e;H46_log('edge.api.h46','API falhou; tentando rota direta Microsoft',{error:e.message||String(e)},'error');}}try{return await H46_audioFromSsmlDirect(ssml,o);}catch(e){last=e;}throw last||new Error('Edge TTS falhou.');}
