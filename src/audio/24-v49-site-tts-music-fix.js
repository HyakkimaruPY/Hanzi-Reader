...
// -----------------------------------------------------------------------------
// Ajuste da ordem de resolução de áudio:
// O HTML único chama o engine direto (h42AudioFromSsml/H46_audioFromSsml) primeiro
// e somente em caso de falha recorre ao backend /api/edge-tts. Na build modular
// original, o código priorizava o backend e capturava a função de fallback no
// momento da importação, o que gerava "null" quando os módulos ainda não
// estavam carregados. Para reproduzir o comportamento correto, determinamos a
// função de fallback de forma dinâmica e invertemos a prioridade. Agora, a
// função abaixo tenta gerar áudio usando o engine local; em caso de erro, faz
// uma segunda tentativa via API. Se ambas falharem, propaga o último erro.

// Obtenha a função de engine local de forma lazy; ela só estará disponível
// após a inicialização de h42 ou h46.
function hz49GetPrevAudioFromSsml() {
  const fn = (window.h42AudioFromSsml || window.H46_audioFromSsml || null);
  return (typeof fn === 'function') ? fn : null;
}

async function hz49AudioFromSsml(ssml, settings = {}) {
  let lastErr = null;
  // Primeiro: tente o engine local (direto). Somente se existir e falhar,
  // passaremos para a chamada de API.
  try {
    const localFn = hz49GetPrevAudioFromSsml();
    if (localFn) {
      return await localFn(ssml, settings);
    }
  } catch (e) {
    lastErr = e;
    hz49Log('tts.direct.hz49', 'motor direto falhou', { error: e.message || String(e) }, 'error');
  }
  // Segundo: chame a API se estivermos em HTTP/HTTPS
  try {
    if (location.protocol === 'http:' || location.protocol === 'https:') {
      const b = await hz49ApiSsmlBlob(ssml, settings);
      hz49Log('tts.api.hz49', '/api/edge-tts retornou áudio', { size: b.size, type: b.type }, 'ok');
      return b;
    }
  } catch (e) {
    lastErr = e;
    hz49Log('tts.api.hz49', '/api/edge-tts falhou', { error: e.message || String(e), plain: hz49ExtractTextFromSsml(ssml).slice(0, 120) }, 'error');
  }
  // Se nada funcionou, lance o erro mais recente ou um genérico
  throw lastErr || new Error('TTS neural indisponível no momento');
}
async function hz49Speak(text,kind='auto'){
 ...
 try{if(typeof h36Busy==='function')h36Busy(true);else if(typeof setAudioBusy==='function')setAudioBusy(kind==='char'?'char':'natural',true);}catch{}
  try{
    // Recupere configurações do usuário (velocidade, voz, etc.) das APIs existentes
    ...
    // Tente construir SSML usando v36 ou h42 utilitários, se disponíveis
    ...
    try {
      ...
      await hz49PlayBlob(blob);
      return true;
    } catch (e) {
      // Registra falha e não tenta outras rotas de voz: a voz neural é o foco.
      hz49Log('tts.primary.hz49','TTS neural falhou',{error:e.message || String(e),kind,preview:text.slice(0,180)},'error');
      hz49Toast('Falha no TTS: ' + (e.message || e));
      return false;
    }
  } finally {
    try {
      if (typeof h36Busy === 'function') h36Busy(false);
      else if (typeof setAudioBusy === 'function') setAudioBusy(kind === 'char' ? 'char' : 'natural', false);
    } catch {}
  }
}
